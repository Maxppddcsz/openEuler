				   ...
				   pgoff_t ilx, int *nid)
{
	...
	case MPOL_INTERLEAVE:
		if (smart_grid_used())
			tnid = sched_grid_preferred_interleave_nid(pol);
		if (tnid == NUMA_NO_NODE)
			/* Override input node id */
			tnid = (ilx == NO_INTERLEAVE_INDEX) ?
		...
		struct mempolicy *pol, pgoff_t ilx, int nid)
{
	...
	if (pol->mode != MPOL_INTERLEAVE && smart_grid_used())
