...
{
	atomic_inc(&page->_refcount);
	reserve_page_from_buddy(HPAGE_PMD_NR, page);
	SetPageActive(page);
	...
	while (start < end_addr) {
		...
		if ((pte_entry[0] & PM_SWAP) && (count == 1)) {
			res = get_user_pages_remote(task->mm, start, 1,
				FOLL_TOUCH | FOLL_GET, &temp_page, NULL);
			if (!res) {
		...
		for (i = 0; i < count; i++) {
			if (pte_entry[i] & PM_SWAP) {
				res = get_user_pages_remote(task->mm, start_addr + i * PAGE_SIZE,
					1, FOLL_TOUCH | FOLL_GET, &tmp_page, NULL);
				if (!res) {
		...
		unsigned long start_addr, unsigned long end_addr)
{
	...
	vma = find_extend_vma_locked(mm, start_addr);
	if (!vma) {
	...
	if (start_addr == (start_addr & HPAGE_PMD_MASK) &&
		hugepage_vma_check(vma, vma->vm_flags, false, false, false)) {
		...
		struct page_map_entry *pme)
{
	...
	for (j = 0; j < pme->nr_pages; j++) {
		...
		if (redirect_start && (redirect_pages < redirect_start->redirect_pages) &&
			...
			folio = vma_alloc_folio(gfp, HPAGE_PMD_ORDER, vma, address, true);
			if (!folio) {
