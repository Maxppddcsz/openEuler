...
find_idlest_group(struct sched_domain *sd, struct task_struct *p, int this_cpu)
{
	...
	case group_has_spare:
		...
		if (sd->flags & SD_NUMA) {
			...
			int local_buys_cpus = local_sgs.group_weight - local_sgs.idle_cpus;
			int idlest_buys_cpus = idlest_sgs.group_weight - idlest_sgs.idle_cpus;
#ifdef CONFIG_NUMA_BALANCING
			int idlest_cpu;
			/*
			...
			imbalance = abs(idlest_buys_cpus - local_buys_cpus);
