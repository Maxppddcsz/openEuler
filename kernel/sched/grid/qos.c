...
int __init sched_grid_zone_init(void)
{
	...
	for (index = 0; index < SMART_GRID_ZONE_NR; index++) {
		if (!zalloc_cpumask_var(&sg_zone.cpus[index], GFP_KERNEL))
			return -ENOMEM;
	}
