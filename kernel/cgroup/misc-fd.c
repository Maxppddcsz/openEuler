...
bool is_misc_fd_init(void)
{
	return is_init;
}

void misc_fd_init(void)
{
	misc_cg_set_capacity(MISC_FD, U64_MAX);
	is_init = true;
}
...
int misc_fd_alloc_fd(struct files_struct *files, u64 n)
{
	if (!is_misc_fd_init())
		misc_fd_init();
	if (files != &init_files)
		return misc_cg_try_charge(MISC_FD, files->misc_cg, n);
	return 0;
}
...
{
	if (files != &init_files)
		return misc_cg_uncharge(MISC_FD, files->misc_cg, n);
}
...
int misc_fd_dup_fds(struct files_struct *newf)
{
	...
	err = misc_fd_alloc_fd(newf, fc_count_fds(newf));
