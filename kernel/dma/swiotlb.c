...
static void *
kh40000_swiotlb_alloc_buffer(struct device *dev, size_t size,
...
out_unmap:
	...
	swiotlb_tbl_unmap_single(dev, phys_addr, size, DMA_TO_DEVICE,
			DMA_ATTR_SKIP_CPU_SYNC);
out_warn:
	if (!(attrs & DMA_ATTR_NO_WARN) && pr_info_ratelimited()) {
...
static void *kh40000_swiotlb_alloc(struct device *dev, size_t size,
		dma_addr_t *dma_handle,	gfp_t gfp, unsigned long attrs)
{
	...
	if (!vaddr)
		vaddr = kh40000_swiotlb_alloc_buffer(dev, size,
					dma_handle, attrs);
	return vaddr;
}

static void kh40000_swiotlb_sync_single_for_cpu(struct device *dev,
...
static void kh40000_swiotlb_sync_sg_for_cpu(struct device *dev,
			...
			enum dma_data_direction dir)
{
	...
	for_each_sg(sgl, sg, nelems, i)
		kh40000_sync_single_dma_for_cpu(dev, sg_dma_address(sg),
						dir, 0);

	swiotlb_sync_sg_for_cpu(dev, sgl, nelems, dir);
}

static void kh40000_swiotlb_unmap_sg(struct device *dev,
				struct scatterlist *sgl, int nelems,
				enum dma_data_direction dir, unsigned long attrs)
{
	struct scatterlist *sg;
...
static void kh40000_swiotlb_unmap_page(struct device *dev, dma_addr_t addr,
				size_t size, enum dma_data_direction dir,
				unsigned long attrs)
{
	kh40000_sync_single_dma_for_cpu(dev, addr, dir, 0);
