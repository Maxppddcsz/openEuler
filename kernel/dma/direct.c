...
void *kh40000_dma_direct_alloc(struct device *dev, size_t size,
	...
	if (node == NUMA_NO_NODE) {
		...
		if (!(gfp & (GFP_DMA | GFP_DMA32))) {
			...
			page = __alloc_pages_nodemask(gfp | __GFP_HIGH,
					page_order, node, &nodemask);
		} else
			page = __alloc_pages_nodemask(gfp | __GFP_HIGH,
					page_order, node, NULL);
	}
...
static void kh40000_direct_unmap_page(struct device *dev, dma_addr_t addr,
...
static void kh40000_direct_sync_single_for_cpu(struct device *dev,
...
static void kh40000_direct_sync_sg_for_cpu(struct device *dev,
...
static void kh40000_direct_unmap_sg(struct device *dev,
				struct scatterlist *sgl, int nelems,
				enum dma_data_direction dir,
				unsigned long attrs)
{
