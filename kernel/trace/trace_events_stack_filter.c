...
addr_map_get(struct stack_filter_addr_map *addr_map, unsigned long key)
{
	...
	hlist_for_each_entry_rcu(node, &addr_map->map[idx], node) {
		if (node->key == key) {
			ret = node->value;
#ifdef __DEBUG
			pr_info("Addr map hit!\n");
#endif
			goto out;
		}
	}
#ifdef __DEBUG
	pr_info("Addr map miss.\n");
#endif
...
event_stack_filter_init(struct event_stack_filter *esf)
{
	...
	/* addr_map should be pre-allocated */
...
#undef pos
	...
	list_for_each_entry_reverse(faddr, &filter->addrs, list) {
		char end = faddr->list.prev == &filter->addrs ? '\n' : '/';
		if (faddr->addr != DSTARS_ADDR)
			pr_info(KERN_CONT "%ps%c", (void *)faddr->addr, end);
		else
			pr_info(KERN_CONT "**%c", end);
	}
#endif
...
int stack_filter_match(struct event_stack_filter *esf)
{
	...
	for (i = num_entries - 1; i >= 0; i--) {
		char name[KSYM_NAME_LEN];
		lookup_symbol_name(buf[i], name);
		pr_info(KERN_CONT "%s%c", name, "/\n"[i == 0]);
	}
#endif
