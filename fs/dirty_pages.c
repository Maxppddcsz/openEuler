...
unsigned long lock_word;	/* for exclusive access to buffer */
...
static void dump_dirtypages_sb(struct super_block *sb, struct seq_file *m)
{
	...
	unsigned long limit = READ_ONCE(buf_limit);
...
static ssize_t proc_buffer_write(struct file *filp, const char *buf, size_t count,
				 loff_t *offp)
{
	unsigned long new_buf_num, new_buf_size;
	char *new_buf_dirty;
	int ret = kstrtoul_from_user(buf, count, 10, &new_buf_num);
...
static ssize_t proc_limit_write(struct file *filp, const char *buf, size_t count,
				loff_t *offp)
{
	unsigned long limit;
	int ret = kstrtoul_from_user(buf, count, 10, &limit);
...
{
	struct proc_dir_entry *dirty_dir = proc_mkdir(DIRTY_ROOT, NULL);
