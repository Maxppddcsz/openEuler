		...
		if (pdir != SAU_FLG_TX) {
			...
			if (edif_entry) {
				ql_dbg(ql_dbg_edif, vha, 0x130f,
				    "%s: remove edif_entry %p, update_sa_index: 0x%x, "
				    "delete_sa_index: 0x%x\n",
				...
				ql_dbg(ql_dbg_edif, vha, 0x1310,
				    "%s: release edif_entry %p, update_sa_index: 0x%x, "
				    "delete_sa_index: 0x%x\n",
				    __func__, edif_entry, edif_entry->update_sa_index,
				    edif_entry->delete_sa_index);
	...
	if (((sa_frame.flags & SAU_FLG_TX) == 0) &&
		...
		if (!edif_entry) {
			ql_dbg(ql_dbg_edif, vha, 0x1342,
			    "%s: WARNING: no active sa_index for nport_handle 0x%x, forcing "
			    "delete for sa_index 0x%x\n", __func__, fcport->loop_id, sa_index);
		...
		if ( (sa_frame.flags & SAU_FLG_FORCE_DELETE) == SAU_FLG_FORCE_DELETE) {
			...
			ql_dbg(ql_dbg_edif, vha, 0x1343,
			    "%s: FORCE DELETE flag found for nport_handle 0x%x, sa_index 0x%x, "
			    "forcing DELETE\n", __func__, fcport->loop_id, sa_index);
			kfree(edif_entry);
			goto force_rx_delete;
		}
...
qla_enode_init(scsi_qla_host_t *vha)
{
	...
	if (vha->pur_cinfo.enode_flags == ENODE_ACTIVE) {
		...
		ql_dbg(ql_dbg_edif, vha, 0x134f, "%s enode still active\n",
		    __func__);
		return;
	}
...
qla_enode_stop(scsi_qla_host_t *vha)
{
	...
	list_for_each_entry_safe(node, q, &vha->pur_cinfo.head, list) {
		ql_dbg(ql_dbg_edif, vha, 0x1351, "%s freeing enode type="
		    "%x, cnt=%x\n", __func__, node->ntype,
		    node->dinfo.nodecnt);
		list_del_init(&node->list);
...
void qla_enode_clear(scsi_qla_host_t *vha, port_id_t portid)
{
	unsigned    long flags;
	struct enode    *e, *tmp;
...
void qla_edb_clear(scsi_qla_host_t *vha, port_id_t portid)
{
	unsigned long flags;
...
void qla24xx_sa_update_iocb(srb_t *sp, struct sa_update_28xx *sa_update_iocb)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x136f,
	    "%s SAU Port ID = %02x%02x%02x, flags=%xh, index=%u, ctl=%xh, SPI 0x%x flags 0x%x "
	    "hdl=%x gmac %d\n",
...
{
	struct edif_list_entry *indx_lst, *tindx_lst;
	struct list_head *indx_list = &fcport->edif.edif_indx_list;
	unsigned long flags = 0;
		...
		struct qla_sa_update_frame *sa_frame)
{
	...
	if (!entry) {
		...
		ql_dbg(ql_dbg_edif, vha, 0x138c,
		    "%s: Created new sadb entry for nport_handle 0x%x, spi 0x%x, returning "
		    "sa_index %d\n",
		...
		fc_port_t *fcport, uint32_t handle, uint16_t sa_index)
{
	...
	ql_dbg(ql_dbg_edif, vha, 0x1392,
	    "%s: invalidating delete_sa_index,  update_sa_index: 0x%x sa_index: 0x%x, "
	    "delete_sa_index: 0x%x\n",
