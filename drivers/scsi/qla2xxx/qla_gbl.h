...
void qla_edif_sadb_release(struct qla_hw_data *);
int qla_edif_sadb_build_free_pool(struct qla_hw_data *);
void qla_edif_sadb_release_free_pool(struct qla_hw_data *);
void qla_chk_edif_rx_sa_delete_pending(scsi_qla_host_t *vha,
    srb_t *sp, struct sts_entry_24xx *sts24);
void qlt_chk_edif_rx_sa_delete_pending(scsi_qla_host_t *, fc_port_t *,
    struct ctio7_from_24xx *);

void qla2x00_release_all_sadb(struct scsi_qla_host *vha, struct fc_port *fcport);
int qla_edif_process_els(scsi_qla_host_t *, bsg_job_t *);
...
extern int ql2x_scmr_drop_pct;
extern int ql2x_scmr_drop_pct_low_wm;
extern int ql2x_scmr_up_pct;
...
struct edif_sa_ctl *qla_edif_find_sa_ctl_by_index(fc_port_t *, int , int );
...
extern void qla2x00_timer(qla_timer_arg_t timer_arg);
extern void qla2xxx_perf_timer(qla_timer_arg_t timer_arg);
 ...
 */
void qla_els_pt_iocb(struct scsi_qla_host *,
	struct els_entry_24xx *, struct qla_els_pt_arg *);
cont_a64_entry_t * qla2x00_prep_cont_type1_iocb(scsi_qla_host_t *,
    struct req_que *);
...
extern int qla24xx_issue_sa_replace_iocb(scsi_qla_host_t *,
	struct qla_work_evt *);
...
extern int qla26xx_dport_diagnostics_v2(scsi_qla_host_t *,
...
struct scsi_qla_host *qla_find_host_by_d_id(struct scsi_qla_host *, be_id_t d_id);
int qla24xx_async_notify_ack(scsi_qla_host_t *, fc_port_t *,
	struct imm_ntfy_from_isp *, int);
...
void qla_remove_hostmap(struct qla_hw_data *);
...
extern void qla_nvme_abort_process_comp_status(
		struct abort_entry_24xx *abt, srb_t *sp);
void qlt_handle_fast_error(struct scsi_qla_host *, struct qla_tgt_cmd *,
       void *);
...
/* qla_edif.c */
fc_port_t * qla2x00_find_fcport_by_pid(scsi_qla_host_t *, port_id_t *);
void qla_edb_eventcreate(scsi_qla_host_t *, uint32_t, uint32_t, uint32_t,
    fc_port_t *);
void qla_edb_stop(scsi_qla_host_t *);
int32_t qla_edif_app_mgmt(bsg_job_t *);
int qla2x00_check_rdp_test( uint32_t cmd, uint32_t port);
void qla_enode_init(scsi_qla_host_t *);
void qla_enode_stop(scsi_qla_host_t *);
void qla_edif_flush_sa_ctl_lists(fc_port_t *fcport);
void qla_edb_init(scsi_qla_host_t *);
void qla_edif_timer(scsi_qla_host_t *vha);
int qla28xx_start_scsi_edif(srb_t *);
void qla24xx_sa_update_iocb(srb_t *, struct sa_update_28xx *);
void qla24xx_sa_replace_iocb(srb_t *, struct sa_update_28xx *);
void qla24xx_auth_els(scsi_qla_host_t *, void **, struct rsp_que **);
void qla24xx_purex_cont_entry(scsi_qla_host_t *, struct rsp_que *,
    sts_cont_entry_t *);
void qla28xx_sa_update_iocb_entry(scsi_qla_host_t *, struct req_que *,
    struct sa_update_28xx *);
void qla_parse_auth_els_ctl(struct srb *);
...
int qla2xxx_start_stats(struct Scsi_Host *, uint32_t flags);
int qla2xxx_stop_stats(struct Scsi_Host *, uint32_t flags);
int qla2xxx_reset_stats(struct Scsi_Host *, uint32_t flags);

int qla2xxx_get_ini_stats(struct Scsi_Host *, uint32_t flags, void *data, uint64_t size);
int qla2xxx_get_tgt_stats(struct Scsi_Host *, uint32_t flags, struct fc_rport *rport, void *data, uint64_t size);
int qla2xxx_disable_port(struct Scsi_Host *);
int qla2xxx_enable_port(struct Scsi_Host *);

uint64_t qla2x00_get_num_tgts(scsi_qla_host_t *vha);
uint64_t qla2x00_count_set_bits(uint32_t num);
