...
static void init_reg_v3_hw(struct hisi_hba *hisi_hba)
{
	...
	for (i = 0; i < hisi_hba->queue_count; i++) {
		...
		hisi_sas_write32(hisi_hba, DLVRY_Q_0_BASE_ADDR_LO +
				 (i * NEXT_DQCQ_REG_OFF),
				 lower_32_bits(hisi_hba->cmd_hdr_dma[i]));

		hisi_sas_write32(hisi_hba, DLVRY_Q_0_DEPTH +
				 (i * NEXT_DQCQ_REG_OFF),
				 HISI_SAS_QUEUE_SLOTS);

		/* Completion queue */
		hisi_sas_write32(hisi_hba, COMPL_Q_0_BASE_ADDR_HI +
				 (i * NEXT_DQCQ_REG_OFF),
				 upper_32_bits(hisi_hba->complete_hdr_dma[i]));

		hisi_sas_write32(hisi_hba, COMPL_Q_0_BASE_ADDR_LO +
				 (i * NEXT_DQCQ_REG_OFF),
				 lower_32_bits(hisi_hba->complete_hdr_dma[i]));

		hisi_sas_write32(hisi_hba, COMPL_Q_0_DEPTH +
				 (i * NEXT_DQCQ_REG_OFF),
				 HISI_SAS_QUEUE_SLOTS);
	}
...
static int reset_hw_v3_hw(struct hisi_hba *hisi_hba)
{
	...
	/* Ensure axi bus idle, with 20ms delay and 1s time out */
...
static void disable_phy_v3_hw(struct hisi_hba *hisi_hba, int phy_no)
{
	...
	/*
	 * Wait 1us till all misleading error code to have occured
	 */
	udelay(1);
	...
	irq_msk = hisi_sas_phy_read32(hisi_hba, phy_no, CHL_INT2_MSK);
	hisi_sas_phy_write32(hisi_hba, phy_no, CHL_INT2_MSK, (~msk) & irq_msk);
}
			  ...
			  struct hisi_sas_slot *slot)
{
	...
	hdr->dw0 = cpu_to_le32((1 << CMD_HDR_RESP_REPORT_OFF) |
			       (2 << CMD_HDR_TLR_CTRL_OFF) | /* no retransmit */
	...
	if (tmf) {
		...
		if (!HISI_SAS_IS_RW_CMD(scsi_cmnd->cmnd[0]))
			dw1 = 0 << CMD_HDR_VDTL_OFF;
		else
			dw1 = 1 << CMD_HDR_VDTL_OFF;
...
static irqreturn_t phy_up_v3_hw(int phy_no, struct hisi_hba *hisi_hba)
{
	...
	if (context & (1 << phy_no)) {
		...
		attached_sas_addr[0] = 0x50; /* sas address for SATA */
...
static void handle_chl_int2_v3_hw(struct hisi_hba *hisi_hba, int phy_no)
{
	...
	if (pci_dev->revision > PCIDEV_REVISION_1620_ES &&
	    (irq_value & msk)) {
		struct asd_sas_phy *sas_phy = &phy->sas_phy;
	...
	if ((irq_value & BIT(CHL_INT2_RX_INVLD_DW_OFF)) &&
	    (pci_dev->revision == PCIDEV_REVISION_1620_ES)) {
		u32 reg_value;
		int rc;
...
static irqreturn_t fatal_axi_int_v3_hw(int irq_no, void *p)
{
	...
	for (i = 0; i < ARRAY_SIZE(fatal_axi_error); i++) {
		...
		if (pdev->revision < PCIDEV_REVISION_1620_CS) {
			u32 reg_val;
...
static int interrupt_preinit_v3_hw(struct hisi_hba *hisi_hba)
{
	...
	shost->nr_hw_queues = hisi_hba->nvecs;
...
static int disable_host_v3_hw(struct hisi_hba *hisi_hba)
{
	...
	/* wait until bus idle, delay:10us, timeout:100us */
...
static const struct hisi_sas_debugfs_reg debugfs_port_reg = {
	...
	.count = 0x100, /* number of port regs */
	.base_off = PORT_BASE,
};

static const struct hisi_sas_debugfs_reg_lu debugfs_global_reg_lu[] = {
	HISI_SAS_DEBUGFS_REG(DLVRY_QUEUE_ENABLE),
	HISI_SAS_DEBUGFS_REG(IOST_BASE_ADDR_LO),
	HISI_SAS_DEBUGFS_REG(IOST_BASE_ADDR_HI),
	HISI_SAS_DEBUGFS_REG(ITCT_BASE_ADDR_LO),
	HISI_SAS_DEBUGFS_REG(ITCT_BASE_ADDR_HI),
	HISI_SAS_DEBUGFS_REG(IO_BROKEN_MSG_ADDR_LO),
	HISI_SAS_DEBUGFS_REG(IO_BROKEN_MSG_ADDR_HI),
	...
	HISI_SAS_DEBUGFS_REG(DLVRY_Q_0_BASE_ADDR_LO),
	HISI_SAS_DEBUGFS_REG(DLVRY_Q_0_BASE_ADDR_HI),
	...
	HISI_SAS_DEBUGFS_REG(COMPL_Q_0_BASE_ADDR_LO),
	HISI_SAS_DEBUGFS_REG(COMPL_Q_0_BASE_ADDR_HI),
...
static const struct hisi_sas_debugfs_reg debugfs_global_reg = {
	...
	.count = 0x800, /* number of global regs */
};
...
static int hisi_sas_map_queues(struct Scsi_Host *shost)
{
	...
	return blk_mq_pci_map_queues_by_qmap(qmap, hisi_hba->pci_dev,
					     HISI_SAS_CQ_INT_BASE_VECTORS_V3_HW);
}
					    ...
					    size_t count, loff_t *ppos)
{
	...
	hisi_hba->bist_loopback_phy_id = phy_no;
...
static const
struct hisi_sas_debugfs_reg *debugfs_reg_array_v3_hw[DEBUGFS_REGS_NUM] = {
...
static void debugfs_bist_init_v3_hw(struct hisi_hba *hisi_hba)
{
	...
	hisi_hba->bist_loopback_linkrate = SAS_LINK_RATE_1_5_GBPS;
}
