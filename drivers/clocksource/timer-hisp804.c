...
static u64 hisp804_clocksource_read(struct clocksource *cs)
{
	return ~(u64)readl_relaxed(to_hiclksrc(cs)->base + TIMER_VALUE);
}
...
static int hisp804_clockevent_setup(struct hisp804_clockevent_device *hiclkevt)
{
	...
	writel(0, hiclkevt->base + TIMER_CTRL);
	pr_debug("%s, irq=%d\n", __func__, clkevt->irq);
	WARN_ON(request_irq(clkevt->irq, hisp804_clockevent_timer_interrupt,
				IRQF_TIMER | IRQF_NOBALANCING, "a", &hiclkevt->action));
...
static void hisp804_clockevent_stop(struct hisp804_clockevent_device *hiclkevt)
{
	...
	free_irq(clkevt->irq, &hiclkevt->action);
...
TIMER_OF_DECLARE(hisp804, "hisilicon,hisp804", hisp804_timer_init);
