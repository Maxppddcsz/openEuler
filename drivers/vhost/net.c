...
static_assert(CONFIG_VHOST_NET_HIGH_FREQ_TX_INTERVAL_THRESHOLD >= 0);

static unsigned int high_freq_txi_threshold =
	(unsigned int)CONFIG_VHOST_NET_HIGH_FREQ_TX_INTERVAL_THRESHOLD;
...
MODULE_PARM_DESC(high_freq_txi_threshold,
		...
		__stringify(CONFIG_VHOST_NET_HIGH_FREQ_TX_INTERVAL_THRESHOLD));
	...
	do {
		...
		if (head == vq->num) {
			if (unlikely(busyloop_intr)) {
				vhost_poll_queue(&vq->poll);
			} else if (nvq->tx_interval < high_freq_txi_threshold &&
