 ...
 *
 * File Name	 : roce_srq_comm.c
 * Version	   : v2.0
 * Created	   : 2021/3/12
 * Last Modified : 2021/12/7
 * Description   : The definition of RoCE SRQ common functions.
 */
...
{
	return roce3_srq_buf_offset(srq->buf, (int)((u32)n << (unsigned)srq->wqe_shift));
}
...
void roce3_srq_async_event(struct roce3_device *rdev, struct roce3_srq *srq, int type)
{
	...
	if (ibsrq->event_handler) {
		...
		default:
			dev_warn_ratelimited(rdev->hwdev_hdl, "[ROCE] %s: unexpected event type(%d) on SRQ(%06x),\
