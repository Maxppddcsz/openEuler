 ...
 * File Name	 : roce_cq_cqe.c
 * Version	   : v2.0
 * Created	   : 2021/12/7
 * Last Modified : 2022/3/24
 * Description   : The definition of RoCE CQ kernel CQE related functions.
 */
...
static int roce3_cq_get_cur_qp(struct roce3_cq *cq, struct roce3_qp **cur_qp, struct roce_cqe *cqe)
{
	...
	if ((!*cur_qp) || (cqe->dw0.bs.qpn != (u32)(*cur_qp)->qpn)) {
		...
		cqm_obj_qp = cqm_object_get(rdev->hwdev, CQM_OBJECT_SERVICE_CTX, cqe->dw0.bs.qpn, false);
		if (cqm_obj_qp == NULL) {
...
**************************************************************************** */
static int roce3_cq_get_xrc_srq(struct roce3_cq *cq, struct roce3_srq **srq, struct roce_cqe *cqe, u32 qp_type)
{
	u32 srq_num = 0;
	struct roce3_device *rdev = NULL;
...
static int roce3_poll_recv_cqe(struct roce3_qp *cur_qp, struct roce3_srq *srq, struct roce_cqe trans_cqe,
	struct ib_wc *wc)
{
	u16 wqe_ctr = 0;
	unsigned int tail = 0;
...
static void roce3_poll_move_tail(const struct roce3_qp *cur_qp, struct roce3_wq *wq, struct roce_cqe *trans_cqe)
{
	u16 wqe_ctr = 0;
...
static void roce3_poll_one_get_av_info(const struct roce3_qp *cur_qp, struct ib_wc *wc, struct roce_cqe trans_cqe,
	struct roce_cqe *cqe)
{
	if ((cur_qp->qp_type == IB_QPT_UD) || (cur_qp->qp_type == IB_QPT_GSI)) {
		...
		wc->smac[0] = (u8)(trans_cqe.dw4.bs.smac_h >> 8); /* Take the first Byte data, shift 8bit */
...
static int roce3_poll_one_get_qp_and_srq(struct roce3_cq *cq, struct roce3_qp **cur_qp, struct ib_wc *wc,
	struct roce_cqe *trans_cqe, struct roce3_srq **srq)
{
	int ret = 0;
...
static int roce3_get_and_trans_cqe(struct roce3_device *rdev, struct roce3_cq *cq, struct roce_cqe *trans_cqe,
	int *is_send, struct roce_cqe **cqe)
{
	do {
		*cqe = roce3_next_cqe_sw(cq);
		...
		roce3_bytes_trans((u32 *)(void *)trans_cqe, (int)(sizeof(struct roce_cqe) / CQ_DW_TO_BYTE));
...
static void roce3_poll_send_cqe(struct roce3_qp *cur_qp, struct roce_cqe *trans_cqe, struct ib_wc *wc)
{
...
**************************************************************************** */
static int roce3_poll_one(struct roce3_device *rdev, struct roce3_cq *cq, struct roce3_qp **cur_qp, struct ib_wc *wc)
{
	int ret = 0;
...
int roce3_arm_cq(struct ib_cq *ibcq, enum ib_cq_notify_flags flags)
{
	...
	roce3_cq_arm(to_roce3_cq(ibcq),
		(((unsigned int)flags & IB_CQ_SOLICITED_MASK) == IB_CQ_SOLICITED) ? ROCE_CQ_DB_REQ_NOT_SOL : ROCE_CQ_DB_REQ_NOT,
