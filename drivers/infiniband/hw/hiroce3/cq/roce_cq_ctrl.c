 ...
 * File Name	 : roce_cq_ctrl.c
 * Version	   : v2.0
 * Created	   : 2021/12/7
 * Last Modified : 2022/2/21
 * Description   : Define standard kernel RoCE CQ control realted functions.
 */
...
static int roce3_cq_modify(struct roce3_device *rdev, struct roce3_cq *cq, u32 cnt, u32 period)
{
	...
	roce_cmd_modify_cq_s *cq_modify_inbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_modify_cq_s), NULL, 0);
	if (ret != 0) {
	...
	cq_modify_inbuf = (roce_cmd_modify_cq_s *)cqm_cmd_inbuf->buf;
	cq_modify_inbuf->com.index = cpu_to_be32(cq->cqn);
	cq_modify_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_CQ_BITMASK); //lint !e778
	cq_modify_inbuf->max_cnt = cpu_to_be32(cnt);
	cq_modify_inbuf->timeout = cpu_to_be32(period);

	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_MODIFY_CQ, cqm_cmd_inbuf, NULL, NULL,
		ROCE_CMD_TIME_CLASS_A, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: Failed to send MODIFY_CQ, cqn(0x%x), ret(%d), func_id(%hu)\n", __func__,
			cq->cqn, ret, rdev->glb_func_id);

		if (roce3_hca_is_present(rdev) != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(MODIFY_CQ), cqn(0x%x), func_id(%hu)\n", __func__,
				cq->cqn, rdev->glb_func_id);
...
static int roce3_cq_alloc_resize_buf(struct roce3_device *rdev, struct roce3_cq *rcq, int entries)
{
	...
	rcq->resize_buf = (struct roce3_cq_resize_buf *)kmalloc(sizeof(*rcq->resize_buf), GFP_ATOMIC);
	...
	ret = hiudk_cqm_object_resize_alloc_new(rdev->hwdev, &rcq->cqm_cq->object, (u32)rcq->resize_buf->buf.buf_size);
	if (ret != 0) {
	...
	struct ib_udata *udata)
{
	...
	rcq->resize_buf = (struct roce3_cq_resize_buf *)kmalloc(sizeof(*rcq->resize_buf), GFP_ATOMIC);
	if (rcq->resize_buf == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cq resize buffer, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -ENOMEM;
	}

	ret = roce3_cq_get_umem(rdev, udata, &rcq->resize_buf->buf, &rcq->resize_umem, ucmd.buf_addr, entries);
	if (ret != 0) {
...
static int roce3_cq_get_next_cqe(struct roce_cqe **cqe, struct roce3_cq *cq, unsigned int *i, u32 *times,
...
static int roce3_cq_resize_copy_cqes(struct roce3_cq *cq)
{
	...
	*/
	...
	while (cqe->dw1.bs.op_type != ROCE_OPCODE_RESIZE_CQE) {
		...
		new_cqe =
			(struct roce_cqe *)roce3_get_cqe_from_buf(&cq->resize_buf->buf, (i & ((unsigned)cq->resize_buf->cqe)));
		 ...
		 */
		new_cqe->dw0.bs.owner = ((i & ((unsigned)cq->resize_buf->cqe + 1)) != 0) ? 1 : 0;
...
static void roce3_cq_fill_resize_inbuf(struct roce3_device *rdev, struct roce3_cq *rcq, int page_shift,
	cqm_cmd_buf_s *cqm_cmd_inbuf)
{
	...
	u64 mtt_paddr = 0;
	u32 mtt_page_size = 0;
	roce_cmd_resize_cq_s *cq_resize_inbuf = NULL;
	...
	cq_resize_inbuf = (roce_cmd_resize_cq_s *)cqm_cmd_inbuf->buf;
	cq_resize_inbuf->com.index = cpu_to_be32(rcq->cqn);
	cq_resize_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_CQ_BITMASK); //lint !e778
	...
	cq_resize_inbuf->mtt_info.mtt_cache_line_start = cpu_to_be32(rdma_cap->dev_rdma_cap.roce_own_cap.cmtt_cl_start);
	cq_resize_inbuf->mtt_info.mtt_cache_line_end = cpu_to_be32(rdma_cap->dev_rdma_cap.roce_own_cap.cmtt_cl_end);
	cq_resize_inbuf->mtt_info.mtt_cache_line_size = cpu_to_be32(rdma_cap->dev_rdma_cap.roce_own_cap.cmtt_cl_sz);
}
...
static int roce3_cq_resize(struct roce3_device *rdev, struct roce3_cq *rcq, int page_shift)
{
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_resize_cq_s), NULL, 0);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
		return -ENOMEM;
	...
	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_RESIZE_CQ, cqm_cmd_inbuf, NULL, NULL,
		ROCE_CMD_TIME_CLASS_A, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
		...
		if (roce3_hca_is_present(rdev) != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(RESIZE_CQ), cqn(0x%x), func_id(%hu)\n", __func__,
				rcq->cqn, rdev->glb_func_id);
...
**************************************************************************** */
static int roce3_user_cq_resize(struct roce3_device *rdev, struct roce3_cq *rcq, int entries, struct ib_udata *udata)
{
	int page_shift = 0;
	int ret = 0;
...
**************************************************************************** */
static int roce3_resize_user_cq(struct roce3_device *rdev, struct roce3_cq *rcq, int entries, struct ib_udata *udata)
{
	int ret = 0;
	struct roce3_resize_cq_cmd ucmd = { 0 };
	...
	/* The new depth of CQ cannot be the same as the old depth. A special CQE space is reserved when CQ is created,
...
static int roce3_kernel_cq_resize(struct roce3_device *rdev, struct roce3_cq *rcq, int entries)
{
	...
	if (entries < (outst_cqe + 1)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Can't resize, \
			because smaller than the number of outstanding CQES, func_id(%d)\n",
			__func__, rdev->glb_func_id);
		return -EINVAL;
	}
...
void roce3_lock_cqs(struct roce3_cq *roce3_send_cq, struct roce3_cq *roce3_recv_cq) __acquires(&roce3_send_cq->lock)
	__acquires(&roce3_recv_cq->lock)
{
...
void roce3_unlock_cqs(struct roce3_cq *roce3_send_cq, struct roce3_cq *roce3_recv_cq) __releases(&roce3_send_cq->lock)
	__releases(&roce3_recv_cq->lock)
{
	if (roce3_send_cq == roce3_recv_cq) {
