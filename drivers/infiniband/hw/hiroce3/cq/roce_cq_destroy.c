 ...
 * File Name	 : roce_cq_destroy.c
 * Version	   : v2.0
 * Created	   : 2021/12/7
 * Last Modified : 2022/2/21
 * Description   : Define standard kernel RoCE CQ destroy realted functions.
 */
	...
	struct roce_cq_context *cqc_data, u32 check_state)
{
	...
	if (read_back_flag == 0) {
		dev_err(rdev->hwdev_hdl,
			...
			rdev->try_times, cqc_data->dw2.value, (u32)rdev->glb_func_id);
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Cqn(0x%x), timer_dw(0x%x), func_id(%u)\n", __func__, cq->cqn,
			cqc_data->dw2.value, (u32)rdev->glb_func_id);
		return -1;
	}
...
int roce3_cq_hw2sw(struct roce3_device *rdev, struct roce3_cq *cq)
{
	...
	roce_cmd_cq_hw2sw_s *cq_hw2sw_inbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_cq_hw2sw_s), NULL, 0);
	if (ret != 0) {
	...
	cq_hw2sw_inbuf = (roce_cmd_cq_hw2sw_s *)cqm_cmd_inbuf->buf;
	cq_hw2sw_inbuf->com.index = cpu_to_be32((u32)cq->cqn);
	cq_hw2sw_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_CQ_BITMASK); //lint !e778

	cqc_data = (struct roce_cq_context *)((void *)cq->cqm_cq->q_ctx_vaddr);
	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_HW2SW_CQ, cqm_cmd_inbuf, NULL, NULL,
	...
	if (ret != 0) {
		...
		if (roce3_hca_is_present(rdev) != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(HW2SW_CQ), cqn(0x%x), func_id(%hu)\n", __func__,
...
static int roce3_cq_cache_out(struct roce3_device *rdev, struct roce3_cq *cq)
{
	...
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	roce_cmd_cq_cache_invalidate_s *cq_cache_invld_inbuf = NULL;

	/* Send the cache out command */
	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_cq_cache_invalidate_s), NULL,
		0);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
	...
	cq_cache_invld_inbuf = (roce_cmd_cq_cache_invalidate_s *)cqm_cmd_inbuf->buf;
	cq_cache_invld_inbuf->com.index = cpu_to_be32((u32)cq->cqn);
	cq_cache_invld_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_CQ_BITMASK); //lint !e778
	...
	cq_cache_invld_inbuf->mtt_info.mtt_cache_line_end = cpu_to_be32(rdma_cap->dev_rdma_cap.roce_own_cap.cmtt_cl_end);
	cq_cache_invld_inbuf->mtt_info.mtt_cache_line_size = cpu_to_be32(rdma_cap->dev_rdma_cap.roce_own_cap.cmtt_cl_sz);

	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_MISC_CQ_CACHE_INVLD, cqm_cmd_inbuf, NULL, NULL,
		ROCE_CMD_TIME_CLASS_A, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
		...
		if (roce3_hca_is_present(rdev) != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(CQ_CACHE_INVLD), cqn(0x%x), func_id(%hu)\n", __func__,
				cq->cqn, rdev->glb_func_id);
...
int roce3_destroy_cq(struct ib_cq *ibcq, struct ib_udata *udata)
{
	...
	if (roce3_hca_is_present(rdev) == 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA not present(return ok), func_id(%hu)\n", __func__, rdev->glb_func_id);
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to modify CQC, ret(%d), func_id(%hu), cqn(0x%x)\n",
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: Failed to modify cqc, ret(%d), func_id(%hu)\n", __func__, ret,
			rdev->glb_func_id);
		return ret;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: check cqc data state fail, func_id(%hu)\n", __func__, rdev->glb_func_id);
		return ret;
	}
