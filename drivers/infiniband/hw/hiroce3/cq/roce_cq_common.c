 ...
 * File Name	 : roce_cq_common.c
 * Version	   : v2.0
 * Created	   : 2021/12/7
 * Last Modified : 2022/3/24
 * Description   : The definition of RoCE CQ kernel functions.
 */
...
{
	struct roce_cqe *cqe = (struct roce_cqe *)roce3_get_cqe(cq, (n & ((unsigned int)cq->ibcq.cqe)));
	...
	if ((ROCE_LIKELY(tmp_cqe.dw1.bs.op_type != ROCE_OPCODE_CQE_UNUSED)) && ((tmp_cqe.dw0.bs.owner ^ tmp_val) != 0)) {
		return cqe;
	}
	...
	struct ib_umem **umem, u64 buf_addr, int cqe)
{
	...
	*umem = ib_umem_get(&rdev->ib_dev, buf_addr, (unsigned long)(cqe * cqe_size), IB_ACCESS_LOCAL_WRITE);
	if (IS_ERR(*umem)) {
