 ...
 * Description   : Define standard kernel RoCE CQ common header.
 */
...
Transport Interface and Software Transport Verbs chapters of the IB specifica-
tion.
*/
...
struct roce3_cq_buf {
	...
	struct rdma_mtt mtt;	 /* the mtt struct used by kernel mode and user mode to discribe buf */
...
struct roce3_cq {
	...
	struct roce3_db db;  /* The address information of the software DB that stores the CI user mode/kernel mode */
	...
	struct ib_umem *resize_umem; /* record the infomation mapped by buf which the User-mode resized */
	...
	void (*reset_flow_comp)(struct roce3_cq *);
};
...
static inline struct roce3_cq *cqmobj_to_roce3_cq(const cqm_object_s *object)
{
	cqm_queue_s *cqm_cq;
	cqm_cq = container_of(object, cqm_queue_s, object);
