 ...
 * File Name	 : roce_cq_create.c
 * Version	   : v2.0
 * Created	   : 2021/12/7
 * Last Modified : 2022/2/21
 * Description   : Define standard kernel RoCE CQ create realted functions.
 */
...
**************************************************************************** */
static void roce_cq_attr_assign(roce_verbs_cq_attr_s *cq_attr, struct roce3_cq *rcq, int eqn, int page_shift)
{
	/* Assign value to CQ */
	/* dw0 */
	cq_attr->dw0.bs.size = (u32)ROCE_ILOG2((unsigned)(rcq->ibcq.cqe + 1));
	cq_attr->dw0.bs.page_size = (u32)page_shift;
	cq_attr->dw0.bs.cqe_size =
		(u32)ROCE_ILOG2((unsigned)(rcq->buf.entry_size / 16)); /* entry_size=(2^cq_cqe_size)*16B. */
	...
	cq_attr->dw2.bs.cqecnt_lth = 6; /* update ci threshhold: 2^6=64 */
	...
	/* dw3 */
	cq_attr->dw3.bs.timeout = 0; /* The timeout mechanism is disabled by default when CQ is created */
	cq_attr->dw3.bs.max_cnt = 0; /* The overtime mechanism is disabled by default when CQ is created */
	...
	/* dw6 - dw7 */
	cq_attr->ci_record_gpa_at_hop_num = cpu_to_be64((rcq->db.dma & (~0x3uLL)) | rcq->buf.mtt.mtt_layers);

	return;
}

static int roce3_cq_fill_create_inbuf(struct roce3_device *rdev, struct roce3_cq *rcq, int vector, int page_shift,
	cqm_cmd_buf_s *cqm_cmd_inbuf)
{
	int eqn = 0;
	roce_verbs_cq_attr_s *cq_attr = NULL;
	roce_uni_cmd_creat_cq_s *cq_sw2hw_inbuf = NULL;

	cq_sw2hw_inbuf = (roce_uni_cmd_creat_cq_s *)cqm_cmd_inbuf->buf;
	cq_sw2hw_inbuf->com.index = cpu_to_be32(rcq->cqn);
	cq_sw2hw_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_CQ_BITMASK); //lint !e778
...
**************************************************************************** */
static int roce3_cq_sw2hw(struct roce3_device *rdev, struct roce3_cq *rcq, int vector, int page_shift)
{
	int ret = 0;
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_uni_cmd_creat_cq_s), NULL, 0);
	if (ret != 0) {
	...
	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_SW2HW_CQ, cqm_cmd_inbuf, NULL, NULL,
	...
	if (ret != 0) {
		...
		if (roce3_hca_is_present(rdev) != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(SW2HW_CQ), CQN(0x%x), func_id(%hu)\n", __func__,
...
**************************************************************************** */
static int roce3_create_cq_check(struct ib_device *ibdev, int entries, int vector, const struct ib_ucontext *ibcontext,
...
static int roce3_check_cq_create_flags(u32 flags)
{
	 ...
	 */
	return (int)(flags & ~(IB_UVERBS_CQ_FLAGS_IGNORE_OVERRUN | IB_UVERBS_CQ_FLAGS_TIMESTAMP_COMPLETION)); /*lint !e40*/
}

static int roce3_cq_cqc_cfg(struct roce3_device *rdev, struct roce3_cq *rcq, int vector, struct ib_udata *udata)
{
	int ret = 0;
	int page_shift = 0;
...
**************************************************************************** */
static int roce3_create_user_cq(struct roce3_device *rdev, struct roce3_cq *rcq, int entries, int vector,
	struct ib_ucontext *ibcontext, struct ib_udata *udata, u32 index)
{
	int ret = 0;
	struct roce3_create_cq_cmd ucmd = { 0 };

	rcq->cqm_cq = cqm_object_rdma_queue_create(rdev->hwdev, SERVICE_T_ROCE, CQM_OBJECT_RDMA_SCQ, 0, rcq, false, index);
	if (rcq->cqm_cq == NULL) {
	...
	ret = roce3_db_map_user(to_roce3_ucontext(ibcontext), (unsigned long)ucmd.db_addr, &rcq->db);
	if (ret != 0) {
...
**************************************************************************** */
static int roce3_create_kernel_cq(struct roce3_device *rdev, struct roce3_cq *rcq, int entries, int vector, u32 index)
{
	int ret = 0;
	int page_shift = 0;

	rcq->buf.buf_size = entries * (int)rdev->rdma_cap.cqe_size;
	rcq->cqm_cq = cqm_object_rdma_queue_create(rdev->hwdev, SERVICE_T_ROCE, CQM_OBJECT_RDMA_SCQ, (u32)rcq->buf.buf_size,
		rcq, true, index);
	if (rcq->cqm_cq == NULL) {
		dev_err(rdev->hwdev_hdl,
	...
	ret = hmm_rdma_mtt_alloc(rdev->hwdev, rcq->buf.buf->buf_number, (u32)page_shift, &rcq->buf.mtt, SERVICE_T_ROCE);
	if (ret != 0) {
	...
	struct ib_ucontext *ibcontext, struct ib_udata *udata, u32 index)
{
	...
	rcq = (struct roce3_cq *)kzalloc(sizeof(*rcq), GFP_KERNEL);
	if (rcq == NULL) {
		ret = -ENOMEM;
		pr_err("[ROCE, ERR] %s: Failed to alloc rcq\n", __func__);
		goto err_out;
	}
