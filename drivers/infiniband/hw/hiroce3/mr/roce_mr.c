...
static u32 convert_ib_access(int access_flag)
{
	...
	access = (u32)((((u_access_flag & IB_ACCESS_REMOTE_ATOMIC) != 0) ? RDMA_IB_ACCESS_REMOTE_ATOMIC : 0) |
		(((u_access_flag & IB_ACCESS_REMOTE_WRITE) != 0) ? RDMA_IB_ACCESS_REMOTE_WRITE : 0) |
		(((u_access_flag & IB_ACCESS_REMOTE_READ) != 0) ? RDMA_IB_ACCESS_REMOTE_READ : 0) |
		(((u_access_flag & IB_ACCESS_LOCAL_WRITE) != 0) ? RDMA_IB_ACCESS_LOCAL_WRITE : 0) |
		(((u_access_flag & IB_ACCESS_MW_BIND) != 0) ? RDMA_IB_ACCESS_MW_BIND : 0) |
		(((u_access_flag & IB_ZERO_BASED) != 0) ? RDMA_IB_ACCESS_ZERO_BASED : 0));
...
int roce3_alloc_tpt(struct roce3_device *rdev, struct rdma_mr *mr, u32 npages, u32 page_shift)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc mpt, ret(%d), func_id(%d)\n", __func__, ret,
			rdev->glb_func_id);
		return ret;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc mtt, ret(%d), func_id(%d)\n", __func__, ret,
			rdev->glb_func_id);
		goto err_alloc_mtt;
	}
...
void roce3_set_rdma_mr(struct rdma_mr *mr, enum rdma_mr_type mr_type, u32 pdn, u64 iova, u64 size, u32 access)
{
	...
	mr->size = size;
	mr->pdn = pdn;
	mr->access = convert_ib_access((int)access);	 /* Convert ib permissions to rdma component permissions */
	mr->key = get_key_from_index(mr->mpt.mpt_index); /* Convert from mpt index to key */
	mr->mr_type = mr_type;
}
...
struct ib_mr *roce3_get_dma_mr(struct ib_pd *ibpd, int access)
{
	...
	mr = (struct roce3_mr *)kzalloc(sizeof *mr, GFP_KERNEL);
	if (mr == NULL) {
		ret = -ENOMEM;
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc memory for dma mr, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		goto err_out;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc mpt and mtt, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		goto err_alloc_tpt;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to enable mpt of DMA mr, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		goto err_enable_mpt;
	}
...
static int roce3_alloc_priv_pages(struct ib_device *ibdev, struct roce3_mr *rmr, u32 max_pages)
{
	...
	rmr->page_map = dma_map_single(ibdev->dev.parent, rmr->pages, rmr->page_map_size, DMA_TO_DEVICE);
...
static void roce3_free_priv_pages(struct roce3_mr *rmr)
{
	...
	if (rmr->pages) {
		...
		dma_unmap_single(ib_dev->dev.parent, rmr->page_map, rmr->page_map_size, DMA_TO_DEVICE);
		free_page((unsigned long)(uintptr_t)(void *)rmr->pages);
		rmr->pages = NULL;
	}
}

static int roce3_alloc_mr_param_validate(const struct ib_pd *ibpd, enum ib_mr_type mr_type, u32 max_num_sg)
{
	int ret = 0;
...
static int roce3_alloc_mr_param_check(struct roce3_device *rdev, struct roce3_mr **mr, u32 max_num_sg)
{
	if (roce3_hca_is_present(rdev) == 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA not present(return fail), func_id(%hu)\n", __func__,
			rdev->glb_func_id);
	...
	*mr = (struct roce3_mr *)kzalloc(sizeof(struct roce3_mr), GFP_KERNEL);
	if ((*mr) == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc mr memory, func(%d)\n", __func__, rdev->glb_func_id);
		return -ENOMEM;
	}
...
struct ib_mr *roce3_alloc_mr(struct ib_pd *ibpd, enum ib_mr_type mr_type, u32 max_num_sg)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc tpt, func(%d)\n", __func__, rdev->glb_func_id);
		goto err_alloc_tpt;
	}

	ret = roce3_alloc_priv_pages(ibpd->device, mr, max_num_sg); // alloc memory to store pa
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: Failed to alloc memory, func(%u)\n", __func__, rdev->glb_func_id);
	...
	ret = hmm_rdma_enable_mr_mpt(rdev->hwdev, &(mr->rdmamr), HINIC3_CHANNEL_ROCE); // send to chip by cmdq
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to enable mpt, func(%d)\n", __func__, rdev->glb_func_id);
		goto err_enale_mpt;
	}
...
int roce3_map_kernel_frmr_sg(struct ib_mr *ibmr, struct scatterlist *sg, int sg_nents, unsigned int *sg_offset)
{
	int ret;
	struct roce3_mr *rmr = NULL;
	...
	ib_dma_sync_single_for_device(ibmr->device, rmr->page_map, rmr->page_map_size, DMA_TO_DEVICE);
...
static int roce3_umem_write_mtt_update(struct roce3_device *rdev, struct rdma_mtt *mtt, struct ib_umem *umem,
	u64 *page_list)
{
	int ret = 0;
	int i = 0;
	...
	for_each_sgtable_dma_sg(&umem->sgt_append.sgt, sg, i) {
		/* Calculate the number of pages in a memory block, the page size is 1 << page_shift */
		...
		for (j = 0; j < pages_in_chunk; ++j) {
			...
			/* The size of the memory pointed to by page_list is one page, which can store up to PAGE_SIZE / sizeof(u64)
			   pas. After a page is full, mtt needs to be written, else continue */
			if (npages != PAGE_SIZE / sizeof(u64)) {
				continue;
			}

			ret = hmm_rdma_write_mtt(rdev->hwdev, mtt, start_index, npages, page_list, SERVICE_T_ROCE);
			if (ret) {
				dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to write mtt, func_id(%hu)\n", __func__,
					rdev->glb_func_id);
				return ret;
			}
			start_index += npages;
	...
	if (npages != 0) {
		ret = hmm_rdma_write_mtt(rdev->hwdev, mtt, start_index, npages, page_list, SERVICE_T_ROCE);
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to write mtt, ret(%d), start_index(%u), func_id(%hu)\n",
...
int roce3_umem_write_mtt(struct roce3_device *rdev, struct rdma_mtt *mtt, struct ib_umem *umem)
{
	...
	if (page_list == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc memory for page list, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -ENOMEM;
	}
...
int roce3_buf_write_mtt(struct roce3_device *rdev, struct rdma_mtt *mtt, struct tag_cqm_buf *buf)
{
	...
	page_list = (u64 *)kzalloc(buf->buf_number * sizeof *page_list, GFP_KERNEL);
	if (page_list == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc memory for page list, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return (-ENOMEM);
	...
	u64 length, int access)
{
	...
	if (ret != 0) {
		dev_err(hwdev->dev_hdl, "[ROCE, ERR] %s(%d): Failed to alloc mpt and mtt, func_id(%hu)\n", __func__, __LINE__,
	...
	if (ret != 0) {
		dev_err(hwdev->dev_hdl, "[ROCE, ERR] %s(%d): Failed to write mtt, func_id(%hu)\n", __func__, __LINE__,
			hinic3_global_func_id(hwdev));
		goto err_write_mtt;
	}
	...
	if (ret != 0) {
		dev_err(hwdev->dev_hdl, "[ROCE, ERR] %s(%d): Failed to enable mpt of user mr, func_id(%hu)\n",
...
**************************************************************************** */
struct ib_mr *roce3_reg_user_mr(struct ib_pd *ibpd, u64 start, u64 length, u64 virt_addr, int access,
	struct ib_udata *udata)
{
	int ret;
	struct roce3_mr *mr = NULL;
	...
	if (mr == NULL) {
		ret = -ENOMEM;
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc memory for roce mr, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		goto err_out;
	}
	...
	if (IS_ERR(mr->umem)) {
		...
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to get ib umem, func_id(%hu)\n", __func__, rdev->glb_func_id);
		goto err_get_umem;
	}
...
int roce3_dereg_mr(struct ib_mr *ibmr, struct ib_udata *udata)
{
	...
	ret = hmm_rdma_disable_mr_mpt(rdev->hwdev, &mr->rdmamr, SERVICE_T_ROCE, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to disable mpt of mr, ret(%d), func_id(%d)\n", __func__, ret,
			rdev->glb_func_id);
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Mr may has mw bind on it, mr_key(%#x), func_id(%d)\n", __func__,
			mr->rdmamr.key, rdev->glb_func_id);
...
int roce3_alloc_mw(struct ib_mw *ibmw, struct ib_udata *udata)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc mpt, func_id(%hu)\n", __func__, rdev->glb_func_id);
		goto err;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to enable mpt of mw, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		if (ret == (-RDMA_CMDQ_TIMEOUT)) {
			rdev->dev_status_to_ofed = ROCE_DEV_STATUS_CMDQ_TIMEOUT;
		}
...
int roce3_dealloc_mw(struct ib_mw *ibmw)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to disable mpt of mw, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		if (ret == (-RDMA_CMDQ_TIMEOUT)) {
			rdev->dev_status_to_ofed = ROCE_DEV_STATUS_CMDQ_TIMEOUT;
		}
