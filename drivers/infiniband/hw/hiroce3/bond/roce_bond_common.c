...
static bool roce3_can_do_bond(struct bonding *bond)
{
	...
	bond_for_each_slave_rcu(bond, slave, iter) {
		...
		slave_cnt++;
		pr_info("%s:can do bond? slave_cnt(%d), slave_name(%s)", __func__, slave_cnt, slave->dev->name);
	}
...
struct net_device *roce3_bond_get_netdev(struct roce3_device *rdev)
{
	...
	for (i = 0; i < bond_dev->slave_cnt; i++) {
		...
		if (bond_is_active_slave(slave)) {
			if (netif_running(bond_dev->slaves[i].netdev) && netif_carrier_ok(bond_dev->slaves[i].netdev)) {
				ret_dev = bond_dev->slaves[i].netdev;
			} else if (netif_running(bond_dev->slaves[(i + 1) % bond_dev->slave_cnt].netdev) &&
				netif_carrier_ok(bond_dev->slaves[(i + 1) % bond_dev->slave_cnt].netdev)) {
...
int roce3_add_bond_real_slave_mac(struct roce3_device *rdev, u8 *mac)
{
	...
	for (i = 0; i < bond_dev->slave_cnt; i++) {
		...
		ret = roce3_add_mac_tbl_mac_entry(rdev->hwdev, mac, ROCE_BOND_RSVD_VLAN_ID, rdev->glb_func_id, slave->func_id);
		if (ret != 0) {
			mutex_unlock(&bond_dev->slave_lock);
		...
		if (slave->func_id != rdev->glb_func_id) {
			...
			(void)roce3_add_ipsu_tbl_mac_entry(rdev->hwdev, mac, 0, rdev->glb_func_id, slave->er_id);
		}
	}
	mutex_unlock(&bond_dev->slave_lock);
...
int roce3_add_bond_vlan_slave_mac(struct roce3_device *rdev, u8 *mac, u16 vlan_id)
{
	...
	for (i = 0; i < bond_dev->slave_cnt; i++) {
		...
		ret = roce3_add_mac_tbl_mac_entry(rdev->hwdev, mac, vlan_id, slave->func_id, slave->func_id);
		if (ret != 0) {
			mutex_unlock(&bond_dev->slave_lock);
			pr_err("[ROCE, ERR] %s: Failed to add mac_vlan entry, ret(%d)\n", __func__, ret);
			return ret;
		...
		/* The IPSU MAC table is used for fast forwarding. Even if the addition fails, the forwarding information
		   can still be obtained by checking the MAC table later, without judging the execution result. */
		(void)roce3_add_ipsu_tbl_mac_entry(rdev->hwdev, mac, vlan_id, rdev->glb_func_id, slave->er_id);
	}
	mutex_unlock(&bond_dev->slave_lock);
...
void roce3_del_bond_real_slave_mac(struct roce3_device *rdev)
{
	...
	for (i = 0; i < bond_dev->slave_cnt; i++) {
		slave = &bond_dev->slaves[i];
		if (slave->func_id != rdev->glb_func_id) {
			(void)roce3_del_ipsu_tbl_mac_entry(rdev->hwdev, rdev->mac, 0, rdev->glb_func_id, slave->er_id);
		}

		(void)roce3_del_mac_tbl_mac_entry(rdev->hwdev, rdev->mac, ROCE_BOND_RSVD_VLAN_ID, rdev->glb_func_id,
			slave->func_id);
	}
	mutex_unlock(&bond_dev->slave_lock);
...
void roce3_del_bond_vlan_slave_mac(struct roce3_device *rdev, u8 *mac, u16 vlan_id)
{
	...
	for (i = 0; i < bond_dev->slave_cnt; i++) {
		...
		roce3_del_ipsu_tbl_mac_entry(rdev->hwdev, mac, vlan_id, slave->func_id, slave->er_id);

		(void)roce3_del_mac_tbl_mac_entry(rdev->hwdev, mac, vlan_id, rdev->glb_func_id, slave->er_id);
	}
	mutex_unlock(&bond_dev->slave_lock);
}
...
void roce3_deatch_bond(u16 bond_id)
{
	...
	if (!detach_work) {
		pr_err("[ROCE, ERR] %s: malloc failed, bond id(%d)\n", __func__, bond_id);
		return;
	}
...
void roce3_before_bond_active(const char *bond_name, struct bond_attr *attr)
{
	...
	if (!bond_dev) {
		pr_err("[ROCE, ERR] %s: Malloc memory failed\n", __func__);
		goto err;
	}

	(void)strncpy(bond_dev->name, bond_name, strlen(bond_name));
...
void roce3_after_bond_modify(const char *bond_name, struct bond_attr *attr)
{
	...
	for (i = 0; i < BOND_PORT_MAX_NUM; i++) {
		for (j = 0; j < bond_dev->slave_cnt; j++) {
			...
			bond_dev->slaves[j].netdev_state.tx_enabled = tracker.netdev_state[i].tx_enabled;
			break;
		}
	}
	mutex_unlock(&bond_dev->slave_lock);
...
void roce3_queue_bond_work(struct net_device *upper_netdev, work_func_t func)
{
	...
	(void)strncpy(work->name, upper_netdev->name, strlen(upper_netdev->name));
...
int roce3_bond_init(void)
{
	...
	for_each_netdev(&init_net, upper_netdev) {
		if (netif_is_bond_master(upper_netdev) && roce3_can_do_bond(netdev_priv(upper_netdev))) {
			roce3_queue_bond_work(upper_netdev, roce3_attach_bond_work);
		}
	}
	rtnl_unlock();
	...
	while (!list_empty(&g_roce3_bond_list)) {
		...
		for (i = 0; i < bond_dev->slave_cnt; i++) {
			pr_info("[ROCE, INFO] %s: EXIT dev_put: bond_name(%s),slave_cnt(%d), slave_name(%s)\n",
				__func__, bond_dev->name, bond_dev->slave_cnt, bond_dev->slaves[i].netdev->name);
			dev_put(bond_dev->slaves[i].netdev);
		}
		hinic3_bond_detach(bond_dev->attr.bond_id, HINIC3_BOND_USER_ROCE);
		kfree(bond_dev);
