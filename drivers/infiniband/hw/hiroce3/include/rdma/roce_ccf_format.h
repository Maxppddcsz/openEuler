...
typedef struct ucode_dcc_ip_table_rate_ctx {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
} ucode_ip_table_rate_ctx_s;

typedef struct ucode_dcc_bw_ctrl_ctx {
	union {
		struct {
...
typedef struct ucode_ext_table_dcc_ctx {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
} ucode_ext_table_dcc_ctx_s;

typedef struct smf_rdma_dcc_ip_table {
	ucode_ip_table_rate_ctx_s algo_para; // dw0-dw3
} smf_rdma_dcc_ip_table_s;
...
typedef struct tag_iqpcn_hash_entry {
	u32 dw0;
	union {
		struct {
			u32 rsvd_key[7];
...
typedef struct tag_ipqcn_hash_value {
	union {
		struct {
			u32 code : 2;
			u32 subcode : 2;
