 ...
 * Description: RDMA offloaded DIF context.
 * Create: 2021-12-30
 */
...
typedef struct tag_roce_dif_cqe_s {
	// roce_cqe_s roce_cqe;
...
typedef struct tag_roce_dif_com_s {
...
typedef union tag_roce_dif_misc_rx_s {
	struct {
		u32 rsvd : 18;
		u32 fst_bt_tp : 1;
		u32 fst_sector : 1;
...
typedef struct roce_wqe_reg_mr_dsgl_seg {
	struct roce_wqe_reg_sig_mr_seg reg_mr_seg;
	struct roce3_sge_info data_sge;
	struct roce3_sge_info prot_sge;
} roce_wqe_reg_mr_dsgl_seg_s;

typedef struct roce_dif_ssgl_mpt_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
	...
	roce_dif_user_data_s dif_data;
} roce_dif_ssgl_mpt_context_s;

typedef struct roce_dif_dsgl_mpt_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
