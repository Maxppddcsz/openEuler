 ...
 * Description: RDMA upper level macros.
 * Create: 2021-12-30
 */
...
#define ROCE_NOFAA_BMPIDX2QPN(bmp_idx) ((bmp_idx) + ROCE_NOFAA_QPN_RSVD - ROCE_NOFAA_MASTER_BMPIDX_BASE)
...
#define ROCE_NOFAA_VFID2HOSTIDX(vf_id) (((vf_id) >> ROCE_NOFAA_PORT_NUM_SHIFT) & ROCE_NOFAA_HOSTID_MASK)
#define ROCE_NOFAA_VFID2PORTID(vf_id) ((vf_id) & ROCE_NOFAA_PORT_NUM_MASK)
#define ROCE_NOFAA_SLAVE_ALIVE_GET(active_bmp, slave_id) ((active_bmp) & (1u << ((slave_id) & ROCE_NOFAA_HOSTID_MASK)))
#define ROCE_AA_SRCCHNL2HOST(src_chnl) (((src_chnl) >> 3) & 0x3)
#define ROCE_NOFAA_GET_GID_IDX(qpc, host_id) (*((u8 *)&((qpc)->nofaa_ctx.dw1.value) + (host_id)) & 0x3f)
...
#define ROCE_GET_CMINFO_ADDR(sp_addr, index) \
		((roce_nof_cm_ctx_s *)(sp_addr) + (((index) >> ROCE_LT_XID_LB_SHIFT) & ROCE_CMINFO_PER_16B_MASK))

#define ROCE_NOFAA_QUERY_BMP_SIZE ((ROCE_NOFAA_PORT_QP_NUM) >> 3)
#define ROCE_NOFAA_GET_PORT_QPNUM_BYTE(port_id) ((u32)((port_id) << ROCE_NOFAA_PORT_QP_NUM_SHIFT) >> 3)
...
/* data struct */
typedef struct tag_roce_nof_cm_ctx {
	u32 local_comm_id;
	u32 remote_comm_id;
} roce_nof_cm_ctx_s;

typedef struct tag_nofaa_shard_hash_key {
	u32 xid : 16;
	u32 lun_id : 16;
} nofaa_shard_hash_key;

typedef struct tag_nofaa_shard_hash_item {
	struct {
		u32 volume_id : 24;
		u32 dd_id : 8;
	} bs;
	...
	u8 hash_low_offset;
	u8 hash_high_offset;
} nofaa_shard_hash_item;

typedef struct tag_nofaa_shard_hash_entry {
	nofaa_shard_hash_key key;
	nofaa_shard_hash_item item;
} nofaa_shard_hash_entry;

typedef struct tag_nofaa_shard_hash_value {
	union {
		struct {
			u32 code : 2;
			u32 rsvd0 : 30;
	...
	u32 rsvd0;
	nofaa_shard_hash_item item;
} nofaa_shard_hash_value_s;

typedef struct tag_nofaa_shard_lt_value {
	union {
		struct {
			u32 v : 1;
			u32 s : 1;
