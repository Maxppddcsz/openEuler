 ...
 * Description: RDMA common context.
 * Create: 2021-12-30
 */
...
#define ROCE_GET_HOST_OQID(host_id, xid) \
	((((128U * (host_id) + ((xid) & 0x7f)) << 2) & 0x7ff) | (((host_id) & 0x7) << 11U) | (((xid) & 0x1) + 1))
...
struct chip_seg_sqc {
	...
	union {
		struct {
			...
			u32 sq_size : 5;
#else
			u32 sq_size : 5; /* Send WQ size is (2^sq_size)*WQEBBs; the maximum SQ size is 16K WQEs, so this field
							  * doesn't exceed 14. They have same configuration value: SQC.sq_size and SQAC.sq_size.
							  * Configured by Driver */

			u32 sq_page_size : 4;  /* Page size of SQ and RQ, equals to (2^sq_page_size)*4KB.
									* The following three Page size have same configuration value: SQC.sq_page_size,
									* SQAC.sq_page_size and RQC.rq_page_size. Configured by Driver */
			u32 sq_wqebb_size : 3; /* Send WQE Basic Block (WQEBB) size in bytes is 16*(2^sq_weqbb_size).
									* for the SQ, this field should be fixed to 2.
									* They have same configuration value: SQC.sq_wqebb_size and SQAC.sq_wqebb_size.
									* Configured by Driver */
			u32 sq_pi_on_chip : 1; /* If set, the counter (PI index) of Send Queue is stored in the chip, the counter is
									* absolute value. Configured by Driver */
			u32 sq_inline_en : 1;  /* Indicates if the SQ supports inline data operation.
									* Configured by Driver */
			u32 sq_pd : 18;		/* Protection Domain.
									* The following five Protection Domains have same configuration value: QCC.rc_pd,
									* SQC.sq_pd, SQAC.sq_pd, RQC.rq_pd and RRWC.rrw_pd. Configured by Driver */
#endif
		} bs;
		u32 value;
	} dw0;
...
#else
			u32 rsvd : 16;
			u32 sq_wqecnt_lth : 4;	 /* WQE Counter Low Threshold, equals to (2^sq_wqecnt_lth).
										* Configured by Driver */
			u32 sq_wqecnt_rctl_en : 1; /* If this field is equal to zero, the SQ PI updating should be not controlled by
										* the "sq_wqecnt_rctl" field; else the SQ RI updating can be performed only if
										* the "sq_wqecnt_rctl" is equal to one. Configured by Driver */
			u32 sq_pi_vld : 1;		 /* If set, the SQ should use the Index Mechanism, else the SQ uses the Owner Bit
										* Mechanism. Configured by Driver */
			u32 sq_prewqe_mode : 1;	/* If set,the engine just performs prefetch WQE
										* processing after completing the current WQE in
										* the SQ_DMA_GEN_API flow.
										*/
			u32 sq_wqecnt_rctl : 1; /* The hardware clear it to zero when performing a SQ PI updating, and driver set it
									 * to one to indicate the hardware can performing SQ PI updating.
									 * N/A */
			u32 sq_dma_attr_idx : 6; /* bit[05:00] It specifies the outbound PCIe TLP header attribute of the DMA
									  * operation. This filed is only valid when processing SQ's WQEs. The following two
									  * "dma_attr_idx" have same configuration value: SQC.sq_dma_attr_idx and
									  * SQAC.sq_dma_attr_idx. Configured by Driver */
			u32 sq_so_ro : 2; /* It specifies the ATTR[1:0] bits in the outbound PCIe TLP headers of the DMA operation.
							   ...
							   * The following two "so_ro" have same configuration value: SQC.sq_so_ro and
	...
	union {
		struct {
			...
			u32 sq_pi : 16;
#else
			u32 sq_pi : 16; /* The Producer Index (PI) of Send Queue (Step: WQEBB) 0x0 */
			u32 rsvd1 : 6;
			u32 sq_wqe_check_en : 1;
			u32 sq_rkey_en : 1;
			u32 rsvd2 : 4;
			u32 sq_state : 4; /* Send Queue State.0x0:ok;0x1:error;0xf:hardware has no access right.Other:reserved.
							   * 0x0 */
#endif
		} bs;
		u32 value;
...
#else
			u32 sq_wqe_curt_page_gpa_h : 8; /* bit[63:56] Indicates the GPA of current SQ buffer's page pointed by
											   "sq_ci". 0x0 */
			u32 sq_wqe_next_page_vld : 1;   /* Indicates if the "sq_wqe_next_page_gpa" field is valid. */
			u32 sq_wqe_curt_page_vld : 1;   /* Indicates if the "sq_wqe_curt_page_gpa" field is valid.
											 * 1: it is valid;0: it is invalid.
											 * 0x0 */
			u32 sq_load_page_gpa : 1;	   /* Indicates the thread is performing a prefetch for GPA of WQE page. 0x0 */
			u32 sq_load_pi : 1;			 /* Indicates the thread is performing a PI prefetch processing */
			u32 dsgl_en : 1;
			u32 rsvd : 3;
			u32 sq_ci : 16; /* bit[19:00] The CI index of Send Queue (Step: WQEBB) 0x0 */
#endif
		} bs;
		u32 value;
	} dw3;
	...
	/* DW6 */
	u32 sq_wqe_next_page_gpa_l; /* bit[43:12] Indicates the page GPA of next SQ buffer's page pointed by "sq_ci". 0x0 */
	...
	union {
		struct {
			...
			u32 sq_signature : 3;
			u32 rsvd : 4;
			u32 sq_wqe_prefetch_ci : 16; /* bit[15:00] The prefetch CI index of Send Queue (Step: WQEBB).0x0 */
#endif
		} bs;
		u32 value;
	} dw7;
...
#else
			u32 sq_curt_sge_lkey : 28; /* bit[27:00] Indicates the L_Key of current SGE. Using sq_curt_sge_lkey[27:8] to
										  access the corresponding MPT. 0x0 */
			u32 rsvd : 3;
			u32 sq_curt_sge_vld : 1; /* Indicates current SGE information is valid. The remaining sq_curt_sge_* field are
									  * only valid when the sq_curt_sge_vld is asserted.1: SGE is valid;,0: SGE is
									  * invalid. 0x0 */
#endif
		} bs;
		u32 value;
	...
	/* DW11 */
	u32 sq_curt_sge_remain_len; /* bit[31:00] Indicates the remaining memory space of current SGE. 0x0 */

	/* DW12 */
	u32 sq_curt_sge_dbuff_gpa_h; /* bit[63:32] Indicates the GPA of current data buffer page pointed by
									"sq_curt_sge_va".0x0 */
...
#else
			u32 rsvd : 8;
			u32 sq_curt_sge_last : 1; /* Indicates the current SGE is the last SGE for the current WQE. 0x0 */
			u32 sq_curt_sge_used : 1;
			u32 sq_curt_sge_dsgl : 1;
			u32 sq_curt_sge_dbuff_gpa_vld : 1; /* Indicates if the "sq_curt_sge_dbuff_gpa" field is valid. 1: it is
												  valid;0:it is invalid. */
			u32 sq_curt_sge_dbuff_gpa_l : 20;  /* bit[31:12] Indicates the GPA of current data buffer page pointed by
												  "sq_curt_sge_va".0x0 */
#endif
		} bs;
		u32 value;
...
#else
			u32 sq_mtt_prefetch_wqe_ci : 16; /* bit[15:0] Indicates the WQE index of prefetch MTTs operation */
			u32 rsvd : 4;
			u32 sq_wqe_prefetch_finish : 1;
			u32 sq_mtt_prefetch_cline_ptr : 6;
			u32 sq_mtt_prefetch_cline_num : 5;
	...
	union {
		struct {
			...
			u32 sq_mtt_prefetch_sge_idx : 6;
			u32 rsvd : 8;
			u32 sq_curt_sge_idx : 6; /* The SGE index of current accessed WQE. 0x0 */
			u32 sq_prefetch_one_wqe : 1;
			u32 sq_wqe_prefetch_mode : 1;
			u32 sq_mtt_prefetch_maxlen : 3;
			u32 sq_wqe_cache_thd_sel : 2;
...
struct chip_seg_sqac {
	...
	union {
		struct {
			...
			u32 sqa_cqn : 20;
#else
			u32 sqa_cqn : 20; /* bit[19:00] SQ's Completion Queue Number. */
			u32 rsvd1 : 1;
			u32 vbs_sqa_pi_vld : 1;
			u32 sqa_prewqe_mode : 1;
			u32 rsvd2 : 1;
...
typedef struct chip_seg_rqc {
	...
	union {
		struct {
			...
			u32 rq_size : 5;
#else
			u32 rq_size : 5;	   /* Receive Queue size, equals to (2^rq_size)*WQEBB. The maximum RQ size is 16K WQEs,
									* so this field doesn't exceed 14.
									* Configured by Driver */
			u32 rq_page_size : 4;  /* Page size of SQ and RQ, equals to (2^rq_page_size)*4KB.
									* Configured by Driver */
			u32 rq_wqebb_size : 3; /* Receive WQE Basic Block (WQEBB) size in bytes is (2^rq_wqebb_size)*16B.
									* The minimum size is 16B and the values 4, 5, 6, 7 are reserved.
									* Configured by Driver */
			u32 rq_pi_on_chip : 1; /* If set, the counter (PI index) of Receive Queue is stored in the chip, the counter
									* is absolute value. Configured by Driver */
			u32 rq_inline_en : 1;  /* If set, in-line scatter is enabled for this RQ.
									* Configured by Driver */
			u32 rq_pd : 18; /* bit[17:00] Protection Domain.The following five Protection Domains have same configuration
							 * value: QCC.rc_pd, SQC.sq_pd, SQAC.sq_pd, RQC.rq_pd and RRWC.rrw_pd. Configured by
							 * Driver */
#endif
		} bs;
		u32 value;
	...
	union {
		struct {
			...
			u32 rq_cqn : 20;
#else
			u32 rq_cqn : 20; /* bit[19:00] Receive Queue completions are to be reported to this CQ number.
							  * Configured by Driver */
			u32 rq_signature : 3;
			u32 rq_rkey_en : 1;
			u32 rq_dma_attr_idx : 6; /* bit[05:00] It specifies the outbound PCIe TLP header attribute of the DMA
									  * operation. This filed is only valid when processing RQ's WQEs. Configured by
									  * Driver */
			u32 rq_so_ro : 2; /* It specifies the ATTR[1:0] bits in the outbound PCIe TLP headers of the DMA operation.
	...
	union {
		struct {
			...
			u32 rq_srq_en : 1;
#else
			u32 rq_srq_en : 1;		 /* If set, indicates it should use SRQ as the receive queue.
										* Configured by Driver */
			u32 rq_wqecnt_lth : 4;	 /* WQE Counter Low Threshold, equals to (2^rq_wqecnt_lth).
										* Configured by Driver */
			u32 rq_wqecnt_rctl_en : 1; /* If this field is equal to zero, the RQ PI updating should be not controlled by
										* the "rq_wqecnt_rctl" field; else the RQ RI updating can be performed only if
										* the "rq_wqecnt_rctl" is equal to one. Configured by Driver */
			u32 rsvd1 : 2;

			u32 rq_wqecnt_rctl : 1; /* The hardware clear it to zero when performing a RQ PI updating, and driver set it
									 * to one to indicate the hardware can performing RQ PI updating. N/A */
			u32 rq_inlin_len : 4;   /* Inline Data Length, equals to (2^rq_inline_len) bytes.
									 * Configured by Driver */
			u32 rq_wqe_cache_thd_sel : 2;
			u32 rsvd2 : 13;
			u32 rq_state : 4; /* Receive Queue State.0xf: ok;0x1: error;0x0: hardware has no access right.Other:
							   * reserved. 0x0 */
#endif
		} bs;
		struct {
...
#else
			u32 rq_wqe_curt_page_gpa_h : 8; /* bit[63:56] Indicates the GPA of current RQ buffer's page pointed by
											 * "rq_ci". 0x0 */
			u32 rq_wqe_next_page_vld : 1;   /* Indicates if the "rq_wqe_curt_page_gpa" field is valid.
											 * 1: it is valid;0: it is invalid.
											 * 0x0 */
			u32 rq_wqe_curt_page_vld : 1;   /* Indicates if the "rq_wqe_curt_page_gpa" field is valid.
											 * 1: it is valid;0: it is invalid.
											 * 0x0 */
			u32 rq_load_page_gpa : 1;	   /* Indicates the thread is performing a prefetch for GPA of WQE page.
											 * 0x0 */
			u32 rq_load_pi : 1;
			u32 rsvd : 4;
			u32 rq_ci : 16; /* bit[15:00] The CI index of Receive Queue (Step: WQEBB) 0x0 */
#endif
		} bs;
		u32 value;
	} dw3;
...
#else
			u32 rq_wqe_prefetch_ci : 16; /* bit[15:00] The prefetch CI index of Receive Queue (Step: WQEBB).0x0 */
			u32 rq_pi : 16;			  /* bit[15:00] The Producer Index (PI) of Receive Queue (Step: WQEBB).0x0 */
#endif
		} bs;
		u32 value;
	} dw7;
...
#else
			u32 rq_curt_sge_lkey : 28; /* bit[27:00] Indicates the L_Key of current SGE. Using rq_curt_sge_lkey[27:8] to
										  access the corresponding MPT.0x0 */
			u32 rsvd : 3;
			u32 rq_curt_sge_vld : 1; /* Indicates current SGE information is valid. The remaining rq_curt_sge_* field are
									  * only valid when the rq_curt_sge_vld is asserted.1: SGE is valid;0: SGE is
									  * invalid.0x0 */
#endif
		} bs;
		u32 value;
	...
	/* DW11 */
	u32 rq_curt_sge_remain_len; /* bit[31:00] Indicates the remaining memory space of current SGE.0x0 */

	/* DW12 */
	u32 rq_curt_sge_dbuff_gpa_h; /* bit[63:32] Indicates the GPA of current data buffer page pointed by
									"rq_curt_sge_va".0x0 */
	...
	union {
		struct {
			...
			u32 rq_curt_sge_dbuff_gpa_vld : 1; /* Indicates if the "rq_curt_sge_dbuff_gpa" field is valid.1: it is
												  valid;0: it is invalid.0x0 */
			u32 rq_curt_sge_dsgl : 1;
			u32 rq_curt_sge_used : 1;
			u32 rq_curt_sge_last : 1;   /* Indicates the current SGE is the last SGE for the current WQE.0x0 */
			u32 rq_curt_wqe_status : 1; /* Indicates the current Receive WQE's status.0: the Receive WQE don't be
	...
	union {
		struct {
			...
			u32 rq_load_next_mtt : 1;
			u32 rsvd : 11;
			u32 rq_curt_sge_idx : 4; /* The SGE index of current accessed WQE.0x0 */
			u32 rq_mtt_prefetch_maxlen1 : 2;
			u32 rq_mtt_prefetch_maxlen0 : 2;
			u32 rq_mtt_prefetch_maxwqe : 3;
			u32 rq_wqe_prefetch_minnum : 3;
...
struct chip_seg_srqc {
	...
	union {
		struct {
			...
			u32 srq_size : 5;
			u32 srq_page_size : 4;
			u32 srq_wqebb_size : 3; /* Shared Receive WQE Basic Block (WQEBB) size in bytes is
									 ...
									 * 0x0 */
			u32 rsvd : 2;
			u32 srq_pd : 18; /* bit[17:00]Protection Domain. If the QP is a XRC transport service type,
							  * this filed should be updated by the "srq_pd" in the SRQ context
							  * pointed by XRCSRQ; else this field is updated by the "srq_pd" in the
							  * SRQ context associated with the QP.
							  * The following five Protection Domains have same configuration value:
	...
	union {
		struct {
			...
			u32 srq_cqn : 20;
#else
			u32 srq_cqn : 20; /* bit[19:00] Receive Queue completions are to be reported to this CQ number.
							   * If the QP is a XRC transport service type, this filed should be updated
							   * by the "srq_xrc_cqn" in the SRQ context pointed by XRCSRQ; else this
							   * field is configured by the driver.
							   * Configured by Driver */
			u32 srq_signature : 3;
			u32 srq_rkey_en : 1;
			u32 srq_dma_attr_idx : 6; /* bit[05:00] */
			u32 srq_so_ro : 2;		/* It specifies the ATTR[1:0] bits in the outbound PCIe TLP headers of the
	...
	union {
		struct {
			...
			u32 srq_en : 1;
#else
			u32 srq_en : 1; /* If set, indicates it should use SRQ as the receive queue.
							 * Configured by Driver */
			u32 rsvd1 : 5;
			u32 srqn : 18; /* SRQ number. If the QP is a XRC transport service type, this
							* filed should be updated by the XRCSRQ; else this field is configured by the driver.
							* Configured by Driver */
			u32 rsvd2 : 4;
			u32 srq_state : 4; /* Receive Queue State.
...
typedef struct chip_seg_qpcc {
	...
	union {
		struct {
			...
			u32 qp_rwe : 1;
			u32 qp_rkey_en : 1;
			u32 rc_size : 4;	   /* RDMARC table size, equals to (2^rc_size)*Entry Size.
									...
									*Others: reserved.
									*Configured by Driver */
			u32 rc_entry_size : 2; /* Entry size of RDMARC table in bytes is (2^rc_entry_size)*16B.
									* The minimum size is 32B and the maximum size is 64B; so the values 0 and 3 are
	...
	union {
		struct {
			...
			u32 rsvd : 5;
			u32 rc_entry_prefetch_maxnum : 3; /* Maximum number of prefetch Entries for RDMARC table.000: prefetch number
...
typedef struct chip_seg_rcc {
...
#else
			u32 rc_curt_sge_rkey : 28; /* bit[27:00] Indicates the R_Key of current SGE. Using rc_curt_sge_rkey[27:8] to
										  access the corresponding MPT.0x0 */
			u32 rsvd : 3;
			u32 rc_curt_sge_vld : 1; /* Indicates current SGE information is valid. The remaining rc_curt_sge_* field are
									  * only valid when the rc_curt_sge_vld is asserted.1: SGE is valid;0: SGE is
									  * invalid. */
#endif
		} bs;
		u32 value;
	...
	/* DW3 */
	u32 rc_curt_sge_remain_len; /* bit[31:00]Indicates the remaining memory space of current SGE.0x0 */
	...
	union {
		struct {
			...
			u32 rc_pi : 8;
#else
			u32 rc_pi : 8; /* bit[07:00] The Producer Index (PI) of RDMARC table (Step: Entry Size).0x0 */
			u32 dsgl_en : 1;
			u32 rc_curt_sge_used : 1;
			u32 rc_curt_sge_dsgl : 1;
			u32 rc_curt_sge_dbuff_gpa_vld : 1; /* Indicates if the "rc_curt_sge_dbuff_gpa" field is valid.1: it is
												  valid;0: it is invalid.0x0 */
			u32 rc_curt_sge_dbuff_gpa_l : 20;  /* bit[31:12] */
#endif
		} bs;
	...
	union {
		struct {
			...
			u32 rc_ci : 8;
#else
			u32 rc_ci : 8; /* bit[07:00] The Consumer Index (CI) of RDMARC table (Step: Entry Size). */
			u32 rsvd : 3;
			u32 rc_signature : 3;
			u32 rc_pd : 18; /* bit[17:00] Protection Domain. If the QP is a XRC transport service type, this filed should
							 * be updated by the "srq_pd" in the SRQ context pointed by XRCSRQ; else this field should
...
/* Queue Pair Context */
typedef struct qpc_chip_seg {
	struct chip_seg_rrwc rrwc; /* RQ RDMA Write Context (32B) */
	struct chip_seg_rcc rcc;   /* RDMARDC Context (32B) */

	struct chip_seg_sqc sqc;   /* Send Queue Context (64B) */
	struct chip_seg_sqac sqac; /* Send Queue ACK Context (64B) */
	union {
		struct chip_seg_rqc rqc;   /* Receive Queue Context (64B) */
		struct chip_seg_srqc srqc; /* RQC Shared Receive Queue Mode(64B) */
	};

	struct chip_seg_qpcc qpcc; /* Queue Pair Common Context (32B) */
} qpc_chip_seg_s;
...
struct qpc_timer_seg {
...
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			u32 rsvd3 : 1;
			u32 tss_timer_num : 3;   /* Indicate how many timers this timer section
									  supports:0:rsvd,X:support up to X timers */
			u32 tss_lock_flag : 1;   /* Lock flag indicates whether this timer section is locked by one API or not
									  1: locked 0: unlock */
			u32 tss_modify_flag : 1; /* Modify flag indicates whether this timer section has been modified by other API
										without lock or not. 1: has been modified,0: has not been modified */
			u32 rsvd2 : 1;
			u32 tss_close_flag : 1; /* close flag indicates this connection needs to close and this XID context will be
									 de-allocated by control plane; 0: not close,1: close,this timer section will be
									 picked off from timer wheel and issue a special time-out API to tell ucode this
									 context resource can be de-allocated. */
			u32 rsvd1 : 3;
			u32 rsvd0 : 2;
			u32 tlwi_lv : 1;		  /* Timer link valid,
							   1: this timer section is hung in timing wheel
							   0: this timer section is not hung in any timing wheel */
			u32 tlwi_timer_id : 3;	/* indicate which timer is hung in timing wheel */
			u32 tlwi_wheel_id : 3;	/* indicate which timing wheel this timer section is hung in */
			u32 tlwi_link_spoke : 12; /* indicate which fire spoke this timer section is hung in */
#else
	...
	/* DW1 */
	u32 pre_link_ptr0; /* Previous link pointer, data structure is referred to timer pointer data structure */
	...
	/* DW4 */
	u32 next_link_ptr1; /* Next link pointer, data structure is referred to timer pointer data structure */
...
/* * sq ack ctx */
typedef struct ucode_sq_ack_ctx {
	/* DW12 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
typedef struct ucode_sq_ctx {
...
#else
			u32 next_send_psn : 24; /* Initial value of the driver in BTH Packet Sequence Number(initial)RTR2RTS mode */
	...
	ucode_sq_ack_ctx_s ack_ctx;
} ucode_sq_ctx_s;

/* * rq ack ctx */
typedef struct ucode_rq_ack_ctx {
	/* DW24 */
	u32 rq_rsp_left;
...
typedef struct ucode_rq_ctx {
	...
	/* DW24~27 */
	ucode_rq_ack_ctx_s ack_ctx;
} ucode_rq_ctx_s;

typedef struct ucode_dcqcn_ctx {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
typedef struct ucode_common_ctx {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			...
			u32 ccf_appid : 3;
			u32 port : 2;
			u32 ulp_type : 4; /* for user plugin extension */
			u32 fake : 1;
			u32 cq_ovl_flag : 1; /* Cq overflow Indicates whether WQ CATAS is generated. */
			u32 sqd_event : 1;
			u32 vf_id : 13;
#else
			u32 vf_id : 13;
...
typedef struct drv_ucode_all_info {
	/* DW0~DW4 */
	ucode_common_ctx_s common;

	/* DW5~DW18 */
	ucode_sq_ctx_s sq_ctx;

	/* DW19~DW27 */
	ucode_rq_ctx_s rq_ctx;
} drv_ucode_all_info;

typedef struct drv_path_info {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
typedef struct qpc_drv_seg {
	...
	union {
		struct {
			...
			u32 state : 4;		/* QP state 0000:RST 0001:INIT 0010:RTR 0011:RTS 0100:SQEr 0101:SQD(Drained) 0110:ERR
									 0111:Draining This field is not modified for the *1XXX:Reserved driver verbs and is
	...
	union {
		struct {
			...
			u32 invalid_credit : 1; /* Indicates whether the RQ disables the credit. The SRQ is disabled by default and
									   must be set to 1. */
...
/* QPC Struct */
typedef struct qpc_sw_seg {
	/* driver seg, DW0 ~ DW3 */
	qpc_drv_seg_s drv_seg;

	/* path seg, DW4 ~ DW11 */
	drv_path_info_s path_seg;

	/* 112B(DW12~DW39) */
	drv_ucode_all_info ucode_seg;

	/* 32B(DW40~DW47) */
	ucode_ext_ctx_s ext_seg;
} qpc_sw_seg_s;

/* QPC Struct */
typedef struct roce_qp_context {
	struct qpc_chip_seg chip_seg;
	struct qpc_timer_seg timer_seg;
	struct qpc_sw_seg sw_seg;
} roce_qp_context_s;

typedef struct roce_aa_qp_context {
	struct qpc_chip_seg chip_seg;
	struct qpc_timer_seg timer_seg;
	struct qpc_sw_seg sw_seg;
	struct ucode_nofaa_ctx nofaa;
} roce_aa_qp_context_s;

/* *QPC Format end */

/* * SRQC Format start */
typedef struct roce_srq_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			...
			u32 lth_pre_en : 1;
			u32 pcnt_on_chip : 1; /* If set, the producer counter of Shared Receive Queue
	...
	union {
		struct {
			...
			u32 rsvd : 2;
			u32 xrc_cqn : 20; /* Completion Queue to report XRC messages directed to this SRQ (XRC only) */
#else
			u32 xrc_cqn : 20; /* Completion Queue to report XRC messages directed to this SRQ (XRC only) */
			u32 rsvd : 2;
			u32 srq_wqe_check_en : 1;
	...
	union {
		...
		struct {
			...
			u32 state : 4;		  /* SRQ status.
									...
									0xf: ok.
									Other: reserved. */
			u32 container_size : 2; /* 0:15 normal + 1 link wqe; 1:7 normal + 1 linkwqe  2:3normal + 1 linkwqe 3:1
									   normal + 1 linkwqe */
			u32 container_en : 1;
			u32 xrc_cqn1 : 3;
	...
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			u32 lth : 16;		/* Limit Threshold. If the "srq_lth" is not zero, and the "srq_wqe_cnt"
										 drops below the "srq_lth" when a WQE is de-queued from the SRQ, then an SRQ
										 limit event is fired and the "srq_lth" is set to zero. */
			u32 xrcd : 16;	   /* The Scalable RC Domain. The received message can be served only if the XRC
										  Domain of the transport QP of received message matches this field. */
#else
			u32 xrcd : 16; /* The Scalable RC Domain. The received message can be served only if the XRC
									Domain of the transport QP of received message matches this field. */
			u32 lth : 16;  /* Limit Threshold. If the "srq_lth" is not zero, and the "srq_wqe_cnt"
		...
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			u32 head_index : 16; /* Limit Threshold. If the "srq_lth" is not zero, and the "srq_wqe_cnt"
								  drops below the "srq_lth" when a WQE is de-queued from the SRQ, then an SRQ
								  limit event is fired and the "srq_lth" is set to zero. */
			u32 pcnt : 16;	   /* The Scalable RC Domain. The received message can be served only if the XRC
								  Domain of the transport QP of received message matches this field. */
#else
			u32 pcnt : 16;	   /* The Scalable RC Domain. The received message can be served only if the XRC
									Domain of the transport QP of received message matches this field. */
	...
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			u32 pcnt : 16;	   /* A 16-bit counter incremented for each WQE posted to the SRQ. */
	...
	union {
		...
		struct {
			...
			u32 rsvd : 1;
			u32 pcnt_on_chip : 1;
			u32 resd : 6; /* The current WQE index; uses this field to get the corresponding WQE from SRQ. */
#else
			u32 resd : 6;
			u32 pcnt_on_chip : 1;
			u32 rsvd : 1; /* The current WQE index; uses this field to get the corresponding WQE from SRQ. */
			u32 head_gpa : 24;
#endif
		} bs_osd;
	...
	/* DW8~9 */
	union {
		u64 l0mtt_gpa; /* The GPA of Layer 0 MTT. It may point to the SRQ's buffer directly.
	...
	/* DW10~11 */
	union {
		u64 pcnt_record_gpa_at_hop_num; /* bit[63:2]:The GPA of stored producer counters of Shared Receive Queue.
		...
		struct {
			...
			u32 pcnt_record_gpa_lo_at_hop_num; /* bit[1:0]: Address translation hop numbers. */
		} dw10;
...
typedef struct roce_ssgl_mpt_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
#else
			u32 task_id : 16; /* This parameter is valid when SIG MR. */
			u32 smd_type : 4; /* This is sector metadata type: 2b'0x is 8B;2b'10 is [8B+56B];2b'11 is [56B+8B] */
			u32 smd_size : 1; /* metadata size[0]:0 is 512, 1'b1 is 4096 */
			u32 tp2_fst_sct : 1;
			u32 ext_mtu : 1;
			u32 rsvd : 9;
	...
	union {
		struct {
			...
			u32 w_bg_type : 1;	   /* Indicates whether the line is CRC(0) or IP CheckSum(1). */
			...
			u32 rx_m_dif_type : 2; /* type 0 ~ 3,def:1 */
			u32 rx_w_dif_type : 2; /* type 0 ~ 3,def:1 */
			u32 w_dif_en : 1;	  /* Indicates whether dif is enabled in the wire domain. */
#endif
		} bs;
...
} roce_ssgl_mpt_context_s;

typedef struct roce_dsgl_mpt_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
	...
	union {
		struct {
			...
			u32 m_app_tag : 16;
#else
			u32 m_app_tag : 16; /* TX: Verify ref tag domain
								   RX: Insert&replace ref tag domain */

			u32 w_app_tag : 16; /* TX: Insert&replace ref tag domain
	...
	u32 m_ref_tag; /*  TX: Verify ref tag domain
					   RX: Insert&replace ref tag domain */

	u32 w_ref_tag; /* TX: Insert&replace ref tag domain
					  RX: Verify ref tag domain */
...
} roce_dsgl_mpt_context_s;

typedef struct iw_mpt_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
...
/* * CQC Format start */
typedef struct roce_cq_context {
	/* DW0 */
	union {
		struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
			...
			u32 ceqn_low : 5; /* Completion Event Queue, this CQ reports Completion Events to this CQE (cq_ceqn[04:00])
							   */
	...
	union {
		struct {
			...
			u32 ci : 24;
#else
			u32 ci : 24;		  /* The CI index of Receive Queue (Step: CQE). */
			u32 dma_attr_idx : 6; /* It specifies the outbound PCIe TLP header attribute of the DMA operation.
	...
	union {
		struct {
			...
			u32 pi : 24;
#else
			u32 pi : 24;	   /* The PI index of Completion Queue (Step: CQE) */
			u32 ceqn_high : 3; /* Completion Event Queue, this CQ reports Completion Events to this CQE (cq_ceqn[07:05])
								*/
			u32 rsvd : 1;
			u32 state : 4; /* CQ status.
								   0x0: hardware has no access right;
								   0x1: CQ is error;
								   0x2: CQ is overflow;
								   0xf: ok.
	...
	union {
		struct {
			...
			u32 cqe_next_pg_vld : 1;  /* Indicates if the "cq_cqe_next_page_gpa" field is valid.
											  1: it is valid;
											  0: it is invalid. */
			u32 cqe_cur_pg_vld : 1;   /* Indicates if the "cq_cqe_curt_page_gpa" field is valid.
										   1: it is valid;
										   0: it is invalid. */
			u32 load_page_gpa : 1;	/* Indicates the thread is performing a prefetch for GPA of CQE page. */
			u32 load_ci : 1;
	...
	/* DW4 */
	u32 cqe_curt_pg_gpa_m; /* Indicates the page GPA of current CQ buffer's page pointed by "cq_pi".
	...
	/* DW6 */
	u32 cqe_next_pg_gpa_l; /* Indicates the page GPA of next CQ buffer's page pointed by "cq_pi".
	...
	union {
		struct {
			...
			u32 cqe_cnt : 16;
			u32 rsvd : 12;
			u32 arm_state : 2; /* The CQ ARM State machine.
									  0x0: idle state;
									  0x1: solicited state;
									  0x2: next state;
									  Others: reserved. */
	...
	/* DW12~13 */
	union {
		u64 cqc_l0mtt_gpa; /* The GPA of Layer 0 MTT. It may point to the CQ's buffer directly.
...
/* * GID Format start */
typedef struct roce_gid_context {
	/* DW0~3 */
	u32 gid[4];
...
typedef struct roce_sqrq_db {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
	u32 rsvd : 16;
	u32 recieve_wqe_counter : 16;
#else
	u32 recieve_wqe_counter : 16;
	u32 rsvd : 16;
#endif
...
typedef union tag_roce_qu_db_dw4 {
	struct {
#if defined(BYTE_ORDER) && defined(BIG_ENDIAN) && ((BYTE_ORDER == BIG_ENDIAN))
		u32 sub_type : 4;
		u32 udf_3 : 12;
...
typedef struct tag_roce_cq_mtt_info {
...
typedef struct tag_timer_section {
	union {
		u32 value;
		struct {
