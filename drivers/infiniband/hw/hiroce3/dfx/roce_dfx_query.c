 ...
 *
 * File Name	 : roce_dfx_query.c
 * Version	   : v2.1
 * Created	   : 2020/8/29
 * Last Modified : 2021/10/16
 * Description   : The definition of callback functions and internal functions of RoCE driver dfx features on qp level.
 */
...
int roce3_dfx_cmd_query_qp(struct roce3_device *rdev, u32 qpn, struct roce_qp_context *qp_ctx)
{
	...
	roce_cmd_qp_query_s *qp_query_inbuf = NULL;
	struct roce3_qp_query_outbuf *qp_query_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_qp_query_s), &cqm_cmd_outbuf,
	...
	qp_query_inbuf = (roce_cmd_qp_query_s *)cqm_cmd_inbuf->buf;
	...
	if ((rdev->cfg_info.lb_en != 0) && (rdev->cfg_info.lb_mode == ROCE_LB_MODE_1)) {
		u8 cos = qpn & 0x3;
		roce3_dfx_print("lb_mode1 roce3_dfx_cmd_query_qp func_id(%d) qpn:%u cos:%u \n", rdev->glb_func_id, qpn, cos);
		ret = cqm_lb_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_QUERY_QP, cos, cqm_cmd_inbuf, cqm_cmd_outbuf,
			NULL, ROCE_CMD_TIME_CLASS_B, HINIC3_CHANNEL_ROCE);
	} else {
		roce3_dfx_print("Not lb_mode1 roce3_dfx_cmd_query_qp func_id(%d) qpn:%u\n", rdev->glb_func_id, qpn);
		ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_QUERY_QP, cqm_cmd_inbuf, cqm_cmd_outbuf, NULL,
...
static int roce3_dfx_get_dev_algo(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	u32 cc_algo;
...
static int roce3_dfx_get_qp_count(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
...
static int roce3_dfx_query_cache_qpc(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	int ret;
	u32 qpn = inbuf->qpn;
...
static int roce3_dfx_query_host_qpc(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
...
int roce3_dfx_cmd_query_cq(struct roce3_device *rdev, u32 cqn, struct roce_cq_context *cq_ctx)
{
	...
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	cqm_cmd_buf_s *cqm_cmd_outbuf = NULL;
	roce_cmd_cq_query_s *cq_query_inbuf = NULL;
	struct roce3_cq_query_outbuf *cq_query_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_cq_query_s), &cqm_cmd_outbuf,
	...
	cq_query_inbuf = (roce_cmd_cq_query_s *)cqm_cmd_inbuf->buf;
	cq_query_inbuf->com.index = cpu_to_be32(cqn);
	cq_query_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_CQ_BITMASK);

	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_QUERY_CQ, cqm_cmd_inbuf, cqm_cmd_outbuf, NULL,
		ROCE_CMD_TIME_CLASS_B, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
...
static int roce3_dfx_query_cache_cqc(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	int ret;
...
static int roce3_dfx_query_host_cqc(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	cqm_object_s *cqm_obj_cq = NULL;
...
int roce3_dfx_cmd_query_srq(struct roce3_device *rdev, u32 srqn, struct roce_srq_context *srq_ctx)
{
	...
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	cqm_cmd_buf_s *cqm_cmd_outbuf = NULL;
	roce_cmd_srq_query_s *srq_query_inbuf = NULL;
	struct roce3_srq_query_outbuf *srq_query_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_srq_query_s), &cqm_cmd_outbuf,
	...
	srq_query_inbuf = (roce_cmd_srq_query_s *)cqm_cmd_inbuf->buf;
	srq_query_inbuf->com.index = cpu_to_be32(srqn);
	srq_query_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_SRQ_BITMASK);

	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_QUERY_SRQ, cqm_cmd_inbuf, cqm_cmd_outbuf, NULL,
		ROCE_CMD_TIME_CLASS_B, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
...
static int roce3_dfx_query_cache_srqc(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	int ret;
	...
	if (cqm_obj_srq == NULL) {
		...
		roce3_dfx_print("******************From Cache: srqn(%#x) ********************", srqn);
		roce3_dfx_print(">>>>>>>>>>> QUERY CQC FROM CACHE FAILED, NOT EXIST <<<<<<<<<<<");
		roce3_dfx_print("******************From Cache: srqn(%#x) ********************", srqn);
...
static int roce3_dfx_query_host_srqc(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	cqm_object_s *cqm_obj_srq = NULL;
	...
	if (cqm_obj_srq == NULL) {
		roce3_dfx_print("Failed to get cqm_obj_srq.");
		roce3_dfx_print("******************From Host: srqn(%#x) ********************", srqn);
...
static int roce3_dfx_cmd_query_mpt(struct roce3_device *rdev, u32 key, struct roce_mpt_context *mpt_entry)
{
	int ret;
	u32 mpt_index;
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	cqm_cmd_buf_s *cqm_cmd_outbuf = NULL;
	roce_cmd_mpt_query_s *mpt_query_inbuf = NULL;
	struct roce3_mpt_query_outbuf *mpt_query_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_mpt_query_s), &cqm_cmd_outbuf,
	...
	mpt_query_inbuf = (roce_cmd_mpt_query_s *)cqm_cmd_inbuf->buf;
	...
	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_QUERY_MPT, cqm_cmd_inbuf, cqm_cmd_outbuf, NULL,
		ROCE_CMD_TIME_CLASS_B, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
...
static int roce3_dfx_cmd_query_gid(struct roce3_device *rdev, u32 port, u32 gid_index, struct rdma_gid_entry *gid_entry)
{
	int ret;
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	cqm_cmd_buf_s *cqm_cmd_outbuf = NULL;
	roce_cmd_query_gid_s *gid_query_inbuf = NULL;
	struct rdma_gid_query_outbuf *gid_query_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_query_gid_s), &cqm_cmd_outbuf,
	...
	gid_query_inbuf = (roce_cmd_query_gid_s *)cqm_cmd_inbuf->buf;
	...
	ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, ROCE_CMD_QUERY_GID, cqm_cmd_inbuf, cqm_cmd_outbuf, NULL,
		ROCE_CMD_TIME_CLASS_B, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
...
static int roce3_dfx_query_cache_mpt(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	int ret;
	...
	if (ret != 0) {
		...
		roce3_dfx_print("******************From Cache: mpt_key(%#x) ********************", mpt_key);
...
static int roce3_dfx_query_host_mpt(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	u32 mpt_index = 0;
	...
	if (cqm_obj_mpt == NULL) {
		...
		roce3_dfx_print("******************From Host: mpt_key(%#x) ********************", mpt_key);
...
static int roce3_dfx_query_cache_gid(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	int ret;
	...
	if (ret != 0) {
		roce3_dfx_print("***************port(1), gid_index(%d) *****************", gid_index);
...
static int roce3_dfx_query_pi_ci(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf)
{
	int ret;
...
typedef int (*roce3_adm_dfx_query_t)(const struct roce3_dfx_query_inbuf *inbuf, struct roce3_device *rdev,
	union roce3_dfx_query_outbuf *outbuf);
...
int roce3_adm_dfx_query(struct roce3_device *rdev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
