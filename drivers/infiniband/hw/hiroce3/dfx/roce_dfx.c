 ...
 *
 * File Name	 : roce_dfx.c
 * Version	   : v2.1
 * Created	   : 2020/8/29
 * Last Modified : 2021/10/16
 * Description   : The definition of callback functions and internal functions of RoCE driver dfx features.
 */
...
int roce3_get_drv_version(struct roce3_device *rdev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
	...
	rc = snprintf(ver_info->ver, sizeof(ver_info->ver), "%s  %s",
		HIROCE3_DRV_VERSION, __TIME_STR__);
	if (rc == -1) {
...
static int roce3_dfx_change_bw_ctrl_param(struct roce3_device *rdev, struct roce3_bw_ctrl_inbuf *inbuf,
	struct roce3_bw_ctrl_outbuf *outbuf)
{
	return roce3_set_bw_ctrl_state(rdev, ROCE_BW_CTRL_RESET, inbuf);
}

static int roce3_dfx_query_bw_ctrl_param(struct roce3_device *rdev, struct roce3_bw_ctrl_inbuf *inbuf,
	struct roce3_bw_ctrl_outbuf *outbuf)
{
	return roce3_query_bw_ctrl_state(rdev, &outbuf->bw_ctrl_param);
}

typedef int (*roce3_adm_dfx_bw_ctrl_func_t)(struct roce3_device *rdev, struct roce3_bw_ctrl_inbuf *inbuf,
	struct roce3_bw_ctrl_outbuf *outbuf);
...
int roce3_adm_dfx_bw_ctrl(struct roce3_device *rdev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
