 ...
 *
 * File Name	 : roce_dfx_cap.c
 * Version	   : v2.1
 * Created	   : 2022/7/28
 * Last Modified : 2022/7/28
 * Description   : The definition of callback functions and internal functions of RoCE driver dfx cap features.
 */
...
{
	thread_info->thread_obj = kthread_run(roce3_linux_thread_func, thread_info, thread_info->name);
	if (!thread_info->thread_obj) {
...
static void roce3_fill_cap_cfg(struct roce3_dfx_cap_cfg_tbl *cfg_info, u32 state, u32 ci_index, u8 mode)
{
	cfg_info->ci_index = ci_index;

	cfg_info->dw1.bs.cap_block_num_shift = (u8)g_nattr[g_roce3_cap_info.block_num_idx].shift; // 8
...
static void roce3_pkt_cap_poll_hdr(struct roce3_pkt_cap_info *cap_info, union roce3_cap_hdr **cap_hdr,
	union roce3_cap_hdr *hdr_value)
{
	u32 lt_index;
	u32 lt_offset;
	u8 sel;
	...
	*cap_hdr = (roce3_cap_hdr_u *)(cap_info->que_addr[sel][0][lt_index] + (lt_offset * CAP_PKT_ITEM_SIZE));
...
static void roce3_pkt_cap_poll(void *data)
{
	...
	do {
		...
		roce3_netif_skb_info(rdev, ((u8 *)cap_hdr + CAP_HDR_OFFSET + hdr_value.cap_ctrl_info.pad), CAP_SKB_LEN);
		...
		if (g_roce3_cap_info.cap_mode != 0) {
			...
			if (count >= (FILL_CAP_CFG_PKT_NUM)) {
				...
				(void)roce3_set_cap_cfg(rdev->hwdev, CAP_PA_TBL_ENTRY_NUM, (u32 *)&cfg_info);
	...
	struct roce3_dfx_cap_tbl *pa_info)
{
	...
	v_addr = (uintptr_t)(u64 *)dma_alloc_coherent(&rdev->pdev->dev,
		(unsigned long)(CAP_PKT_ITEM_SIZE * ((g_nattr[g_roce3_cap_info.block_num_idx].num) >> 1)), &p_addr, GFP_KERNEL);
	if (v_addr == 0) {
	...
	pa_info->pa[que_index].wr_init_pc_h32 = (u32)((p_addr >> CAP_ADDR_COMBIN_SHIFT) & 0xffffffff);
	...
	dev_info(rdev->hwdev_hdl, "[ROCE] %s:  v_addr que_index(%d), cap_index(%d), PA:0x%x %x\n", __func__, que_index,
		cap_index, pa_info->pa[que_index].wr_init_pc_h32, pa_info->pa[que_index].wr_init_pc_l32);
...
static void roce3_dfx_dma_free_addr(struct roce3_device *rdev, int cap_index, int que_index)
{
	...
	dma_free_coherent(&rdev->pdev->dev,
		(unsigned long)(CAP_PKT_ITEM_SIZE * ((g_nattr[g_roce3_cap_info.block_num_idx].num) >> 1)), (void *)v_addr,
		(dma_addr_t)p_addr);
}
...
static void roce3_dfx_init_g_cap_info(struct roce3_device *rdev, const struct roce3_dfx_capture_inbuf *inbuf)
{
	u32 mode = inbuf->mode;
	...
	g_roce3_cap_info.maxnum = (g_nattr[g_roce3_cap_info.block_num_idx].num) * CAP_PA_TBL_ENTRY_NUM;
	g_roce3_cap_info.mode = mode;
	g_roce3_cap_info.qp_list_cnt = 0;
}

static int roce3_dfx_start_cap_pkt(struct roce3_device *rdev, const struct roce3_dfx_capture_inbuf *inbuf,
...
static int roce3_dfx_query_cap_pkt(struct roce3_device *rdev, const struct roce3_dfx_capture_inbuf *inbuf,
	union roce3_dfx_capture_outbuf *outbuf)
{
	int ret;
	u32 pi = 0;
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to read counter(lt index %x), ret(%d)\n", __func__,
			ROCE_CAP_COUNTER_INDEX, ret);
		return -EFAULT;
	}
...
typedef int (*roce3_adm_dfx_capture_func_t)(struct roce3_device *rdev, const struct roce3_dfx_capture_inbuf *inbuf,
	union roce3_dfx_capture_outbuf *outbuf);
...
int roce3_adm_dfx_capture(struct roce3_device *rdev, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
	int ret;
