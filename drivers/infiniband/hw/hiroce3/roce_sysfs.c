 ...
 *
 * File Name	 : roce_sysfs.c
 * Version	   : v2.0
 * Created	   : 2021/3/14
 * Last Modified : 2022/02/19
 * Description   : RoCE sysfs for cc_algo.
 */
...
#define ROCE_ATTR_RW(_name, _show, _store) \
	static struct kobj_attribute roce_attr_##_name = \
		INIT_ROCE_KOBJ_ATTR(_name, (S_IWUSR | S_IRUSR | S_IRGRP), _show, _store)

#define ROCE_ATTR_PTR(_name) (&roce_attr_##_name.attr)

static int roce3_update_cfg_ccf_param(struct roce3_device *rdev, const struct roce3_ecn_ctx *ecn_ctx)
{
	...
	for (i = 0; i < PRI_ARRAY_LEN; i++) {
		...
		ccf_param.dw0.bs.ip_enable |= (ip_prio_enable_ctx->prio_en << ip_prio_enable_ctx->prio);
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] : Failed to set ccf param, func_id(%d) ret(%d)\n", rdev->glb_func_id,
			ret);
	}
...
static int roce3_update_dcqcn_param(struct roce3_device *rdev, const struct roce3_ecn_ctx *ecn_ctx)
{
	...
	ret = roce3_set_cfg_dcqcn_param(rdev->hwdev, rdev->glb_func_id, (u32 *)(void *)&dcqcn_param);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] : Failed to set dcqcn param, func_id(%d), ret(%d)\n", rdev->glb_func_id,
			ret);
	}
...
static int roce3_update_ipqcn_param(struct roce3_device *rdev, const struct roce3_ecn_ctx *ecn_ctx)
{
	...
	ret = roce3_set_cfg_ipqcn_param(rdev->hwdev, rdev->glb_func_id, (u32 *)(void *)&ipqcn_param);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] : Failed to set ipqcn param, func_id(%d) ret(%d)\n", rdev->glb_func_id,
			ret);
	}
...
static ssize_t roce3_store_cc_algo(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	struct roce3_ecn_ctx *ecn_ctx = to_roce3_ecn_ctx(kobj);
	u32 old_cc_algo = 0;
	int cc_algo = 0;
	...
	} else if ((strncmp(buf, "ipqcn", strlen("ipqcn")) == 0) || (strncmp(buf, "hc3_1", strlen("hc3_1")) == 0)) {
	...
	if (ecn_ctx->cc_algo != (u32)cc_algo) {
		...
		if (ret != 0) {
			pr_err("[ROCE] %s: Failed to update cc_algo param, ret(%d)", __func__, (int)ret);
...
static ssize_t roce3_show_ecn_prio_enable(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_prio_enable_ctx *prio_enable_ctx = container_of(attr, struct roce3_prio_enable_ctx, enable);

	return sprintf(buf, "%d\n", (int)prio_enable_ctx->prio_en);
}

static ssize_t roce3_store_ecn_prio_enable(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	int prio_en = 0;
	...
	struct roce3_prio_enable_ctx *prio_enable_ctx = container_of(attr, struct roce3_prio_enable_ctx, enable);
	struct roce3_ecn_enable_ctx *ecn_enable_ctx = (struct roce3_ecn_enable_ctx *)prio_enable_ctx->ecn_enable_ctx;
...
static ssize_t roce3_show_ecn_ip_prio_enable(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
...
static ssize_t roce3_store_ecn_ip_prio_enable(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	int prio_en = 0;
	...
	struct roce3_ecn_ctx *ecn_ctx = container_of(ip_enable_ctx, struct roce3_ecn_ctx, ip_enable_ctx);
...
static ssize_t roce3_store_rate_inc_hai(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	int rate_inc_hai = 0;
...
static ssize_t roce3_store_rate_inc_ai(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	int rate_inc_ai = 0;
...
static ssize_t roce3_show_initial_alpha(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_initial_alpha(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	int initial_alpha = 0;
	...
	ret = sscanf(buf, "%d", &initial_alpha);
	if (ret != 1) {
		return -EIO;
	}
...
static ssize_t roce3_store_factor_gita(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	int factor_gita = 0;
...
static ssize_t roce3_show_rate_inc_period(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_rate_inc_period(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &rate_inc_period);
	if (ret != 1) {
		return -EIO;
	}

	if ((rate_inc_period < ROCE_MIN_RATE_INC_PERIOD) || (rate_inc_period > ROCE_MAX_RATE_INC_PERIOD)) {
		return -EIO;
	}
...
static ssize_t roce3_show_cnp_cnt_threshold(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_cnp_cnt_threshold(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &cnp_cnt_threshold);
	if (ret != 1) {
		return -EIO;
	}

	if ((cnp_cnt_threshold < ROCE_MIN_CNP_CNT_THRESHOLD) || (cnp_cnt_threshold > ROCE_MAX_CNP_CNT_THRESHOLD)) {
		return -EIO;
	}
...
static ssize_t roce3_show_alpha_threshold(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_alpha_threshold(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%u", &alpha_threshold);
	if (ret != 1) {
		return -EIO;
	}

	if ((alpha_threshold < ROCE_MIN_ALPHA_THRESHOLD) || (alpha_threshold > ROCE_MAX_ALPHA_THRESHOLD)) {
		return -EIO;
	}
...
static ssize_t roce3_show_rate_dec_period(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_rate_dec_period(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &rate_dec_period);
	if (ret != 1) {
		return -EIO;
	}

	if ((rate_dec_period < ROCE_MIN_RATE_DEC_PERIOD) || (rate_dec_period > ROCE_MAX_RATE_DEC_PERIOD)) {
		return -EIO;
	}
...
static ssize_t roce3_store_token_period(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	int token_period = 0;
	...
	ret = sscanf(buf, "%d", &token_period);
	if (ret != 1) {
		return -EIO;
	}

	if ((token_period < ROCE_MIN_TOKEN_PERIOD) || (token_period > ROCE_MAX_TOKEN_PERIOD)) {
		return -EIO;
	}
...
static ssize_t roce3_store_min_rate(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	int min_rate = 0;
	u32 old_min_rate = 0;
...
static ssize_t roce3_show_alpha_dec_period(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_alpha_dec_period(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &alpha_dec_period);
	if (ret != 1) {
		return -EIO;
	}

	if ((alpha_dec_period < ROCE_MIN_ALPHA_DEC_PERIOD) || (alpha_dec_period > ROCE_MAX_ALPHA_DEC_PERIOD)) {
		return -EIO;
	}
...
static ssize_t roce3_show_rate_first_set(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_rate_first_set(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &rate_first_set);
	if (ret != 1) {
		return -EIO;
	}

	if ((rate_first_set < ROCE_MIN_RATE_FIRST_SET) || (rate_first_set > ROCE_MAX_RATE_FIRST_SET)) {
		return -EIO;
	}
...
static ssize_t roce3_show_rate_target_clamp(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_rp_ctx *ecn_rp_ctx = to_roce3_ecn_rp_ctx(kobj);
...
static ssize_t roce3_store_rate_target_clamp(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &rate_target_clamp);
	if (ret != 1) {
		return -EIO;
	}
...
static ssize_t roce3_show_min_cnp_period(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
...
static ssize_t roce3_store_min_cnp_period(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &min_cnp_period);
	if (ret != 1) {
		return -EIO;
	}

	if ((min_cnp_period < ROCE_MIN_MIN_CNP_PERIOD) || (min_cnp_period > ROCE_MAX_MIN_CNP_PERIOD)) {
		return -EIO;
	}
...
static ssize_t roce3_store_port_mode(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	int port_mode = 0;
	u32 old_port_mode = 0;
...
static ssize_t roce3_show_quick_adjust_en(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_np_ctx *ecn_np_ctx = to_roce3_ecn_np_ctx(kobj);
...
static ssize_t roce3_store_quick_adjust_en(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &quick_adjust_en);
	if (ret != 1) {
		return -EIO;
	}
...
static ssize_t roce3_show_cnp_prio_enable(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct roce3_ecn_np_ctx *ecn_np_ctx = to_roce3_ecn_np_ctx(kobj);
...
static ssize_t roce3_store_cnp_prio_enable(struct kobject *kobj, struct kobj_attribute *attr, const char *buf,
	size_t count)
{
	ssize_t ret = 0;
	...
	ret = sscanf(buf, "%d", &cnp_prio_enable);
	if (ret != 1) {
		return -EIO;
	}
...
static ssize_t roce3_store_cnp_prio(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	int cnp_prio = 0;
	u32 old_cnp_prio = 0;
...
static int roce3_init_ecn_enable_sysfs(struct kobject *kobj, struct roce3_ecn_enable_ctx *ecn_enable_ctx)
{
	int ret = 0;
	int i = 0;
	int j = 0;
...
err_create_file:
	for (j = i - 1; j >= 0; j--) {
		sysfs_remove_file(ecn_enable_ctx->enable_root, &ecn_enable_ctx->prio_enable[i].enable.attr);
	}
...
static void roce3_remove_ecn_enable_sysfs(struct kobject *kobj, struct roce3_ecn_enable_ctx *ecn_enable_ctx)
{
	int i;
...
static int roce3_init_ecn_ip_enable_sysfs(struct kobject *kobj, struct roce3_ecn_ip_enable_ctx *ip_enable_ctx)
{
	int ret = 0;
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < PRI_ARRAY_LEN; i++) {
		...
		ip_prio_enable->ip_enable.attr.mode = (S_IWUSR | S_IRUSR | S_IRGRP);
		ip_prio_enable->ip_enable.show = roce3_show_ecn_ip_prio_enable;
		ip_prio_enable->ip_enable.store = roce3_store_ecn_ip_prio_enable;
		ret = sysfs_create_file(ip_enable_ctx->ip_enable_root, &ip_prio_enable->ip_enable.attr);
		if (ret != 0) {
...
err_create_file:
	for (j = i - 1; j >= 0; j--) {
		sysfs_remove_file(ip_enable_ctx->ip_enable_root, &ip_enable_ctx->ip_prio_enable[i].ip_enable.attr);
	}
...
static void roce3_remove_ecn_ip_enable_sysfs(struct kobject *kobj, struct roce3_ecn_ip_enable_ctx *ip_enable_ctx)
{
	int i;
...
static ssize_t roce3_store_ecn_ver(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	int ecn_ver = 0;
	u32 old_ecn_ver = 0;
...
static void roce3_prase_qpc(struct roce_qp_context *qpc)
{
	...
	qpc->sw_seg.ucode_seg.common.dw3.value = be32_to_cpu(qpc->sw_seg.ucode_seg.common.dw3.value);
	qpc->sw_seg.ucode_seg.sq_ctx.dw9.value = be32_to_cpu(qpc->sw_seg.ucode_seg.sq_ctx.dw9.value);
	qpc->sw_seg.ucode_seg.sq_ctx.ack_ctx.dw15.value = be32_to_cpu(qpc->sw_seg.ucode_seg.sq_ctx.ack_ctx.dw15.value);
	qpc->sw_seg.ucode_seg.rq_ctx.dw22.value = be32_to_cpu(qpc->sw_seg.ucode_seg.rq_ctx.dw22.value);
	qpc->sw_seg.ucode_seg.rq_ctx.ack_ctx.dw27.value = be32_to_cpu(qpc->sw_seg.ucode_seg.rq_ctx.ack_ctx.dw27.value);
}

static ssize_t roce3_store_dfx_qpc(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret;
	u32 qpn;
	...
	roce3_dfx_print("qpn(0x%x): sqc pi(0x%x), sqc ci(0x%x), ccf_appid(0x%x)\n", qpn, qpc.chip_seg.sqc.dw2.bs.sq_pi,
		qpc.chip_seg.sqc.dw3.bs.sq_ci, qpc.sw_seg.ucode_seg.common.dw0.bs.ccf_appid);
	roce3_dfx_print("sqac ci(0x%x), sqac wqe_prefetch_ci(0x%x), rqc pi(0x%x), rqc ci(0x%x)\n",
		qpc.chip_seg.sqac.dw3.bs.sqa_ci, qpc.chip_seg.sqac.dw7.bs.sqa_wqe_prefetch_ci, qpc.chip_seg.rqc.dw7.bs.rq_pi,
		qpc.chip_seg.rqc.dw3.bs.rq_ci);
	roce3_dfx_print("sq_ssn(0x%x), sq_rcv_msn(0x%x), rq_last_msn(0x%x), rqa_msn(0x%x)\n",
		qpc.sw_seg.ucode_seg.sq_ctx.dw9.bs.ssn, qpc.sw_seg.ucode_seg.sq_ctx.ack_ctx.dw15.bs.sq_rcv_msn,
		qpc.sw_seg.ucode_seg.rq_ctx.dw22.bs.last_msn, qpc.sw_seg.ucode_seg.rq_ctx.ack_ctx.dw27.bs.msn);

	return (ssize_t)count;
}

static ssize_t roce3_store_dfx_cqc(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret;
	u32 cqn;
	...
	if (cqn >= rdma_cap->dev_rdma_cap.roce_own_cap.max_cqs) {
		pr_err("[ROCE] %s: Invalid cqn(%u), max_cqs(%u)\n", __func__, cqn, rdma_cap->dev_rdma_cap.roce_own_cap.max_cqs);
		return -EIO;
	}
...
static ssize_t roce3_store_dfx_srqc(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int ret;
	u32 srqn;
