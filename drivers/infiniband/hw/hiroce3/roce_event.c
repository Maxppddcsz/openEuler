...
static void roce3_event_report_common_a(const struct roce3_device *rdev, int event_str_index)
{
	...
	case OFED_ET_PATH_MIG:
		pr_err_ratelimited("[ROCE] %s: [ofed event type] Path mig. function id: %hu \n", __func__,
			rdev->glb_func_id);
		break;

	case OFED_ET_COMM_EST:
		/*lint -e160 -e522*/
		roce3_pr_err_once("[ROCE] %s: [ofed event type] Communication establish. function id: %hu\n", __func__,
			rdev->glb_func_id);
		/*lint +e160 +e522*/
		break;

	case OFED_ET_SQ_DRAINED:
		pr_err_ratelimited("[ROCE] %s: [ofed event type] Sq drained. function id: %hu\n", __func__,
			rdev->glb_func_id);
		break;

	case OFED_ET_SRQ_QP_LAST_WQE:
		/*lint -e160 -e522*/
		roce3_pr_err_once("[ROCE] %s: [ofed event type] Srq/qp last wqe. function id: %hu\n", __func__,
			rdev->glb_func_id);
		/*lint +e160 +e522*/
		break;

	case OFED_ET_WQ_CATAS_ERR:
		pr_err_ratelimited("[ROCE] %s: [ofed event type] Wq catas error. function id: %hu\n", __func__,
			rdev->glb_func_id);
		break;
...
static void roce3_handle_qp_async_event(struct roce3_device *rdev, u32 qpn, u8 event_type)
{
	...
	if (cqm_obj_queue == NULL) {
		dev_err_ratelimited(rdev->hwdev_hdl, "[ROCE] %s: Failed to get cqm obj queue, func_id(%d), qpn:%u,\
...
static void roce3_handle_cq_async_event(struct roce3_device *rdev, u32 cqn, u8 event_type)
{
	...
	if (cqm_obj_queue == NULL) {
		dev_err_ratelimited(rdev->hwdev_hdl, "[ROCE] %s: Failed to get cqm obj queue, func_id(%d), cqn:%u,\
...
static void roce3_handle_srq_async_event(struct roce3_device *rdev, u32 srqn, u8 event_type)
{
	...
	if (cqm_obj_queue == NULL) {
		dev_err_ratelimited(rdev->hwdev_hdl, "[ROCE] %s: Failed to get cqm obj queue, func_id(%d), srqn:%u,\
...
static void roce3_handle_sq_hot_plug(struct roce3_qp *rqp, unsigned long flags_qp, struct list_head *cq_notify_list)
{
...
static void roce3_handle_rq_hot_plug(struct roce3_qp *rqp, unsigned long flags_qp, struct list_head *cq_notify_list)
{
...
void roce3_kernel_hotplug_event_trigger(struct roce3_device *rdev)
{
	...
	if (rdev->ndev) {
		...
		if (carrier_ok != 0) {
			dev_info(rdev->hwdev_hdl, "[ROCE] %s: Turn off, func_id(%hu), name(%s), pci(%s),\n",
				__func__, rdev->glb_func_id, rdev->ib_dev.name, pci_name(rdev->pdev));
...
void roce3_handle_bonded_port_state_event(struct roce3_device *rdev)
{
	...
	if (netif_running(master) != 0) {
		...
		for (i = 0; i < bond_dev->slave_cnt; i++) {
			...
			bonded_port_state = (bonded_port_state != IB_PORT_ACTIVE) ? curr_port_state : IB_PORT_ACTIVE;
		}
	...
	if (rdev->port_state != bonded_port_state) {
		event = (bonded_port_state == IB_PORT_ACTIVE) ? IB_EVENT_PORT_ACTIVE : IB_EVENT_PORT_ERR;
