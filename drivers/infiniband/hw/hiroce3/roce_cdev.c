...
static struct roce3_qp *roce3_cdev_lookup_rqp(struct roce3_device *rdev, u32 qpn)
{
	...
	if (cqm_obj_qp == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Can't find rqp according to qpn(0x%x), func_id(%d)\n", __func__, qpn,
			rdev->glb_func_id);
		return NULL;
	}
...
static int roce3_get_qp_func_table(struct roce3_device *rdev, u32 qpn, u32 *func_table_val)
{
	...
	roce_get_qp_func_table_s *get_func_tbl_inbuf = NULL;
	struct roce3_get_func_table *get_func_tbl_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_get_qp_func_table_s),
		&cqm_cmd_outbuf, (u16)sizeof(roce_get_qp_func_table_s));
	if (ret != 0) {
	...
	get_func_tbl_inbuf = (roce_get_qp_func_table_s *)cqm_cmd_inbuf->buf;
...
static long roce3_cdev_query_qp_tx_port(struct roce3_device *rdev, void *buf)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to get func_tbl, func_id(%d)\n", __func__, rdev->glb_func_id);
		return ret;
	}
	...
	qp_port_buf.port = (func_tbl_value >>
		(ROCE_BOND_FWD_ID_TBL_ALL_BITS - ((rqp->tx_hash_value % slave_cnt + 1) * ROCE_BOND_FWD_ID_TBL_PER_BITS))) &
...
static int roce3_modify_bond_hash_value(struct roce3_device *rdev, u32 qpn, u32 new_hash_value)
{
	int ret;
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	roce_modify_hash_value_s *modify_hash_value = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_modify_hash_value_s), NULL,
		0);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
	...
	modify_hash_value = (roce_modify_hash_value_s *)cqm_cmd_inbuf->buf;
	...
	ret = roce3_send_qp_lb_cmd(qpn, rdev, ROCE_CMD_MODIFY_HASH_VALUE_QP, cqm_cmd_inbuf, NULL, ROCE_CMD_TIME_CLASS_A);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to send MODIFY_HASH_VALUE_QP command, func_id(%d)\n", __func__,
			rdev->glb_func_id);
...
static long roce3_cdev_set_qp_tx_port(struct roce3_device *rdev, const void *buf)
{
	...
	new_tx_hash_value = rqp->tx_hash_value + (((slave_cnt - old_hash_port) + target_hash_port) % slave_cnt);
...
static int roce3_get_qp_rx_port(struct roce3_device *rdev, u32 qpn, u32 *rx_port)
{
	...
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	cqm_cmd_buf_s *cqm_cmd_outbuf = NULL;
	roce_get_qp_rx_port_s *get_rx_port_inbuf = NULL;
	struct roce3_get_rx_port *get_rx_port_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_get_qp_rx_port_s),
		&cqm_cmd_outbuf, (u16)sizeof(roce_get_qp_rx_port_s));
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
		return -ENOMEM;
	...
	get_rx_port_inbuf = (roce_get_qp_rx_port_s *)cqm_cmd_inbuf->buf;
...
static long roce3_cdev_query_next_wqe_idx(struct roce3_device *rdev, void *buf)
{
	...
	ret_adm = roce3_adm_dfx_query(rdev, &dfx_query_inbuf, sizeof(dfx_query_inbuf), &dfx_query_outbuf, &out_size);
	if (ret_adm != 0) {
	...
	if (dfx_query_outbuf.srq_ctx.dw2.bs.container_en == 0) {
		...
		u32 container_mode = MAX_SUPPORT_CONTAINER_MODE - dfx_query_outbuf.srq_ctx.dw2.bs_c.container_size;
		u32 container_size = roce3_get_container_sz(container_mode);
		next_wqe_idx = (int)(container_size * dfx_query_outbuf.srq_ctx.dw3.bs_c.head_index);
	}
	ret = (long)copy_to_user((void __user *)buf, &next_wqe_idx, sizeof(int));
	if (ret != 0) {
		(void)pr_err("[ROCE] %s: Failed to copy next_wqe_idx to user, ret(0x%lx)\n", __func__, ret);
		return ret;
	}
...
static void roce3_cdev_set_bond_port_info(
	struct roce3_bond_device *bond_dev, u32 func_tbl_value, struct roce3_bond_port_info_buf *bond_port_info_buf)
{
	u32 i;
	...
	for (i = 0; i < bond_port_info_buf->alive_port_num; i++) {
		bond_port_info_buf->alive_port[i] =
			(func_tbl_value >> (ROCE_BOND_FWD_ID_TBL_ALL_BITS - ((i + 1) * ROCE_BOND_FWD_ID_TBL_PER_BITS))) &
			ROCE_BOND_FWD_ID_TBL_PER_BITS_MASK;
	}
}
...
static long roce3_cdev_query_bond_port_info(struct roce3_device *rdev, void *buf)
{
	...
	if (!roce3_bond_is_active(rdev)) {
		ret = (long)copy_to_user((void __user *)buf, &bond_port_info_buf, sizeof(bond_port_info_buf));
		if (ret != 0) {
			pr_err("[ROCE, ERR] %s: Failed to copy data to user\n", __func__);
			return ret;
		}
	...
	if (bond_dev == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Can't find bond_dev, func_id(%d)\n", __func__, rdev->glb_func_id);
		return EINVAL;
	}
...
static int roce3_modify_udp_src_port(struct roce3_device *rdev, u32 qpn, u32 new_udp_src_port)
{
	int ret;
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	roce_modify_udp_src_port_s *modify_udp_src_port = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_modify_udp_src_port_s),
		NULL, 0);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
	...
	modify_udp_src_port = (roce_modify_udp_src_port_s *)cqm_cmd_inbuf->buf;
	...
	ret = roce3_send_qp_lb_cmd(qpn, rdev, ROCE_CMD_MODIFY_UDP_SRC_PORT_QP, cqm_cmd_inbuf, NULL, ROCE_CMD_TIME_CLASS_A);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to send MODIFY_HASH_VALUE_QP command, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		roce3_cqm_cmd_free_inoutbuf(rdev->hwdev, cqm_cmd_inbuf, NULL);
...
static int roce3_get_qp_udp_src_port(struct roce3_device *rdev, u32 qpn, u32 *udp_src_port)
{
	...
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	cqm_cmd_buf_s *cqm_cmd_outbuf = NULL;
	roce_get_qp_udp_src_port_s *get_udp_src_port = NULL;
	struct roce3_get_udp_src_port *get_udp_src_port_outbuf = NULL;

	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_get_qp_udp_src_port_s),
		&cqm_cmd_outbuf, (u16)sizeof(roce_get_qp_udp_src_port_s));
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
		return -ENOMEM;
	...
	get_udp_src_port = (roce_get_qp_udp_src_port_s *)cqm_cmd_inbuf->buf;
...
static long roce3_cdev_query_qp_udp_src_port(struct roce3_device *rdev, void *buf)
{
	...
	ret = (long)copy_to_user((void __user *)buf, &qp_udp_src_port_buf, sizeof(qp_udp_src_port_buf));
	if (ret != 0) {
		pr_err("[ROCE, ERR] %s: Failed to copy data to user\n", __func__);
		return ret;
	}
...
static int roce3_cdev_open(struct inode *inode, struct file *filp)
{
	...
	if (file == NULL) {
		pr_err("[ROCE, ERR] %s: Failed to open cdev\n", __func__);
		return -ENOMEM;
	}
...
#ifdef ROCE_BONDING_EN
static long roce3_cdev_bonding_ioctl_part_1(unsigned int cmd, unsigned long arg, struct roce3_device *rdev)
{
	long ret = 0;
...
static long roce3_cdev_bonding_ioctl_part_2(unsigned int cmd, unsigned long arg, struct roce3_device *rdev)
{
	long ret = 0;
...
static int roce3_bond_get_dcb_info_from_buddy(struct roce3_device *rdev, struct roce3_bond_device *bond_dev)
{
	int i;
	int ret;
...
int roce3_bond_get_dcb_info(struct roce3_device *rdev)
{
	...
	bond_dev_up = (bond_dev->attr.active_slaves & ((unsigned int)1 << rdev->glb_func_id)) != 0;
	if (!bond_dev_up) {
...
static long roce3_cdev_query_dcb(struct roce3_device *rdev, void *buf)
{
	...
	ret = (long)roce3_get_dcb_cfg_cos(rdev, (roce3_get_cos_inbuf_s *)(void *)(&dcb_buf), &cos);
	if (ret != 0) {
...
static long roce3_cdev_ioctl_non_bonding(unsigned int cmd, struct roce3_device *rdev, unsigned long arg)
{
	long ret = 0;
...
typedef struct tag_HW_ROCE_IOCTL_ACTION {
	unsigned int cmd;
	long (*action_func)(struct roce3_device *rdev, void *buf);
} HW_ROCE_IOCTL_ACTION;

static HW_ROCE_IOCTL_ACTION ioctl_reserved_tbl[] = {
...
static long roce3_cdev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	for (idx = 0; idx < sizeof(ioctl_reserved_tbl) / sizeof((ioctl_reserved_tbl)[0]); idx++) {
		if (cmd == ioctl_reserved_tbl[idx].cmd && ioctl_reserved_tbl[idx].action_func != NULL) {
...
static int roce3_init_cdev_info(struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Add cdev failed, func_id(%d)\n", __func__, rdev->glb_func_id);
		goto err_add_cdev;
	}
	...
	if (IS_ERR(rdev->cdev.cdev_class)) {
		...
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Create class failed, func_id(%d)\n", __func__, rdev->glb_func_id);
	...
	rdev->cdev.dev = device_create(rdev->cdev.cdev_class, NULL, rdev->cdev.dev_major, NULL, "%s", rdev->ib_dev.name);
	if (IS_ERR(rdev->cdev.dev)) {
		ret = (int)PTR_ERR(rdev->cdev.dev);
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Create device failed, func_id(%d)\n", __func__, rdev->glb_func_id);
...
int roce3_init_cdev(struct roce3_device *rdev)
{
	...
	rdev->cdev.dev_major = MKDEV(ROCE_BASE_CDEV_MAJOR, (u32)rdev->cdev.dev_num + ROCE_BASE_CDEV_MINOR);
	...
	if (ret == -EBUSY) {
		...
		ret = alloc_chrdev_region(&(rdev->cdev.dev_major), ROCE_BASE_CDEV_MINOR, 1, rdev->ib_dev.name);
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: alloc cdev region, major(%d), minor(%d), func_id(%d), ret(%d)\n",
			__func__, MAJOR(rdev->cdev.dev_major), MINOR(rdev->cdev.dev_major), rdev->glb_func_id, ret);
	}

	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Register region failed, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
