...
static int hmm_init_table(void *hwdev, struct hmm_comp_priv *comp_priv, u32 srv_type)
{
	...
	ret = hinic3_register_service_adapter((void *)hwdev, (void *)comp_priv, (enum hinic3_service_type)srv_type);
	if (ret != 0) {
...
int hmm_init_resource(void *hwdev, u32 service_type)
{
	...
	comp_priv = (struct hmm_comp_priv *)kzalloc(sizeof(struct hmm_comp_priv), GFP_KERNEL);
	if (comp_priv == NULL) {
