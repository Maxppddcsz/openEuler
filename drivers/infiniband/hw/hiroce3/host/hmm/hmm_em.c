...
static void hmm_em_chunk_free(struct pci_dev *pdev, struct hmm_em_chunk *em_chunk)
{
	...
	while (next_buf != NULL) {
		...
		if ((cur_buf->buf != NULL) && (cur_buf->length > 0)) {
			...
			dma_free_coherent(&pdev->dev, (unsigned long)cur_buf->length, cur_buf->buf, cur_buf->dma_addr);
			cur_buf->buf = NULL;
			cur_buf->length = 0;
		}
...
static int hmm_em_chunk_alloc_npages(struct pci_dev *pdev, struct hmm_em_chunk *em_chunk, int min_order)
{
	int cur_order = 0;
	int npages = 0;
	...
	while (npages > 0) {
		if (next_buf == NULL) {
			next_buf = (struct hmm_em_buf *)kzalloc(sizeof(struct hmm_em_buf), GFP_KERNEL);
			if (next_buf == NULL) {
				dev_err(&pdev->dev, "[HMM] %s: Alloc memory for em_buf failed, err(%d)\n", __FUNCTION__, -ENOMEM);
				return (-ENOMEM);
			}
		...
		next_buf->buf = dma_alloc_coherent(&pdev->dev, (size_t)HMM_EM_PAGE_SIZE << (unsigned int)cur_order,
			&next_buf->dma_addr, GFP_KERNEL);
		if (next_buf->buf == NULL) {
			cur_order--;
			if (cur_order < min_order) {
				dev_err(&pdev->dev, "[HMM] %s:em_chunk alloc dma buf failed, err(%d)\n", __FUNCTION__, -ENOMEM);
				return (-ENOMEM);
			} else {
				dev_err(&pdev->dev, "[HMM, WARN] %s: em_chunk alloc  %d o dma failed. cont alloc small mem.\n",
...
static struct hmm_em_chunk *hmm_em_chunk_alloc(struct pci_dev *pdev, int min_order)
{
	...
	em_chunk = (struct hmm_em_chunk *)kzalloc(sizeof(struct hmm_em_chunk), GFP_KERNEL);
	if (em_chunk == NULL) {
		dev_err(&pdev->dev, "[HMM] %s: Alloc memory for em_chunk failed, err(%d)\n", __FUNCTION__, -ENOMEM);
...
void hmm_em_table_put_range(struct pci_dev *pdev, struct hmm_em_table *em_table, u32 start, u32 end)
{
	int i = 0;
	int inc = 0;
...
int hmm_em_table_get_range(struct pci_dev *pdev, struct hmm_em_table *em_table, u32 start, u32 end)
{
	...
	for (i = (int)start; i <= (int)end; i += inc) {
		...
		if (ret != 0) {
			dev_err(&pdev->dev, "[HMM] %s: Get entry failed, start(%d), end(%d), i(%d), ret(%d)\n", __FUNCTION__, start,
				end, i, ret);
			goto err_out;
		}
	}
...
int hmm_em_init_table(struct pci_dev *pdev, struct hmm_em_table *em_table, u32 obj_size, u32 nobj, u32 reserved_bot,
	int min_order)
{
	u32 obj_per_chunk = 0;
	u32 chunk_num = 0;
	...
	em_table->em_chunk = (struct hmm_em_chunk **)kcalloc((size_t)chunk_num, sizeof(struct hmm_em_chunk *), GFP_KERNEL);
	if (em_table->em_chunk == NULL) {
		dev_err(&pdev->dev, "[HMM] %s: Em_table->em_chunk create failed\n", __FUNCTION__);
		return -ENOMEM;
	}
