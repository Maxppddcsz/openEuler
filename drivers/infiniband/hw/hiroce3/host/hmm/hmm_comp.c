...
{
	return (struct hmm_comp_priv *)hinic3_get_service_adapter(hwdev, (enum hinic3_service_type)service_type);
}

static void assemble_mpt_hw2sw(struct rdma_mpt_hw2sw_inbuf **mpt_hw2sw_inbuf, struct hmm_comp_priv *comp_priv,
...
int hmm_enable_roce_mpt(void *hwdev, cqm_cmd_buf_s *cqm_cmd_inbuf, u16 channel)
{
	int ret;

	ret = cqm_send_cmd_box(hwdev, HINIC3_MOD_ROCE, RDMA_ROCE_CMD_SW2HW_MPT, cqm_cmd_inbuf, NULL, NULL,
		RDMA_CMD_TIME_OUT_A, channel);
	if (ret != 0) {
...
static int hmm_mpt_read_back_test(struct hmm_comp_priv *comp_priv, struct rdma_mpt *mpt)
{
	...
	for (retry = 0; retry < RDMA_MAX_RETRY; retry++) {
		...
		/* 通过比较MPT State字段,确认芯片操作完成
		 * 回读状态正确则跳出,否则强制延迟后继续读取比较，直至循环结束 */
		check_mpt_entry.roce_mpt_ctx.dw2.value = be32_to_cpu(mpt_entry->roce_mpt_ctx.dw2.value);
	...
	pr_err("%s: RoCE mpt state read times(%d), mpt_index(0x%x), state_dw(0x%x)\n", __FUNCTION__, retry,
		mpt->mpt_index, mpt_entry->roce_mpt_ctx.dw2.value);
	return -RDMA_CMDQ_ERR;
}
...
int hmm_disable_roce_mpt(struct hmm_comp_priv *comp_priv, struct rdma_mpt *mpt, u16 channel)
{
	...
	ret = cqm_send_cmd_box(comp_priv->hwdev, HINIC3_MOD_ROCE, RDMA_ROCE_CMD_HW2SW_MPT, cqm_cmd_inbuf, NULL, NULL,
		RDMA_CMD_TIME_OUT_A, channel);
	if (ret != 0) {
		if (hinic3_get_heartbeat_status(comp_priv->hwdev) != PCIE_LINK_DOWN) {
...
int hmm_modify_roce_mpt(void *hwdev, u32 mpt_index, u32 new_key, u64 length, u64 iova, u16 channel)
{
	...
	ret = cqm_send_cmd_box(hwdev, HINIC3_MOD_ROCE, RDMA_ROCE_CMD_MODIFY_MPT, cqm_cmd_inbuf, NULL, NULL,
		RDMA_CMD_TIME_OUT_A, channel);
	if (ret != 0) {
		if (hinic3_get_heartbeat_status(hwdev) != PCIE_LINK_DOWN) {
