...
u32 hmm_buddy_alloc(struct hmm_buddy *buddy, u32 order)
{
	...
	for (cur_order = order; cur_order <= buddy->max_order; ++cur_order) {
		if (buddy->num_free[cur_order] != 0) {
			...
			first_index = (u32)find_first_bit(buddy->bits[cur_order], (unsigned long)cur_bit_num);
...
int hmm_buddy_init(struct hmm_buddy *buddy, u32 max_order)
{
	...
	/*lint +e708*/
	buddy->num_free = (unsigned int *)kcalloc((unsigned long)(buddy->max_order + 1UL), sizeof(int), GFP_KERNEL);
	if (buddy->num_free == NULL) {
		pr_err("%s: Alloc memory for buddy->num_free failed, ret(%d)\n", __FUNCTION__, -ENOMEM);
		return -ENOMEM;
	}
	buddy->bits = (unsigned long **)kcalloc((unsigned long)(buddy->max_order + 1UL), sizeof(long *), GFP_KERNEL);
	if (buddy->bits == NULL) {
	...
	for (i = 0; i <= buddy->max_order; i++) {
		...
		buddy->bits[i] = (unsigned long *)kcalloc((unsigned long)bit_num, sizeof(long), GFP_KERNEL | __GFP_NOWARN);
		if (buddy->bits[i] == NULL) {
			pr_err("%s: Kcalloc memory for buddy->bits[%d] failed, ret(%d)\n", __FUNCTION__, i, -ENOMEM);
			buddy->bits[i] = (unsigned long *)vzalloc((unsigned long)bit_num * sizeof(long));
			if (buddy->bits[i] == NULL) {
