 ...
 *
 * File Name	 : roce_qp_extension.c
 * Version	   : v1.1
 * Created	   : 2021/12/18
 * Last Modified : 2021/12/18
 * Description   : Define standard kernel QP realted function prototype.
 */
...
static int roce3_alloc_sqpc(struct roce3_device *rdev, struct roce3_qp *rqp, struct roce3_vbs_qp *vbs_rqp)
{
	...
	if (qpc_info == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE_VBS, ERR] %s: Failed to create qpc by cqm object, func_id(%d), qpn(%u)\n",
			__func__, rdev->glb_func_id, rqp->qpn + QPC_ROCE_VBS_QPC_OFFSET_FOR_SQPC);
		return -ENOMEM;
	}
	...
	if (qpc_info->xid != (rqp->qpn + QPC_ROCE_VBS_QPC_OFFSET_FOR_SQPC)) {
		dev_err(rdev->hwdev_hdl, "[ROCE_VBS, ERR] %s: Create qpc error, func_id(%d), expect qpn(%d), actual qpn(%d)\n",
			__func__, rdev->glb_func_id, rqp->qpn + QPC_ROCE_VBS_QPC_OFFSET_FOR_SQPC, qpc_info->xid);
...
long roce3_vbs_create_sqpc(struct roce3_device *rdev, void *buf)
{
	...
	vbs_rqp = (struct roce3_vbs_qp *)kzalloc(sizeof(*vbs_rqp), GFP_KERNEL);
	if (vbs_rqp == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE_VBS, ERR] %s: Failed to alloc vbs_rqp, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -ENOMEM;
	}

	pd = roce3_get_pd(rqp);
	ret = roce3_db_map_user(to_roce3_ucontext(pd->ibpd.uobject->context), cmd.ci_record_addr, &vbs_rqp->db);
	if (ret != 0) {
