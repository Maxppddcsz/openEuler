 ...
 *
 * File Name	 : rdma_comp_res.c
 * Version	   : v2.0
 * Created	   : 2021/3/10
 * Last Modified : 2021/12/23
 * Description   : implement the support function for management of bitmap resources
 */
...
static int rdma_check_map_param(const void *hwdev, const struct rdma_fmr *fmr, const u64 *page_list, int npages,
	u64 iova)
{
	u32 page_mask = 0;
...
int roce3_rdma_map_phys_fmr(void *hwdev, struct rdma_fmr *fmr, u64 *page_list, int npages, u64 iova, u32 service_type)
{
	...
	length = (u64)((unsigned)npages * (1UL << fmr->page_shift));
...
int roce3_rdma_unmap_fmr(void *hwdev, struct rdma_fmr *fmr, u32 service_type)
{
	...
	if (service_type == RDMA_SERVICE_TYPE_ROCE) {
		ret = hmm_modify_roce_mpt(hwdev, fmr->mr.mpt.mpt_index, new_key, 0ULL, 0ULL, HINIC3_CHANNEL_ROCE);
	} else {
...
static void roce3_rdma_rdmarc_order_set(struct rdma_rdmarc *rdmarc, u32 order, u32 ext_order, u32 log_rdmarc_per_seg)
{
	rdmarc->order = order;
