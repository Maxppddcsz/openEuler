 ...
 *
 * File Name	 : rdma_comp_gid.c
 * Version	   : v2.0
 * Created	   : 2021/3/10
 * Last Modified : 2021/12/23
 * Description   : implement the management of GID
 */
...
static int rdma_update_gid(struct rdma_comp_priv *comp_priv, struct rdma_gid_entry *new_gid_entry, int port,
	int gid_index)
{
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(comp_priv->hwdev, &cqm_cmd_inbuf, (u16)sizeof(struct rdma_gid_update_inbuf),
		NULL, 0);
	if (ret != 0) {
	...
	gid_update_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_GID_BITMASK); //lint !e778
	...
	pr_info("[ROCE] %s:gid_index:0x%x gid:0x%x %x %x %x %x %x %x %x\n", __func__, gid_update_inbuf->com.index,
		*((u32 *)(void *)new_gid_entry + 0), *((u32 *)(void *)new_gid_entry + 1),  // 0 1 is gid array idx
	...
	ret = cqm_send_cmd_box(comp_priv->hwdev, HINIC3_MOD_ROCE, RDMA_ROCE_CMD_UPDATE_GID, cqm_cmd_inbuf, NULL, NULL,
...
static int rdma_reset_gid(struct rdma_comp_priv *comp_priv, u32 port, u32 gid_num)
{
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(comp_priv->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_clear_gid_s), NULL, 0);
	if (ret != 0) {
		pr_err("[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, ret(%d)\n", __func__, ret);
		return -ENOMEM;
	}
	...
	gid_clear_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_GID_BITMASK); //lint !e778

	ret = cqm_send_cmd_box(comp_priv->hwdev, HINIC3_MOD_ROCE, RDMA_ROCE_CMD_CLEAR_GID, cqm_cmd_inbuf, NULL, NULL,
		ROCE3_RDMA_CMD_TIME_OUT_A, HINIC3_CHANNEL_ROCE);
	if (ret != 0) {
