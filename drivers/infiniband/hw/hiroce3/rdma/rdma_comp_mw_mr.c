 ...
 *
 * File Name	 : rdma_comp_mw_mr.c
 * Version	   : v2.0
 * Created	   : 2021/3/10
 * Last Modified : 2021/12/23
 * Description   : implement the management of MR/MW
 */
...
int roce3_rdma_enable_mw_mpt(void *hwdev, struct rdma_mw *mw, u32 service_type)
{
	...
	roce_cmd_mpt_sw2hw_s *mpt_sw2hw_inbuf = NULL;
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_cmd_mpt_sw2hw_s), NULL, 0);
	if (ret != 0) {
	...
	mpt_sw2hw_inbuf = (roce_cmd_mpt_sw2hw_s *)cqm_cmd_inbuf->buf;
	mpt_sw2hw_inbuf->com.index = cpu_to_be32(mw->mpt.mpt_index);
	mpt_sw2hw_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_MR_BITMASK); //lint !e778
...
int roce3_rdma_disable_mw_mpt(void *hwdev, struct rdma_mw *mw, u32 service_type)
{
	...
	if (mw->enabled == RDMA_MPT_EN_HW) {
		if (service_type == RDMA_SERVICE_TYPE_ROCE) {
			ret = hmm_disable_roce_mpt((struct hmm_comp_priv *)(void *)comp_priv, &mw->mpt, HINIC3_CHANNEL_ROCE);
		} else {
			pr_err("%s: service_type not support \n", __FUNCTION__);
			return -EINVAL;
		}
