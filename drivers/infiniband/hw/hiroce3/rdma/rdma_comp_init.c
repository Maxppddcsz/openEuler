 ...
 *
 * File Name	 : rdma_comp_init.c
 * Version	   : v2.0
 * Created	   : 2021/3/10
 * Last Modified : 2021/12/23
 * Description   : implement the initialization of RoCE bitmap resources
 */
...
static int rdma_init_gid_table(struct rdma_comp_priv *comp_priv)
{
	...
	gid_table = (struct rdma_gid_entry **)kzalloc(port_num * sizeof(struct rdma_gid_entry *), GFP_KERNEL);
	if (gid_table == NULL) {
		pr_err("%s: Alloc memory for gid_tbl failed, ret(%d), port_num(%d)\n", __FUNCTION__, -ENOMEM, port_num);
		return -ENOMEM;
	}
	...
	for (i = 0; i < port_num; i++) {
		gid_table[i] = (struct rdma_gid_entry *)kzalloc(gids_per_port * sizeof(struct rdma_gid_entry), GFP_KERNEL);
		if (gid_table[i] == NULL) {
			pr_err("%s: Alloc memory for gid_tbl[%d] failed, ret(%d), gids_per_ports(%d)\n", __FUNCTION__, i, -ENOMEM,
				gids_per_port);
			goto err_out;
		}
	}
...
static int rdma_init_rdmarc_table(struct rdma_comp_priv *comp_priv)
{
	...
	rdmarc_per_qp = comp_priv->rdma_cap.dev_rdma_cap.roce_own_cap.max_qp_dest_rdma + ROCE_MAX_RDMA_RC_EXTEND;
	...
	/*lint -e587*/
	rdmarc_pow_of_two = (u32)(HMM_EM_ROUNDUP_POW_OF_TWO((u32)(qp_num * rdmarc_per_qp)) & 0xffffffff);
	/*lint +e587*/
	ret = hmm_em_init_table(comp_priv->pdev, &comp_priv->rdmarc_em_table, rdmarc_size, rdmarc_pow_of_two, 0,
		RDMA_EM_MIN_ORDER);
	if (ret != 0) {
...
void roce3_rdma_cleanup_resource(void *hwdev)
{
	...
	pr_info("%s: Rdma cleanup resource successful", __FUNCTION__);

	return;
}

static int roce3_rdma_init_comp_priv(struct rdma_comp_priv *comp_priv, void *hwdev, struct rdma_service_cap *rdma_cap)
{
	int ret;
...
int roce3_rdma_init_resource(void *hwdev)
{
	...
	comp_priv = (struct rdma_comp_priv *)kzalloc(sizeof(struct rdma_comp_priv), GFP_KERNEL);
	if (comp_priv == NULL) {
