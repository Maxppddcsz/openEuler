 ...
 *
 * File Name	 : roce_main.c
 * Version	   : v2.0
 * Created	   : 2021/3/2
 * Last Modified : 2021/12/7
 * Description   : The definition of RoCE MAIN related functions.
 */
...
static int roce3_alloc_hw_resource(struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to init rsvd lkey, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR]: Failed to reset gid_table, func_id(%hu)\n", rdev->glb_func_id);
		goto err_gid_reset;
	}
...
static int roce3_board_info_get(struct roce3_device *rdev)
{
	...
	pr_info("[ROCE] Get board info success, board_type:0x%x, port_num:0x%x, pf_num:0x%x, vf_total_num:0x%x, "
		"work_mode:0x%x, "
		"service_mode:0x%x, speed:0x%x.\n",
		rdev->board_info.board_type, rdev->board_info.port_num, rdev->board_info.pf_num, rdev->board_info.vf_total_num,
		rdev->board_info.work_mode, rdev->board_info.service_mode, rdev->board_info.port_speed);
...
static void roce3_fix_ibdev_name(struct roce3_device *rdev)
{
	...
	if (roce3_bond_is_active(rdev)) {
		strcpy(rdev->ib_dev.name, "hrn3_bond_%d");
		return;
	}
#endif
	{
		strcpy(rdev->ib_dev.name, (rdev->is_vroce ? "efi_%d" : "hrn3_%d"));
	}
}
...
static int roce3_register_template(struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to register cqm_service, func(%hu)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
...
int roce3_init_dev_file(struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to init sysfs, func_id(%hu) ret(%d)\n", __func__,
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to init cdev, func_id(%hu)\n", __func__, rdev->glb_func_id);
...
static int roce3_alloc_ceq(struct roce3_device *rdev)
{
	...
	ret = hinic3_alloc_ceqs(rdev->hwdev, SERVICE_T_ROCE, rdev->ib_dev.num_comp_vectors, rdev->ceqn, &rdev->ceq_num);
	if (ret < 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqes, num_comp_vectors(%d), func_id(%hu)\n", __func__,
			rdev->ib_dev.num_comp_vectors, rdev->glb_func_id);
		return ret;
	}
...
static void roce3_init_hw_info(struct roce3_device *rdev)
{
	...
	dev_info(rdev->hwdev_hdl, "[ROCE , INFO] group id rc(%hhu), ud(%hhu), xrc(%hhu)",
...
static int roce3_init_dev_upper(struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to set func_tbl, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
...
static int roce3_init_dev_info(struct roce3_device *rdev)
{
	...
	ret = roce3_set_func_tbl_cpu_endian(rdev->hwdev, rdev->hw_info.cpu_endian, rdev->glb_func_id);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR]: Failed to set func_tbl cpu_endian, func_id(%hu)\n", rdev->glb_func_id);
		goto err_init_endianess;
	}

	ret = roce3_init_dev_ext(rdev);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR]: Failed to init extended service for func_id(%hu)\n", rdev->glb_func_id);
...
static int roce3_init_dev(struct roce3_device *rdev, char *uld_dev_name)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to init dev info, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc hw res, func_id(%hu)\n", __func__, rdev->glb_func_id);
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to reg ibdev, func_id(%hu)\n", __func__, rdev->glb_func_id);
		goto err_reg_dev;
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to reg netdev, func_id(%hu)\n", __func__, rdev->glb_func_id);
		goto err_memcpy_uld;
	}

	ret = roce3_init_dev_file(rdev);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to init sysfs, func_id(%hu)\n", __func__, rdev->glb_func_id);
		goto err_init_dev_file;
	}
	rdev->ib_active = true;
	dev_info(rdev->hwdev_hdl, "[ROCE] %s: RoCE add init all ok, func_id(%hu), dev_name(%s)\n", __func__,
...
static int roce3_add_rdev_init(struct roce3_device *rdev)
{
	...
	rdev->gid_dev =
		(struct net_device **)kzalloc(rdev->rdma_cap.max_gid_per_port * sizeof(struct net_device *), GFP_KERNEL);
	if (rdev->gid_dev == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc gid dev memory, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return -ENOMEM;
	}
	...
	rdev->is_vroce = false;
#ifdef ROCE_VROCE_EN
	ret = hinic3_is_slave_func(rdev->hwdev, &rdev->is_vroce);
	if (ret != 0) {
		pr_err("[ROCE] %s: Failed to get slave_func.\n", __func__);
	}
#endif
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR]%s: Failed to get roce cfg, func_id(%hu)\n", __func__, rdev->glb_func_id);
		goto err_init_cfg;
	}
...
static __be64 rdma_gen_node_guid(u8 *dev_mac)
{
	...
	guid[0] = mac_addr[0] ^ DEV_ADDR_FIRST_BYTE_VAL_MASK; // 0 is mac & guid array idx
...
static void roce3_add_init(struct roce3_device *rdev)
{
	...
	ib_dev->num_comp_vectors =
		(rdev->rdma_cap.num_comp_vectors <= MAX_CEQ_NEED) ? (int)rdev->rdma_cap.num_comp_vectors : MAX_CEQ_NEED;
...
static int roce3_rdev_init(struct roce3_device *rdev)
{
	...
	if (!hinic3_is_vm_slave_host(rdev->hwdev)) {
		if ((hinic3_func_type(rdev->hwdev) == TYPE_VF) && (g_ppf_stateful_init == false) && (g_vf_stateful_num == 0)) {
	...
	if (hinic3_is_vm_slave_host(rdev->hwdev) || (hinic3_func_type(rdev->hwdev) != TYPE_PPF) || !g_vf_stateful_num) {
...
static void roce3_stateful_unit(struct roce3_device *rdev)
{
	...
	if (!hinic3_is_vm_slave_host(rdev->hwdev)) {
		if (hinic3_func_type(rdev->hwdev) == TYPE_VF) {
			hinic3_stateful_deinit(rdev->hwdev);
			g_vf_stateful_num--;
			if ((g_vf_stateful_num == 0) && (g_ppf_stateful_init == false)) { // Delete the last VF when no PF is added
				ppf_hwdev = roce3_get_ppf_lld_dev(rdev);
				hinic3_stateful_deinit(ppf_hwdev);
			}
		} else {
...
{
	int offset = 0;
	hiroce_netlink_dev_s *adp_dev = NULL;

	offset = get_instance_of_func_id(rdev->glb_func_id);
	if (offset >= ROCE_MAX_FUNCTION) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to get offset , func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return -EINVAL;
	}
	...
	if (adp_dev->used_dev_num == 0 && adp_dev->netlink == NULL) {
		adp_dev->netlink = kzalloc(sizeof(netlink_dev_s), GFP_KERNEL);
		if (adp_dev->netlink == NULL) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR]%s, Fail to alloc netlink\n", __func__);
			mutex_unlock(&adp_dev->mutex_dev);
			return -EINVAL;
		}
	}
...
static int roce3_add_do_init(struct roce3_device *rdev, char *uld_dev_name)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to register cqm_service, func_id(%hu) ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc db or dwqe, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		goto err_alloc_db;
	}
	...
	if (ret < 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR]: Failed to alloc ceqs, func_id(%hu)\n", rdev->glb_func_id);
		goto err_alloc_ceq;
	}
...
static bool is_device_v100(const struct hinic3_lld_dev *lld_dev)
{
	...
	return (ssdid == HINIC3_DEV_SSID_2X25G) || (ssdid == HINIC3_DEV_SSID_4X25G) || (ssdid == HINIC3_DEV_SSID_2X100G);
}
#else
static bool pci_support_vroce(const struct hinic3_lld_dev *lld_dev)
{
	struct pci_dev *pdev = lld_dev->pdev;
	unsigned short ssdid = pdev->subsystem_device;

	if (hinic3_func_type(lld_dev->hwdev) == TYPE_VF) {
		if (ssdid == HINIC3_DEV_SDI_5_1_SSDID_VF)
			return true;
	} else { // PF or PPF
		if ((ssdid & HINIC3_DEV_SDI_V100_SSDID_MASK) == 0)
			return true;
	}
...
static int roce3_add_check(const struct hinic3_lld_dev *lld_dev)
{
...
#ifdef ROCE_VROCE_EN
	if (!pci_support_vroce(lld_dev)) {
		pr_err("[ROCE, ERR] Pcidev ssdid %x doesn't support vroce, please change driver.\n",
			lld_dev->pdev->subsystem_device);
		return -EFAULT;
	}
#endif
	...
	if (!enable_roce) {
		pr_warn("[ROCE] %s: %s RoCE dev is not enable, func: %hu\n", __func__,
...
static int roce3_add_device_to_list(struct hinic3_lld_dev *lld_dev)
{
	struct roce3_func_info *info = (struct roce3_func_info *)kzalloc(sizeof(struct roce3_func_info), GFP_ATOMIC);
	if (info == NULL) {
...
static void roce3_wait_probe(struct hinic3_lld_dev *lld_dev)
{
	...
	pr_info("[ROCE] %s func %hu wait finished\n", __func__, hinic3_global_func_id(lld_dev->hwdev));
	return;
out:
	pr_info("[ROCE] %s func %hu wait fail\n", __func__, hinic3_global_func_id(lld_dev->hwdev));
	remove_wait_queue(&g_roce_probe_queue, &wait_queue);
	set_current_state(TASK_RUNNING);
	return;
}
...
static int roce3_add(struct hinic3_lld_dev *lld_dev, void **uld_dev, char *uld_dev_name)
{
	...
	if (!hinic3_support_roce(lld_dev->hwdev, &rdma_cap)) {
		pr_err("[ROCE, ERR] %s: %s Not support RoCE, func: %hu\n", __func__, pci_name(lld_dev->pdev),
	...
	if (ret != 0) {
		pr_err("[ROCE, ERR] %s: Failed to add device to list, ret: %d, func: %hu\n", __func__,
			ret, hinic3_global_func_id(lld_dev->hwdev));
		return ret;
	}
	...
	if (rdev == NULL) {
		...
		pr_err("[ROCE, ERR] %s: Failed to alloc rdev, func: %hu\n", __func__, hinic3_global_func_id(lld_dev->hwdev));
		return -EINVAL;
	}
...
static void roce3_do_remove(struct roce3_device *rdev, u16 glb_func_id, const char *dev_name)
{
	...
	ib_unregister_device(&rdev->ib_dev);
	pr_info("[ROCE] %s: Unregister IB device ok, func_id(%hu), name(%s), pci(%s)\n", __func__, glb_func_id, dev_name,
		pci_name(rdev->pdev));
	...
	pr_info("[ROCE] %s: Function level resource clear ok, func_id(%hu), name(%s), pci(%s)\n", __func__, glb_func_id,
		dev_name, pci_name(rdev->pdev));

	roce3_do_cache_out(rdev->hwdev, ROCE_CL_TYPE_CQC_SRQC, rdev->glb_func_id);

	roce3_rdev_unit(rdev);
	pr_info("[ROCE] %s: RoCE rdev uninit ok, func_id(%hu), name(%s)\n", __func__, glb_func_id, dev_name);
...
static void roce3_remove(struct hinic3_lld_dev *lld_dev, void *uld_dev)
{
	...
	dev_info(rdev->hwdev_hdl, "[ROCE] %s: RoCE remove start, func_id(%hu), name(%s), pci(%s)\n", __func__,
		rdev->glb_func_id, dev_name, pci_name(rdev->pdev));

	roce3_do_remove(rdev, glb_func_id, dev_name);

	pr_info("[ROCE] %s: RoCE remove end, func_id(%hu), name(%s)\n", __func__, glb_func_id, dev_name);
	return;
}
										 ...
										 const struct hinic3_event_info *event)
{
	...
	case EVENT_NIC_BOND_UP:
		if (!roce3_need_proc_bond_event(rdev->hwdev)) {
			dev_info(rdev->hwdev_hdl, "[ROCE, WARN] %s: RoCE don't need proc bond event\n", __func__);
			return -1;
		}
		if (test_and_set_bit(ROCE3_PORT_EVENT, &rdev->status) != 0) {
			return -1;
		}
		dev_info(rdev->hwdev_hdl, "[ROCE, WARN] %s: RoCE report NIC BOND_UP, func_id(%u)\n", __func__,
			rdev->glb_func_id);
	...
	case EVENT_NIC_BOND_DOWN:
		...
		dev_info(rdev->hwdev_hdl, "[ROCE, WARN] %s: RoCE report NIC BOND_DOWN, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return IB_EVENT_PORT_ERR;

	default:
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Nic event unsupported, func_id(%hu), event(%hu)\n", __func__,
							   ...
							   const struct hinic3_event_info *event)
{
	...
	case EVENT_NIC_LINK_UP:
		...
		dev_info(rdev->hwdev_hdl, "[ROCE, WARN] %s: RoCE report NIC LINK_UP, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return IB_EVENT_PORT_ACTIVE;

	case EVENT_NIC_LINK_DOWN:
		if (!roce3_need_proc_link_event(rdev->hwdev)) {
			dev_info(rdev->hwdev_hdl, "[ROCE, WARN] %s: RoCE don't need proc link event\n", __func__);
			return -1;
		}
		...
		dev_info(rdev->hwdev_hdl, "[ROCE, WARN] %s: RoCE report NIC LINK_DOWN, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return IB_EVENT_PORT_ERR;
	...
	case EVENT_NIC_DCB_STATE_CHANGE:
		dev_info(rdev->hwdev_hdl, "[ROCE]%s: DCB state change no longer requires RoCE involvement, func_id(%hu)\n",
			__func__, rdev->glb_func_id);
		return -1;
...
static int roce3_set_ib_event(struct roce3_device *rdev, const struct hinic3_event_info *event)
{
	...
	default:
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: func_id(%hu) event from svc(%hu) is not supported, event(%hu)\n",
...
typedef int (*roce3_adm_func_t)(struct roce3_device *rdev, const void *buf_in, u32 in_size, void *buf_out,
	u32 *out_size);
...
static int roce3_adm(void *uld_dev, u32 cmd, const void *buf_in, u32 in_size, void *buf_out, u32 *out_size)
{
...
{
	if ((scence_id == SCENES_ID_STORAGE_ROCEAA_2x100) || (scence_id == SCENES_ID_STORAGE_ROCEAA_4x25)) {
		return true;
	}
