 ...
 *
 * File Name	 : roce_qp_destroy.c
 * Version	   : v2.0
 * Created	   : 2021/3/12
 * Last Modified : 2021/12/20
 * Description   : Define standard kernel RoCE QP destroy realted functions.
 */
	...
	while ((int) --prod_index - (int)cq->cons_index >= 0) {
		...
		if (cqe->dw0.bs.qpn == qpn) {
			...
			dest = (struct roce_cqe *)roce3_get_cqe(cq,
				(unsigned int)((int)prod_index + nfreed) & (unsigned int)cq->ibcq.cqe);
...
static void roce3_clean_qp_kernel(struct roce3_device *rdev, struct roce3_qp *rqp)
{
	...
	roce3_cq_clean_process(recv_cq, rqp->qpn, rqp->ibqp.srq ? to_roce3_srq(rqp->ibqp.srq) : NULL);
...
static int roce3_qp_destroy(struct roce3_qp *rqp, struct roce3_device *rdev)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: QP(0x%06x) cache invalid, func_id(%d)\n", __func__, rqp->qpn,
			rdev->glb_func_id);
		return ret;
	}
