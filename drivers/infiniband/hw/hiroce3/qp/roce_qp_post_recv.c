 ...
 *
 * File Name	 : roce_qp_post_recv.c
 * Version	   : v2.0
 * Created	   : 2021/3/12
 * Last Modified : 2021/12/20
 * Description   : Define standard kernel RoCE QP Post rq/srq receive wqe functions.
 */
...
{
	return roce3_get_wqe(rqp, rqp->rq.offset + (n << (unsigned)rqp->rq.wqe_shift));
}
	...
	const struct ib_recv_wr **bad_wr, const struct ib_recv_wr *wr)
{
	if (ROCE_UNLIKELY((qp->qp_type == IB_QPT_XRC_INI) || (qp->qp_type == IB_QPT_XRC_TGT))) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Can't post WQE when TGT XRC QP, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		*bad_wr = wr;
		return -EINVAL;
	}
	...
	if (ROCE_UNLIKELY(qp->qp_state == IB_QPS_RESET)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Can't post WQE when QP is RST state, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		*bad_wr = wr;
		return -EINVAL;
	}
...
static int roce3_post_recv_check_wr(struct roce3_qp *qp, const struct ib_recv_wr **bad_wr, const struct ib_recv_wr *wr,
	u32 wr_num)
{
	...
	const struct ib_recv_wr *wr, int i)
{
	...
	if (ROCE_UNLIKELY(wr->sg_list[i].length > rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz - 1)) {
		*bad_wr = wr;
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Sge data len is over range, sg_list(%d), length(0x%x), \
		max_msg_sz(0x%x), func_id(%d)\n",
			__func__, i, wr->sg_list[i].length, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz, rdev->glb_func_id);
		return -EINVAL;
	}
	...
	if (ROCE_UNLIKELY(data_len > rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz)) {
		*bad_wr = wr;
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Data_len is over range, data_len(%d), max_msg_sz(%d), func_id(%d)\n",
			__func__, data_len, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz, rdev->glb_func_id);
		return -EINVAL;
	}
	...
	const struct ib_recv_wr **bad_wr, struct roce3_qp *qp, u32 index)
{
	...
	if (wr->num_sge != 0) {
		dseg--;
		dseg->key = cpu_to_be32((u32)(wr->sg_list[wr->num_sge - 1].lkey | ROCE_WQE_NEXT_SGE_INVALID));
	} else {
