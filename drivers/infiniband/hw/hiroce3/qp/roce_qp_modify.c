 ...
 *
 * File Name	 : roce_qp_modify.c
 * Version	   : v2.0
 * Created	   : 2021/3/12
 * Last Modified : 2021/12/20
 * Description   : Define standard kernel RoCE QP realted functions.
 */
	...
	int func_id, int times)
{
	...
	if (cost >= ROCE_CMDQ_TIMEOUT) {
		dev_warn_ratelimited(rdev->hwdev_hdl, "[ROCE, WARN] %s:destroy qp exec too long, func_id(%d), cost(%llu),\
...
#define ROCE_SQ_WQE_SIGN_CHECKBITS_123 (ROCE_SQ_WQE_SIGN_CHECKBITS_3 + ROCE_SQ_WQE_SIGN_CHECKBITS_12)
...
/* ****************************************************************************
 Prototype	: roce3_caculate_sign_bit
 Description  : roce3_caculate_signature
 Input		: u32 *check_bit
				u32 sign_width
				u32 sign_split
				u32 *sign_bit
...
**************************************************************************** */
static void roce3_caculate_sign_bit(u32 *check_bit, u32 sign_width, u32 sign_split, u32 *sign_bit)
{
	u32 i = 0;
	u32 j = 0;
...
/* ****************************************************************************
 Prototype	: roce3_caculate_signature
 Description  : roce3_caculate_signature
 Input		: u32 *sign_bit
				u32 sign_width
 Output	   : None
...
**************************************************************************** */
static u32 roce3_caculate_signature(u32 *sign_bit, u32 sign_width)
{
	u32 i = 0;
	u32 signature = 0;
...
static u32 roce3_gen_sq_rq_gpa_sign(u64 sq_rq_l0mtt_gpa)
{
	...
	roce3_caculate_sign_bit(check_bit, ROCE_QP_GPA_SIGN_WIDTH, ROCE_QP_GPA_SIGN_SPLITNUM, sign_bit);

	signature = roce3_caculate_signature(sign_bit, ROCE_QP_GPA_SIGN_WIDTH);
...
static u32 roce3_gen_qp_qpcc_sign(u32 qpn)
{
	...
	roce3_caculate_sign_bit(check_bit, ROCE_QP_QPCC_SIGN_WIDTH, ROCE_QP_QPCC_SIGN_SPLITNUM, sign_bit);

	signature = roce3_caculate_signature(sign_bit, ROCE_QP_QPCC_SIGN_WIDTH);
...
static u32 roce3_gen_qp_rcc_sign(u32 qpn)
{
	...
	roce3_caculate_sign_bit(check_bit, ROCE_QP_SUB_SIGN_WIDTH, ROCE_QP_SUB_SIGN_SPLITNUM, sign_bit);

	signature = roce3_caculate_signature(sign_bit, ROCE_QP_SUB_SIGN_WIDTH);
...
static u32 roce3_gen_sq_wqe_sign(u32 qpn, u32 wqe_ci, u32 wqe_header)
{
	...
	roce3_caculate_sign_bit(check_bit, ROCE_SQ_WQE_SIGN_WIDTH, ROCE_SQ_WQE_SIGN_SPLITNUM, sign_bit);

	signature = roce3_caculate_signature(sign_bit, ROCE_SQ_WQE_SIGN_WIDTH);
	...
	roce_verbs_qpc_attr_path_s *path_info)
{
	...
	path_info->dw0.bs.dmac_h16 = (dmac[0] << ROCE_RAH_DMAC_H16_SHIFT) | dmac[1]; // shift dmac[0] left by 8 bits
...
**************************************************************************** */
static void roce3_set_access_flags(struct roce3_qp *rqp, roce_verbs_qp_attr_s *qp_attr, struct ib_qp_attr *attr,
	int attr_mask)
{
...
{
	if (((qp->qp_type == IB_QPT_GSI) && (qp->qpn == ROCE_QP_SMI_QP_NUM)) || (qp->qpn == ROCE_QP_GSI_QP_NUM)) {
		return 1;
	}
...
**************************************************************************** */
static void qpc_seg_to_be(roce_verbs_qp_attr_s *be_ctx, roce_verbs_qp_attr_s *le_ctx, struct roce3_qp *rqp)
{
	/* DRV Seg */
...
**************************************************************************** */
void roce3_qpc_to_be(roce_verbs_qp_attr_s *qp_attr, struct roce3_qp *rqp, u32 *be_ctx)
{
	u32 *ctx = NULL;
	u32 *ctx1 = NULL;
	u32 i = 0;
...
**************************************************************************** */
static int roce3_alloc_opt_rdmarc(struct roce3_qp *rqp, const struct ib_qp_attr *attr, int attr_mask)
{
	int ret = 0;
	...
	/*lint -e587*/
	response_depth = (attr->max_dest_rd_atomic == 0) ? 1 : (u32)ROCE_ROUNDUP_POW_OF_TWO((u32)attr->max_dest_rd_atomic);
	/*lint +e587*/
	if (rqp->rsp_depth != response_depth) {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc rdma rdmarc, func_id(%d)\n", __func__,
				rdev->glb_func_id);
			return ret;
		}
...
**************************************************************************** */
static void roce3_set_opt_rdmarc(struct roce3_device *rdev, struct roce3_qp *rqp, roce_verbs_qp_attr_s *qp_attr)
{
	qp_attr->chip_seg.dw10.bs.rc_page_gpa_h = (rqp->rdmarc.dma_addr >> 40) & 0xffffff; /* high:24bit, 64-24= 40 */
...
static void roce3_set_opt_mtu(roce_verbs_qp_attr_s *qp_attr, const struct ib_qp_attr *attr)
{
	u32 pmtu = (u32)attr->path_mtu & 0x7;

	qp_attr->com_info.dw3.bs.pmtu = pmtu;							  /* 1,2,3,4,5 */
	qp_attr->com_info.dw5.bs.mtu_code = ROCE_QPC_PMTU_TRANSLATE(pmtu); /* 0,1,3,7,15 */
...
**************************************************************************** */
static void roce3_set_opt_sra(roce_verbs_qp_attr_s *qp_attr, struct ib_qp_attr *attr)
{
	u8 initiator_depth;

	/*lint -e587*/
	initiator_depth = (attr->max_rd_atomic == 0) ? 1 : (u8)ROCE_ROUNDUP_POW_OF_TWO((u32)attr->max_rd_atomic);
	/*lint +e587*/
	qp_attr->com_info.dw1.bs.sra_max = ((u32)ROCE_FLS(initiator_depth - 1)) & 0x7; /* sra_max:3bit */
}
...
u8 roce3_get_db_cos_from_vlan_pri(struct roce3_device *rdev, u8 vlan_pri)
{
	...
	if (ret != 0) {
		printk("roce3_get_db_cos_from_vlan_pri, ret:%d vlan_pri:%u\n", ret, vlan_pri);
		db_cos = 0;
	}
...
static void roce3_set_opt_attr_av(struct roce3_qp *rqp, struct roce3_device *rdev, struct ib_qp_attr *attr,
	roce_verbs_qp_attr_s *qp_attr, u32 *optpar)
{
	u8 cos;
	roce3_get_cos_inbuf_s inbuf = { 0 };
...
static void roce3_set_opt_field_attr(struct roce3_qp *rqp, const struct ib_qp_attr *attr, int attr_mask,
	roce_verbs_qp_attr_s *qp_attr, u32 *optpar)
{
	/* RTR2RTS/SQD2SQD RETRY_CNT */
	if ((((u32)attr_mask) & IB_QP_RETRY_CNT) != 0) {
		qp_attr->com_info.dw1.bs.to_retry_limit = attr->retry_cnt & 0x7; /* to_retry_limit:3 */
		*optpar |= ROCE_QP_OPTPAR_RETRY_COUNT;
	}

	/* RTR2RTS/SQD2SQD RNR_RETRY */
	if ((((u32)attr_mask) & IB_QP_RNR_RETRY) != 0) {
		qp_attr->com_info.dw1.bs.rnr_retry_limit = attr->rnr_retry & 0x7; /* rnr_retry_limit:3bits */
		*optpar |= ROCE_QP_OPTPAR_RNR_RETRY;
	}

	/* INIT2RTR/SQD2SQD/RTR2RTS/RTS2RTS MIN_RNR_TIMER */
	if ((((u32)attr_mask) & IB_QP_MIN_RNR_TIMER) != 0) {
		qp_attr->com_info.dw2.bs.min_rnr_nak = attr->min_rnr_timer & 0x1f; /* min_rnr_nak:5bits */
		*optpar |= ROCE_QP_OPTPAR_RNR_TIMEOUT;
	}

	if ((((u32)attr_mask) & IB_QP_SQ_PSN) != 0) {
		qp_attr->com_info.dw8.bs.next_send_psn = attr->sq_psn & 0xffffff; /* next_rcv_psn:24bit */
	}

	if ((((u32)attr_mask) & IB_QP_RQ_PSN) != 0) {
		qp_attr->com_info.dw9.bs.next_rcv_psn = attr->rq_psn & 0xffffff; /* next_rcv_psn:24bit */
	}

	if ((((u32)attr_mask) & IB_QP_DEST_QPN) != 0) {
		qp_attr->com_info.dw0.bs.dest_qp = attr->dest_qp_num & 0xffffff; /* dest_qp:24bit */
	}
...
static s32 roce3_set_db_path_mtu(const struct ib_qp_attr *attr, struct roce3_qp *rqp, struct roce3_device *rdev)
{
	switch (attr->path_mtu) {
#ifdef ROCE_VROCE_EN
	case IB_MTU_256:
		rqp->db_path_mtu = BIT_LENGTH_IB_MTU_256;
		break;
	case IB_MTU_512:
		rqp->db_path_mtu = BIT_LENGTH_IB_MTU_512;
		break;
#else
	case IB_MTU_256:
	case IB_MTU_512:
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s:Only support mtu larger than 1K.func_id(%d) mtu:%u\n", __func__,
...
static void roce3_set_qp_srq_attr(struct roce3_qp *rqp, roce_verbs_qp_attr_s *qp_attr, struct roce3_pd *pd,
	const struct roce3_cq *recv_cq)
{
	struct roce3_srq *rsrq = to_roce3_srq(rqp->ibqp.srq);
	struct roce_srq_context temp;

	if (rsrq->container_flag != 0) {
		struct roce_srq_context *srqc = (struct roce_srq_context *)((void *)rsrq->cqm_srq->q_ctx_vaddr);
		...
		qp_attr->chip_seg.dw9.bs.container_sz = MAX_SUPPORT_CONTAINER_MODE - rsrq->container_mode;
		...
		if (rsrq->rqe_cnt_th > 0) {
			...
			qp_attr->chip_seg.dw8.bs.srq_rqecnt_th = rsrq->rqe_cnt_th & 0xf; /* srq_rqecnt_th:4bits */
		}
...
static void roce3_set_qp_rst2init_attr(struct roce3_qp *rqp, roce_verbs_qp_attr_s *qp_attr, struct roce3_pd *pd,
	struct roce3_cq *recv_cq)
{
	qp_attr->chip_seg.dw4.bs.rq_base_ci = (u32)(ROCE_ILOG2(rqp->rq.offset >> rqp->rq.wqe_shift));
	...
	if (rqp->ibqp.srq) {
		...
		qp_attr->chip_seg.dw15.bs.rq_size = (rqp->rq.wqebb_cnt > 0) ? (((u32)ROCE_ILOG2(rqp->rq.wqebb_cnt)) & 0x1f) : 0;
		...
		qp_attr->chip_seg.dw8.bs.rq_wqebb_size = (u32)(rqp->rq.wqe_shift - BYTES_TO_16B_SHIFT) & 0x7;
...
static void roce3_qp_rst2init_set_qpcc(roce_verbs_qp_attr_s *qp_attr, const struct roce3_qp *rqp,
	const struct roce3_device *rdev)
{
	qp_attr->chip_seg.dw0.sq_rq_l0mtt_gpa = rqp->mtt.mtt_paddr;
	qp_attr->chip_seg.dw2.sq_rq_pi_record_gpa_at_hop_num = rqp->db.dma & (~0x3ULL);	/* bits:63-2 */
	qp_attr->chip_seg.dw2.sq_rq_pi_record_gpa_at_hop_num |= rqp->mtt.mtt_layers & 0x3; /* bits:1-0 */
	if (rqp->qp_type == IB_QPT_XRC_TGT) {
		qp_attr->chip_seg.dw2.sq_rq_pi_record_gpa_at_hop_num = rqp->qpc_info->paddr & (~0x3ULL); /* bits:63-2 */
	}

	qp_attr->chip_seg.dw4.bs.sq_rq_mtt_page_size = rqp->mtt.mtt_page_shift - PAGE_SHIFT_4K;
	/* rc_entry_size:2bits */
	qp_attr->chip_seg.dw10.bs.rc_entry_size =
		((u32)ROCE_ILOG2(rdev->rdma_cap.dev_rdma_cap.roce_own_cap.rdmarc_entry_sz >> BYTES_TO_16B_SHIFT) & 0x3);
	qp_attr->chip_seg.dw14.bs.qp_rkey_en = 1;
	qp_attr->chip_seg.dw10.bs.rc_entry_prefetch_maxnum = 0;
}

static void roce3_qp_rst2init_set_sqc_sqac(roce_verbs_qp_attr_s *qp_attr, const struct roce3_qp *rqp,
	const struct roce3_pd *pd, const struct roce3_cq *send_cq)
{
#ifdef ROCE_COMPUTE
	u8 sq_pi_on_chip = (u8)((rqp->qpn <= ROCE_MAX_PI_ON_CHIP_QPN) && (rqp->qp_type != IB_QPT_UD));
	...
	qp_attr->chip_seg.dw16.bs.sq_page_size = ((u32)(rqp->buf.page_shift - PAGE_SHIFT_4K)) & ROCE_SQ_PAGE_SIZE_MASK;
	...
	if (rqp->sq.wqebb_cnt != 0) {
		qp_attr->chip_seg.dw5.bs.sq_size = (u32)(ROCE_ILOG2((u32)rqp->sq.wqebb_cnt)) & ROCE_SQ_SIZE_MASK;
	}
...
static void roce3_qp_rst2init_set_drv(roce_verbs_qp_attr_s *qp_attr, struct roce3_qp *rqp, struct roce3_device *rdev)
{
	u16 host_oqid = 0;
	u8 host_id = 0;
	...
	qp_attr->com_info.dw0.bs.service_type = (u32)to_roce3_qp_type((enum ib_qp_type)rqp->qp_type) & 0x7;
	if (rdev->is_vroce) {
...
static void roce3_qp_rst2init_set_common(roce_verbs_qp_attr_s *qp_attr, const struct roce3_pd *pd)
{
	qp_attr->com_info.dw12.bs.ccf_app_id = ROCE_CC_DISABLE; // STUB
	qp_attr->chip_seg.dw18.bs.ud_pd = pd->pdn & 0x3ffff;	/* pd:18bits */
}

static void roce3_qp_rst2init_set_dcqcn(roce_verbs_qp_attr_s *qp_attr)
{
	/* ecn ver */
...
static void roce3_qp_rst2init_set_fake_vf(const struct roce3_device *rdev, const struct roce3_pd *pd,
	roce_verbs_qp_attr_s *qp_attr)
{
	u32 fake_vf_id = 0;
	...
	if (pd->ibpd.uobject) {
		...
		printk("[ROCE] func_id:%u The fake_id:%u ,start:%u end:%u  page_bit:%u roce3_uctx->db_dma_addr:%llu \n",
			rdev->glb_func_id, qp_attr->com_info.dw11.bs.vf_id, rdev->cfg_info.pf_start_bit, rdev->cfg_info.pf_end_bit,
			rdev->cfg_info.page_bit, roce3_uctx->db_dma_addr);
		qp_attr->com_info.dw11.bs.fake = 1;
	} else {
		printk("[ROCE] kernel not support fake_vf currently. func:%u \n", rdev->glb_func_id);
	}
}

static void roce3_qp_rst2init_set_xrc_attr(const struct roce3_qp *rqp, const struct ib_qp_attr *attr,
	roce_verbs_qp_attr_s *qp_attr, const struct roce3_device *rdev, const struct roce3_cq *recv_cq)
{
	qp_attr->com_info.dw2.bs.xrc_srq_en = 1;

	if ((((u32)attr->qp_access_flags & XRC_CONTAINER_FLAG) != 0) || (rdev->cfg_info.srq_container_en != 0)) {
		qp_attr->chip_seg.dw9.bs.container_en = 1;
		qp_attr->com_info.dw3.bs.srq_container = 1;
...
**************************************************************************** */
void roce3_qp_rst2init(struct roce3_qp *rqp, const struct ib_qp_attr *attr, roce_verbs_qp_attr_s *qp_attr)
{
...
**************************************************************************** */
static void roce3_qp_init2rtr(struct roce3_qp *rqp, const struct ib_qp_attr *attr, roce_verbs_qp_attr_s *qp_attr)
{
	if (rqp->ibqp.srq || (rqp->qp_type == IB_QPT_XRC_TGT) || (rqp->qp_type == IB_QPT_XRC_INI) ||
		(rqp->qp_type == IB_QPT_UD) || (rqp->qp_type == IB_QPT_UC) || (rqp->qp_type == IB_QPT_GSI)) {
...
**************************************************************************** */
static void roce3_qp_rtr2rts(struct roce3_qp *rqp, const struct ib_qp_attr *attr, roce_verbs_qp_attr_s *qp_attr) {}

static void roce3_qp_2err(struct roce3_qp *rqp, const struct ib_qp_attr *attr, roce_verbs_qp_attr_s *qp_attr) {}
	...
	roce_verbs_qp_attr_s *qp_attr)
{
	...
	cur_state = (enum ib_qp_state)(((((u32)attr_mask) & IB_QP_CUR_STATE) != 0) ? attr->cur_qp_state : rqp->qp_state);
...
int roce3_qp_modify_2rst_cmd(struct roce3_device *rdev, u32 qpn)
{
	...
	ret =
		roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_uni_cmd_qp_modify2rst_s), NULL, 0);
	if (ret != 0) {
	...
	qp_moidfy2rst_inbuf = (roce_uni_cmd_qp_modify2rst_s *)cqm_cmd_inbuf->buf;
	qp_moidfy2rst_inbuf->com.index = cpu_to_be32(qpn);
	if (!roce3_is_roceaa(rdev->cfg_info.scence_id)) {
		qp_moidfy2rst_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_QP_BITMASK); //lint !e778
	} else {
		qp_moidfy2rst_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_NOFAA_BITMASK); //lint !e778
	}

	ret = roce3_send_qp_lb_cmd(qpn, rdev, ROCE_CMD_2RST_QP, cqm_cmd_inbuf, NULL, ROCE_CMD_TIME_CLASS_A);
	if (ret != 0) {
		...
		if (roce3_hca_is_present(rdev) != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(2RST_QP), qpn(0x%x), func_id(%hu)\n", __func__, qpn,
				rdev->glb_func_id);
...
static int roce3_qp_check_rdmarc(struct roce3_device *rdev, struct roce3_qp *rqp, int *cost)
{
	...
	if (rc_num != rc_check_index) {
		dev_err(rdev->hwdev_hdl,
			"[ROCE, ERR] %s: Failed to read rdmarc field back after try %d times, func_id(%d) rc_check_index:%u "
			"rc_num:%u\n",
			__func__, ((rdev->try_times - times) + 1), rdev->glb_func_id, rc_check_index, rc_num);
...
static int roce3_qp_check_state(struct roce3_device *rdev, const struct roce3_qp *rqp, const u32 *wb_va, int *cost)
{
	int times = 0;
	...
	if (read_back_flag == 0) {
		dev_err(rdev->hwdev_hdl,
			"[ROCE, ERR] %s: Failed to read QP field back after try %d times,qpn(0x%x), "
			"wb_data(0x%x), func_id(%d)\n",
			__func__, ((rdev->try_times - times) + 1), rqp->qpn, *(u32 *)wb_va, rdev->glb_func_id);
		*cost = (rdev->try_times - times) + 1;
		return -1;
	}
	*cost = (rdev->try_times - times) + 1;
...
static void roce3_qp_fill_cacheout_inbuf(struct roce3_device *rdev, struct roce3_qp *rqp, dma_addr_t wb_dma,
	cqm_cmd_buf_s *cqm_cmd_inbuf)
{
	...
	qp_cacheout_inbuf = (roce_uni_cmd_qp_cache_invalid_s *)(cqm_cmd_inbuf)->buf;
	rdma_cap = &rdev->rdma_cap;

	if (!roce3_is_roceaa(rdev->cfg_info.scence_id)) {
		qp_cacheout_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_QP_BITMASK); //lint !e778
	} else {
		qp_cacheout_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_NOFAA_BITMASK); //lint !e778
	}
...
static int roce3_qp_send_cache_out_cmd(struct roce3_device *rdev, struct roce3_qp *rqp, void *wb_va, dma_addr_t wb_dma,
	int *cost)
{
	unsigned long end;
	int times = 0;
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_uni_cmd_qp_cache_invalid_s), NULL,
		0);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
	...
	while (time_before(jiffies, end)) {
		times++;
		ret =
			roce3_send_qp_lb_cmd(rqp->qpn, rdev, ROCE_CMD_MISC_CACHE_INVLD, cqm_cmd_inbuf, NULL, ROCE_CMD_TIME_CLASS_C);
...
int roce3_qp_cache_out_cmd(struct roce3_device *rdev, struct roce3_qp *rqp)
{
	...
	if (wb_va == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc wb_va, func_id(%d)\n", __func__, rdev->glb_func_id);
		return -ENOMEM;
	}
	...
	if (rqp->rsp_depth > 0) {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to check qp rdmarc, func_id(%d)\n", __func__,
				rdev->glb_func_id);
			goto err;
		}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to check qp state, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		goto err;
	}
...
static u8 roce3_get_qp_cmdq_cmd(struct roce3_device *rdev, struct roce3_qp *rqp, struct ib_qp_attr *attr, int attr_mask)
{
...
static int roce3_qp_rts2sqd_cmd(struct roce3_device *rdev, struct roce3_qp *rqp, const struct ib_qp_attr *attr,
	int attr_mask)
{
	int ret = 0;
	int sqd_event = 0;
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_uni_cmd_qp_modify_rts2sqd_s),
		NULL, 0);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc cqm_cmd_inoutbuf, func_id(%d), ret(%d)\n", __func__,
			rdev->glb_func_id, ret);
	...
	sqd_event = ((((u32)attr_mask) & IB_QP_EN_SQD_ASYNC_NOTIFY) != 0) && (attr->en_sqd_async_notify != 0);
	qp_rts2sqd_inbuf = (roce_uni_cmd_qp_modify_rts2sqd_s *)cqm_cmd_inbuf->buf;
	qp_rts2sqd_inbuf->com.index = cpu_to_be32(rqp->qpn);
	qp_rts2sqd_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_QP_BITMASK); //lint !e778
	qp_rts2sqd_inbuf->sqd_event_en = cpu_to_be32((u32)(sqd_event != 0));

	ret = roce3_send_qp_lb_cmd(rqp->qpn, rdev, ROCE_CMD_RTS2SQD_QP, cqm_cmd_inbuf, NULL, ROCE_CMD_TIME_CLASS_A);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to send RTS2SQD_QP cmd, func_id(%d), ret(0x%x)\n", __func__,
			rdev->glb_func_id, ret);
		roce3_cqm_cmd_free_inoutbuf(rdev->hwdev, cqm_cmd_inbuf, NULL);
		return -1;
	}
...
static void roce3_qp_fill_modify_inbuf(cqm_cmd_buf_s *cqm_cmd_inbuf, struct roce3_qp *rqp,
	roce_verbs_qp_attr_s *qp_attr, u32 optpar)
{
	...
	qp_modify_inbuf->com.dw0.bs.cmd_bitmask = cpu_to_be16(VERBS_CMD_TYPE_QP_BITMASK); //lint !e778
...
{
	dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to send MODIFY_QP command, func_id(%d), ret(%d)\n", __func__,
		rdev->glb_func_id, ret);

	if (roce3_hca_is_present(rdev) != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE] %s: HCA is present(MODIFY_QP), qpn(0x%x), func_id(%hu)\n", __func__, rqp->qpn,
			rdev->glb_func_id);
...
static int roce3_qp_modify_cmd(struct roce3_device *rdev, struct roce3_qp *rqp, u8 cmd, roce_verbs_qp_attr_s *qp_attr,
	u32 optpar)
{
	int ret = 0;
	cqm_cmd_buf_s *cqm_cmd_inbuf = NULL;
	...
	ret = roce3_cqm_cmd_zalloc_inoutbuf(rdev->hwdev, &cqm_cmd_inbuf, (u16)sizeof(roce_uni_cmd_modify_qpc_s),
	...
	ret = roce3_send_qp_lb_cmd(rqp->qpn, rdev, cmd, cqm_cmd_inbuf, cqm_cmd_outbuf, ROCE_CMD_TIME_CLASS_A);
	if (ret != 0) {
...
static int roce3_qp_modify_2rst(struct roce3_device *rdev, struct roce3_qp *rqp)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to modify QP(0x%06x) to RESET, func_id(%d)\n", __func__,
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: QP(0x%06x) cache invalid failed, func_id(%d)\n", __func__, rqp->qpn,
			rdev->glb_func_id);
		return ret;
	}
	...
	if (rqp->ibqp.uobject == NULL) {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Clean kernel QP(0x%06x)  failed, func_id(%d)\n", __func__,
				rqp->qpn, rdev->glb_func_id);
			return ret;
		}
	}
...
static int roce3_qp_modify(struct roce3_device *rdev, struct roce3_qp *rqp, struct ib_qp_attr *attr, int attr_mask)
{
	int ret = 0;
	u32 optpar = 0;
	roce_verbs_qp_attr_s qp_attr;
	...
	if ((((u32)attr_mask) & IB_QP_MAX_DEST_RD_ATOMIC) != 0) {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc rdmarc, func(%d)\n", __func__, rdev->glb_func_id);
			return ret;
		}
	}
	...
	if (cmd == 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Invalid cmd, func_id(%d), qpn(%d), cmd(%d)\n", __func__,
...
static int roce3_qp_get_path(struct roce3_device *rdev, struct roce3_qp *rqp, struct rdma_ah_attr *ah_attr)
{
	return 0;
}

static int roce3_qp_modify_check_rd_and_mtu(const struct roce3_qp *rqp, const struct ib_qp_attr *attr, int attr_mask,
	const struct rdma_service_cap *rdma_cap)
{
	enum ib_qp_state cur_state =
		(enum ib_qp_state)(((((u32)attr_mask) & IB_QP_CUR_STATE) != 0) ? attr->cur_qp_state : rqp->qp_state);
	enum ib_qp_state new_state =
		(enum ib_qp_state)(((((u32)attr_mask) & IB_QP_STATE) != 0) ? attr->qp_state : cur_state);

	if (((((u32)attr_mask) & IB_QP_MAX_QP_RD_ATOMIC) != 0) &&
		(attr->max_rd_atomic > rdma_cap->dev_rdma_cap.roce_own_cap.max_qp_init_rdma)) {
		pr_err("[ROCE, ERR] %s: Qpn(0x%x), max_rd_atomic (%d) too large. Transition %d to %d. qp_type(%d)\n", __func__,
			rqp->ibqp.qp_num, attr->max_rd_atomic, cur_state, new_state, rqp->ibqp.qp_type);
		return -EINVAL;
	}
	...
	if (((((u32)attr_mask) & IB_QP_MAX_DEST_RD_ATOMIC) != 0) &&
		...
		pr_err("[ROCE, ERR] %s: Qpn(0x%x), max_dest_rd_atomic (%d) too large. Transition %d to %d. qp_type(%d)\n",
			__func__, rqp->ibqp.qp_num, attr->max_dest_rd_atomic, cur_state, new_state, rqp->ibqp.qp_type);
		return -EINVAL;
	}
	...
	struct rdma_service_cap *rdma_cap)
{
	...
	enum ib_qp_state cur_state, new_state;
	cur_state = (enum ib_qp_state)(((((u32)attr_mask) & IB_QP_CUR_STATE) != 0) ? attr->cur_qp_state : rqp->qp_state);
	new_state = (enum ib_qp_state)(((((u32)attr_mask) & IB_QP_STATE) != 0) ? attr->qp_state : cur_state);
	...
	if (!roce3_check_qp_modify_ok(cur_state, new_state, (enum ib_qp_type)(rqp->ibqp.qp_type),
		...
		pr_err("[ROCE, ERR] %s: Qpn(0x%x), invalid attribute mask specified for transition %d to %d, qp_type(%d), "
			"attr_mask(0x%x)\n",
			__func__, rqp->ibqp.qp_num, cur_state, new_state, rqp->ibqp.qp_type, attr_mask);
		return (-EINVAL);
	}
	...
	if (((((u32)attr_mask) & IB_QP_PORT) != 0) && (attr->port_num != ROCE_DEFAULT_PORT_NUM)) {
		pr_err("[ROCE, ERR] %s: Qpn(0x%x), invalid port number(%d) specified for transition %d to %d. qp_type(%d)\n",
			__func__, rqp->ibqp.qp_num, attr->port_num, cur_state, new_state, rqp->ibqp.qp_type);
		return (-EINVAL);
	}

	if ((((u32)attr_mask) & IB_QP_PKEY_INDEX) != 0) {
		if (attr->pkey_index > 0) {
			pr_err("[ROCE, ERR] %s: Qpn(0x%x), invalid pkey index (%d) specified for transition %d to %d. "
				"qp_type(%d)\n",
				__func__, rqp->ibqp.qp_num, attr->pkey_index, cur_state, new_state, rqp->ibqp.qp_type);
			return (-EINVAL);
		}
	}
...
static int roce3_modify_qp_pre(struct ib_qp *ibqp, struct ib_qp_attr *attr, int attr_mask, struct ib_udata *udata)
{
	int ret;
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed qp modify check, func_id(%d)\n", __func__, rdev->glb_func_id);
		return ret;
	}
	...
	if (new_state == IB_QPS_RESET) {
		if (cur_state != new_state) {
			...
			if (ret != 0) {
				dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to modify 2rst, func_id(%d)\n", __func__,
					rdev->glb_func_id);
			}
		}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to modify extend qp, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
...
int roce3_modify_qp(struct ib_qp *ibqp, struct ib_qp_attr *attr, int attr_mask, struct ib_udata *udata)
{
	int ret = 0;
	struct roce3_qp *rqp = NULL;
	struct roce3_device *rdev = NULL;
...
int roce3_send_qp_lb_cmd(u32 qpn, struct roce3_device *rdev, u8 cmd, cqm_cmd_buf_s *buf_in, cqm_cmd_buf_s *buf_out,
	u32 timeout)
{
	int ret = 0;
	struct timespec64 tv_start;
	...
	if ((rdev->cfg_info.lb_en != 0) && (rdev->cfg_info.lb_mode == ROCE_LB_MODE_1)) {
		u8 cos = qpn & 0x3;
		ret = cqm_lb_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, cmd, cos, buf_in, buf_out, NULL, timeout,
			HINIC3_CHANNEL_ROCE);
	} else {
		ret = cqm_send_cmd_box(rdev->hwdev, HINIC3_MOD_ROCE, cmd, buf_in, buf_out, NULL, timeout, HINIC3_CHANNEL_ROCE);
	}
