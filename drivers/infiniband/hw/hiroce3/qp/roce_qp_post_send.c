 ...
 *
 * File Name	 : roce_qp_post_send.c
 * Version	   : v2.0
 * Created	   : 2021/3/12
 * Last Modified : 2021/11/23
 * Description   : Define standard kernel RoCE QP Post send realted functions.
 */
...
static const u32 roce3_ib_opcode[] = {
	...
	ROCE_WQE_OPCODE_MASKED_ATOMIC_FETCH_ADD, /* [IBV_EXP_WR_EXT_MASKED_ATOMIC_FETCH_AND_ADD] : 13 */
...
**************************************************************************** */
static void roce3_set_send_seg(struct roce3_wqe_send_tsk_seg *snd_tsk_seg, const struct ib_send_wr *wr)
{
...
static void roce3_set_atomic_seg(struct roce3_wqe_atomic_tsk_seg *atomic_tsk_seg, const struct ib_send_wr *wr)
{
...
**************************************************************************** */
static __always_inline void roce3_set_rdma_seg(struct roce3_wqe_rdma_tsk_seg *rdma_tsk_seg, const struct ib_send_wr *wr)
{
...
**************************************************************************** */
static void roce3_set_local_inv_seg(struct roce3_wqe_local_inv_tsk_seg *inv_tsk_seg, const struct ib_send_wr *wr)
{
...
**************************************************************************** */
static void roce3_set_ud_seg_cycle1(struct roce3_wqe_ud_tsk_seg_cycle1 *ud_tsk_seg_cycle1, const struct ib_send_wr *wr)
{
...
**************************************************************************** */
static void roce3_set_ud_seg_cycle2(struct roce3_wqe_ud_tsk_seg_cycle2 *ud_tsk_seg_cycle2, const struct ib_send_wr *wr)
{
	const struct ib_ud_wr *ud = ROCE_UD_WR(wr);
	struct roce3_ah *rah = to_roce3_ah(ud->ah);
...
static void roce3_set_dwqe_db(struct roce3_qp *qp, struct roce3_wqe_ctrl_seg *ctrl_seg, unsigned int index)
{
...
static void roce3_set_sq_db(struct roce3_qp *qp, struct roce_sq_db_seg *db, unsigned int index)
{
	...
	if (qp->qp_type == IB_QPT_RC) {
	} else if (qp->qp_type == IB_QPT_UD || qp->qp_type == IB_QPT_GSI) {
...
static void roce3_dwqe_copy(unsigned long *dst, unsigned long *src, unsigned int bytecnt)
{
	...
	unsigned long *dst_tmp = (unsigned long *)((u64)dst + ((bytecnt_tmp & BYTE_MASK) << 2)); // 2 is addr offset
...
static void roce3_set_gsi_seg(struct roce3_sqp *sqp, const struct ib_send_wr *wr, void *wqe)
{
	...
	ud_tsk_seg->qkey = cpu_to_be32(((ud_wr->remote_qkey & 0x80000000) != 0) ? sqp->qkey : ud_wr->remote_qkey);
	ud_tsk_seg->dw12.value = rah->priv_ah.dw7.value;
	ud_tsk_seg->dmac_l32 = rah->priv_ah.dmac_l32;
...
#define IB_WR_REG_SIG_MR IB_WR_REG_MR_INTEGRITY
	...
	if (ROCE_UNLIKELY(((u32)wr->num_sge) > rqp->sq.max_sge)) {
		pr_err("[ROCE, ERR] %s: Sge num is invalide, wr->num_sge(%d), rqp->sq.max_sge(%d)\n", __func__, wr->num_sge,
			rqp->sq.max_sge);
		return -EINVAL;
	}
...
static void roce3_set_rc_wqe_for_optype_send(const struct ib_send_wr *wr, struct roce3_post_send_normal_param *param)
{
...
static void roce3_set_rc_wqe_for_optype_atomic(const struct ib_send_wr *wr, struct roce3_post_send_normal_param *param)
{
...
static void roce3_set_rc_wqe_for_optype_write(const struct ib_send_wr *wr, struct roce3_post_send_normal_param *param)
{
	param->ctrl_seg_tmp.dw0.bs.tsl = ROCE_RDMA_WQE_TSL;
	param->ctrl_seg_tmp.dw1.bs.cl = 1;
...
static void roce3_set_rc_wqe_for_optype_local(const struct ib_send_wr *wr, struct roce3_post_send_normal_param *param)
{
	param->ctrl_seg_tmp.dw0.bs.tsl = ROCE_SEND_LOCAL_WQE_TSL;
	param->ctrl_seg_tmp.dw1.bs.cl = 1;
...
static int set_frmr_wqe_section(const struct ib_send_wr *wr, struct roce3_post_send_normal_param *param)
{
	u64 iova;
	u32 fbo;
	u32 page_num;
	...
	page_num = (u32)((frmr_reg_wr->mr->length + frmr_reg_wr->mr->page_size - 1) / frmr_reg_wr->mr->page_size);
	...
	param->dseg = (roce3_wqe_data_seg_s *) (((u8 *)param->tsk_com_seg) + sizeof(struct roce3_wqe_frmr_tsk_seg));

	param->ctrl_seg_tmp.dw0.bs.tsl = sizeof(struct roce3_wqe_frmr_tsk_seg) / ROCE_TASK_SEG_ALIGN;
	...
	frmr_wqe_task->dw3.bs.be = (u32)(!!((u32)frmr_reg_wr->access & IB_ACCESS_MW_BIND));		/* 绑定操作使能 */
	frmr_wqe_task->dw3.bs.lre = 1;															 /* 本地读使能 */
	frmr_wqe_task->dw3.bs.lwe = (u32)(!!((u32)frmr_reg_wr->access & IB_ACCESS_LOCAL_WRITE));   /* 本地写使能 */
	frmr_wqe_task->dw3.bs.rre = (u32)(!!((u32)frmr_reg_wr->access & IB_ACCESS_REMOTE_READ));   /* 远端读使能 */
	frmr_wqe_task->dw3.bs.rwe = (u32)(!!((u32)frmr_reg_wr->access & IB_ACCESS_REMOTE_WRITE));  /* 远端写使能 */
	frmr_wqe_task->dw3.bs.rae = (u32)(!!((u32)frmr_reg_wr->access & IB_ACCESS_REMOTE_ATOMIC)); /* 远端Atomic使能 */
	...
	struct roce3_qp *rqp, const struct ib_send_wr *wr, const struct ib_send_wr **bad_wr)
{
	...
	if (ROCE_UNLIKELY((wr->opcode != IB_WR_SEND) && (wr->opcode != IB_WR_SEND_WITH_IMM))) {
		// ret = -EINVAL;
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Wr->opcode(%d), func_id(%d)\n", __func__, wr->opcode,
			rdev->glb_func_id);
		*bad_wr = wr;
		return ROCE_NEED_JUMP;
	}
...
static void roce3_post_send_set_normal_data_seg(const struct ib_send_wr *wr, struct roce3_wqe_data_seg **dseg,
	struct roce3_qp *rqp)
{
	if (wr->num_sge != 0) {
		if ((u8 *)(*dseg) == rqp->sq_head_addr) {
			*dseg = (struct roce3_wqe_data_seg *)((void *)(rqp->sq_tail_addr - sizeof(struct roce3_wqe_data_seg)));
		} else {
			(*dseg)--;
		}
...
static int roce3_post_send_set_normal_sge(struct roce3_post_send_normal_param *param, struct roce3_device *rdev,
	struct roce3_qp *rqp, const struct ib_send_wr *wr, const struct ib_send_wr **bad_wr)
{
	int i;
	...
	for (i = 0; i < wr->num_sge; ++i) {
		...
		if (ROCE_UNLIKELY(wr->sg_list[i].length > (rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz))) {
			*bad_wr = wr;
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Sge_data length is over range, sg_list(%d), \
				length(0x%x), max_msg_sz(0x%x), func_id(%d)\n",
				__func__, i, wr->sg_list[i].length, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz,
				rdev->glb_func_id);
			return ROCE_NEED_JUMP;
		}

		param->data_len += wr->sg_list[i].length;
		if (ROCE_UNLIKELY(param->data_len > rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz)) {
			*bad_wr = wr;
			dev_err(rdev->hwdev_hdl,
				"[ROCE, ERR] %s: Data len is over range, data_len(%d), max_msg_sz(%d), func_id(%d)\n", __func__,
				param->data_len, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_msg_sz, rdev->glb_func_id);
			return ROCE_NEED_JUMP;
		}
...
static void roce3_copy_inline_data(struct roce3_post_send_normal_param *param, const struct ib_send_wr *wr,
	struct roce3_qp *rqp, int i)
{
	if (param->sq_rmd_size >= wr->sg_list[i].length) {
		...
		roce3_set_inline_data_cycle2(param->wqe, wr->sg_list + i, wr->sg_list[i].length - param->sq_rmd_size);
...
static int roce3_post_send_set_data_seg(struct roce3_post_send_normal_param *param, struct roce3_device *rdev,
	struct roce3_qp *rqp, const struct ib_send_wr *wr, const struct ib_send_wr **bad_wr)
{
	int i;
	int ret;

	if ((((unsigned int)wr->send_flags & (unsigned int)IB_SEND_INLINE) != 0) && (wr->num_sge != 0) &&
		(param->inline_flag != 0)) {
		param->data_len = 0;
			...
			if (param->data_len > rqp->max_inline_data) {
				*bad_wr = wr;
				dev_err(rdev->hwdev_hdl,
					...
					param->data_len, rqp->max_inline_data, rdev->glb_func_id);
				return ROCE_NEED_JUMP;
			}
		...
		param->ctrl_seg_tmp.dw0.bs.bdsl =
			(ALIGN((u32)(param->data_len), ROCE_TASK_SEG_ALIGN) / ROCE_TASK_SEG_ALIGN) & 0x7ff;
		...
		param->ctrl_seg_tmp.dw0.bs.bdsl =
			(u32)((wr->num_sge * (int)sizeof(struct roce3_wqe_data_seg)) / ROCE_TASK_SEG_ALIGN) & 0x7ff;
		param->ctrl_seg_tmp.dw0.bs.df = 0;
	}
...
static int roce3_post_send_ring_db(struct roce3_post_send_normal_param *param, struct roce3_device *rdev,
	struct roce3_qp *rqp)
{
	int ret = 0;

	if ((rdev->kernel_dwqe_map != NULL) && (param->wr_num == 1) && (param->inline_flag != 0) &&
		(param->wqe_size > (u32)sizeof(struct roce3_wqe_ctrl_seg)) && ((int)param->wqe_size <= rqp->max_dwqe_size) &&
		(param->cycle == 0)) {
		wmb();

		param->ctrl_seg->dw1.value = be32_to_cpu(param->ctrl_seg->dw1.value);
		param->ctrl_seg->dw1.bs.mask_pi = (u32)((param->index & ((unsigned)rqp->sq.wqebb_cnt - 1)) & 0xfffff);
		...
		*(rqp->db.db_record) = cpu_to_be32(param->index);
		...
		++rqp->sq.head;
		roce3_dwqe_copy((unsigned long *)rdev->kernel_dwqe_map, (unsigned long *)param->ctrl_seg, (u32)param->wqe_size);
		wc_wmb();
		...
		*(rqp->db.db_record) = cpu_to_be32(param->index & 0xffff);
		...
		wmb();
		ret = cqm_ring_hardware_db(rdev->hwdev, SERVICE_T_ROCE, param->index & 0xff, param->sq_db.sq_db_val);
		wmb();
	}
...
static int roce3_construct_wqe(struct roce3_post_send_normal_param *param, struct roce3_device *rdev,
	struct roce3_qp *rqp, const struct ib_send_wr *wr, const struct ib_send_wr **bad_wr)
{
	int need_goto = 0;
	...
	default:
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Unknown qp type, func_id(%d)\n", __func__, rdev->glb_func_id);
		break;
	}
...
static void roce3_init_ctrl_seg(struct roce3_post_send_normal_param *param, const struct ib_send_wr *wr,
	const struct roce3_qp *rqp)
{
	...
	param->ctrl_seg_tmp.dw0.bs.cr = (u32)(!(((((u32)wr->send_flags) & IB_SEND_SIGNALED) | rqp->sq_signal_bits) == 0));
...
static void roce3_init_task_com_seg(union roce3_wqe_tsk_com_seg **tsk_com_seg, u8 *wqe, const struct ib_send_wr *wr,
	const struct roce3_qp *rqp)
{
	*tsk_com_seg = (union roce3_wqe_tsk_com_seg *)(void *)wqe;

	(*tsk_com_seg)->bs.c =
		(u32)(!((((unsigned)wr->send_flags & (unsigned)IB_SEND_SIGNALED) | rqp->sq_signal_bits) == 0));
	(*tsk_com_seg)->bs.se = (u32)(!(((unsigned)wr->send_flags & (unsigned)IB_SEND_SOLICITED) == 0));
	(*tsk_com_seg)->bs.f = (u32)(!(((unsigned)wr->send_flags & (unsigned)IB_SEND_FENCE) == 0));
...
static void roce3_post_send_local_operation(struct roce3_post_send_normal_param *param, struct roce3_qp *rqp,
...
**************************************************************************** */
static int roce3_post_send_normal(struct roce3_device *rdev, struct roce3_qp *rqp, const struct ib_send_wr *wr,
	const struct ib_send_wr **bad_wr)
{
	...
	for (param.wr_num = 0; wr_tmp != NULL; ++(param.wr_num), wr_tmp = wr_tmp->next) {
		...
		rqp->sq.wrid[(rqp->sq.head + param.wr_num) & (u32)(rqp->sq.wqebb_cnt - 1)] = wr_tmp->wr_id;
...
/* ****************************************************************************
 ...
 Output	   : None
**************************************************************************** */
int roce3_post_send_standard(struct ib_qp *ibqp, const struct ib_send_wr *wr, const struct ib_send_wr **bad_wr)
{
	int ret = 0;
