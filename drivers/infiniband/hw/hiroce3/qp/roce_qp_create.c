 ...
 *
 * File Name	 : roce_qp_create.c
 * Version	   : v2.0
 * Created	   : 2021/3/10
 * Last Modified : 2021/12/23
 * Description   : The definition of RoCE QP service create realated functions in kernel space.
 */
...
**************************************************************************** */
static int roce3_check_rq_size(struct roce3_device *rdev, struct roce3_qp *rqp, struct ib_qp_init_attr *init_attr,
	bool has_rq)
{
	u32 max_sge_num;
	struct ib_qp_cap *cap = &init_attr->cap;

	max_sge_num = (u32)ROCE_MIN(rdev->rdma_cap.max_sq_sg, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_rq_sg);
	if ((cap->max_recv_wr > rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_wqes) || (cap->max_recv_sge > max_sge_num)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to check rq size, over range, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -EINVAL;
	}

	if (has_rq && ((cap->max_recv_wr == 0) || (cap->max_recv_sge == 0))) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Has rq, cap->max_recv_wr(%d), cap->max_recv_sge(%d), func_id(%d)\n",
			__func__, cap->max_recv_wr, cap->max_recv_sge, rdev->glb_func_id);
		return -EINVAL;
	}

	if ((!has_rq) && (cap->max_recv_wr != 0)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Not has rq, cap->max_recv_wr(%d), func_id(%d)\n", __func__,
...
**************************************************************************** */
static int roce3_set_rq_size(struct roce3_device *rdev, struct roce3_qp *rqp, struct ib_qp_init_attr *init_attr,
	bool is_user, bool has_rq)
{
	int ret;
	u32 wqebb_num = 0;
	u32 sge_total_len = 0;
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to check rq size, func_id(%d)\n", __func__, rdev->glb_func_id);
		return ret;
	}
	...
	if (has_rq) {
		...
		rqp->rq.max_sge = (cap->max_recv_sge <= ROCE_RQ_MIN_SGE) ?
			ROCE_RQ_MIN_SGE :
			((cap->max_recv_sge <= ROCE_RQ_MID_SGE) ? ROCE_RQ_MID_SGE : ROCE_RQ_MAX_SGE);

		sge_total_len = (u32)((u32)rqp->rq.max_sge * sizeof(struct roce3_wqe_data_seg));
		rqp->rq.wqe_shift = (u32)ROCE_ILOG2(sge_total_len);

		if ((u32)((u32)rqp->rq.wqebb_cnt << (unsigned)rqp->rq.wqe_shift) < PAGE_SIZE) {
			rqp->rq.wqebb_cnt = (PAGE_SIZE >> (unsigned)rqp->rq.wqe_shift);
		}

		/* leave userspace return values as they were, so as not to break ABI */
		if (is_user) {
			rqp->rq.max_post = ROCE_MIN(rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_wqes, rqp->rq.wqebb_cnt);
			cap->max_recv_wr = (u32)rqp->rq.max_post;
			cap->max_recv_sge = (u32)rqp->rq.max_sge;
		} else {
			rqp->rq.max_post = ROCE_MIN(rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_wqes, rqp->rq.wqebb_cnt);
			cap->max_recv_wr = (u32)rqp->rq.max_post;
			sge_num = ROCE_MIN(rdev->rdma_cap.max_sq_sg, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_rq_sg);
	...
	const struct roce3_qp *rqp)
{
	...
	if (cap->max_send_wr > (rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_wqes)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Log_sq_bb_count over range, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -EINVAL;
	}
	...
	if (rqp->qp_type == IB_QPT_UD) {
		ud_max_inline_size =
			(rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_sq_inline_data_sz > ROCE_UD_MAX_INLINE_LEN_SUB) ?
			(rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_sq_inline_data_sz - ROCE_UD_MAX_INLINE_LEN_SUB) :
			rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_sq_inline_data_sz;
	} else {
		ud_max_inline_size = rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_sq_inline_data_sz;
	}

	/* RC:192B inline data, UD:176B */
	if (cap->max_inline_data > ud_max_inline_size) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: SQ max inline data over range, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -EINVAL;
	}
...
**************************************************************************** */
static int roce3_set_user_sq_size(struct roce3_device *rdev, struct ib_qp_cap *cap, struct roce3_qp *rqp,
	struct create_qp_cmd *ucmd)
{
	...
	if (rqp->qp_type != IB_QPT_XRC_TGT) {
		/* between max and min */
		/*lint -e587*/
		tmp_max_sq_wqe_sz = (u32)(ROCE_ROUNDUP_POW_OF_TWO(rdev->rdma_cap.max_sq_desc_sz) & 0xffffffff);
		/*lint +e587*/
		if ((ucmd->log_sq_stride > ROCE_ILOG2(tmp_max_sq_wqe_sz)) ||
			...
			dev_err(rdev->hwdev_hdl,
				"[ROCE, ERR] %s: WQE size invalid, func_id(%d) log_sq_stride:%d wqebb_size:%d "
				"tmp_max_sq_wqe_sz:%d\n",
				__func__, rdev->glb_func_id, ucmd->log_sq_stride, rdev->rdma_cap.wqebb_size, tmp_max_sq_wqe_sz);
			return -EINVAL;
		}
		...
		if ((u32)((u64)rqp->sq.wqebb_cnt << (unsigned)rqp->sq.wqe_shift) < PAGE_SIZE) {
			rqp->sq.wqebb_cnt = (PAGE_SIZE >> (unsigned)rqp->sq.wqe_shift);
		}

		sq_buf_sz = (u32)ROCE_ALIGN((u64)rqp->sq.wqebb_cnt << (unsigned)rqp->sq.wqe_shift, PAGE_SIZE);
		rqp->buf_size = (int)ROCE_ALIGN(
			((unsigned)sq_buf_sz + ((unsigned)rqp->rq.wqebb_cnt << (unsigned)rqp->rq.wqe_shift)), PAGE_SIZE);
...
static int roce3_send_wqe_overhead(enum ib_qp_type qp_type)
{
	switch (qp_type) {
	case IB_QPT_RC:
		return sizeof(struct roce3_wqe_ctrl_seg) + sizeof(struct roce3_wqe_mask_atomic_tsk_seg);
...
static int roce3_set_kernel_sq_size_check(const struct roce3_device *rdev, const struct ib_qp_cap *cap,
	int *sq_max_wqe_size, enum ib_qp_type qp_type)
{
	int wqe_overhead = 0;
	int ret = 0;

	if (cap->max_send_wr > (u32)rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_wqes) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: SQ WR over range, func_id(%d)\n", __func__, rdev->glb_func_id);
		return -EINVAL;
	}

	if (cap->max_send_sge >
		(u32)ROCE_MIN(rdev->rdma_cap.max_sq_sg, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_rq_sg)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: SQ SGE over range, func_id(%d)\n", __func__, rdev->glb_func_id);
		return -EINVAL;
	}

	if (cap->max_inline_data > rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_sq_inline_data_sz) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: SQ max inline data over range, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -EINVAL;
	}
	...
	if (wqe_overhead < 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Invalid service type(%d), func_id(%d)\n", __func__, (int)qp_type,
			rdev->glb_func_id);
		return -EINVAL;
	}

	*sq_max_wqe_size =
		ROCE_MAX((int)(cap->max_send_sge * sizeof(struct roce3_wqe_data_seg)), (int)cap->max_inline_data) +
		wqe_overhead;
	if (*sq_max_wqe_size > (int)rdev->rdma_cap.max_sq_desc_sz) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: SQ WQE size over range, func_id(%d)\n", __func__, rdev->glb_func_id);
		return -EINVAL;
	}
...
**************************************************************************** */
static int roce3_set_kernel_sq_size(struct roce3_device *rdev, struct ib_qp_cap *cap, enum ib_qp_type qp_type,
	struct roce3_qp *rqp)

{
	...
	if (rqp->qp_type != IB_QPT_XRC_TGT) {
		...
		rqp->sq_max_wqes_per_wr =
			(u32)ROCE_DIV_ROUND_UP((unsigned)sq_max_wqe_size, (u32)(1UL << (unsigned)rqp->sq.wqe_shift));
		...
		rqp->sq.max_sge = (u32)(((u64)rqp->sq_max_wqes_per_wr << (unsigned)rqp->sq.wqe_shift) -
			(unsigned)roce3_send_wqe_overhead(qp_type));
		rqp->sq.max_sge =
			(u32)ROCE_MIN(rdev->rdma_cap.max_sq_desc_sz, rqp->sq.max_sge) / (int)sizeof(struct roce3_wqe_data_seg);

		sq_buf_sz = (u32)ALIGN((u64)rqp->sq.wqebb_cnt << (unsigned)rqp->sq.wqe_shift, PAGE_SIZE);
		rqp->buf_size =
			(int)ALIGN(((unsigned)sq_buf_sz + ((unsigned)rqp->rq.wqebb_cnt << (unsigned)rqp->rq.wqe_shift)), PAGE_SIZE);

		rqp->sq.offset = 0u;
		rqp->rq.offset = sq_buf_sz;

		if (((rqp->sq.wqebb_cnt - 1) / rqp->sq_max_wqes_per_wr) == 1) {
			dev_err(rdev->hwdev_hdl, "[ROCE] %s:Only one WR,please check,func_id(%d)\n", __func__, rdev->glb_func_id);
		...
		cap->max_send_sge = (u32)ROCE_MIN(rdev->rdma_cap.max_sq_sg, rdev->rdma_cap.dev_rdma_cap.roce_own_cap.max_rq_sg);
...
static int roce3_check_and_get_umem(struct roce3_device *rdev, struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr,
	struct ib_udata *udata, struct roce3_qp *rqp, struct create_qp_cmd *ucmd)
{
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to set user sq_size, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
...
static int roce3_alloc_qpc(struct roce3_qp *rqp, struct roce3_device *rdev, u32 qpn, struct ib_qp_init_attr *init_attr)
{
	...
	if (roce3_need_qpn_lb1_consistent_srqn(rqp, rdev, init_attr)) {
		...
		qpn_tmp = ((rsrq->srqn & ROCE3_LB1_MASK) << ROCE3_QPN_BIT_INDEX) | qpn_tmp >> ROCE3_QPN_CTRL_BIT_NUM;
		low2bit_align_en = true;
	}

	qpc_info = cqm_object_qpc_mpt_create(rdev->hwdev, SERVICE_T_ROCE, CQM_OBJECT_SERVICE_CTX,
		rdev->rdma_cap.dev_rdma_cap.roce_own_cap.qpc_entry_sz, rqp, qpn_tmp, low2bit_align_en);
	if (qpc_info == NULL) {
		ret = -ENOMEM;
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create qpc mpt by cqm object, func_id(%d), qpn(%u)\n",
	...
	if (roce3_need_qpn_lb1_consistent_srqn(rqp, rdev, init_attr)) {
		...
		if ((rsrq->srqn & ROCE3_LB1_MASK) != (rqp->qpn & ROCE3_LB1_MASK)) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create qpc LB srqn, func_id(%d), qpn(%u) srqn(%u)\n",
...
static int roce3_create_qp_user_mtt(struct roce3_device *rdev, struct roce3_qp *rqp)
{
	...
	ret = hmm_rdma_mtt_alloc(rdev->hwdev, npages, (u32)rqp->buf.page_shift, &rqp->mtt, SERVICE_T_ROCE);
	if (ret != 0) {
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to write mtt, func_id(%d)\n", __func__, rdev->glb_func_id);
		goto err_write_mtt;
	}
...
**************************************************************************** */
static int roce3_create_qp_user(struct roce3_device *rdev, struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr,
	struct ib_udata *udata, struct roce3_qp *rqp)
{
	int ret = 0;
	...
	if (init_attr->qp_type != IB_QPT_XRC_TGT) {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to map db page to user, func_id(%d)\n", __func__,
				rdev->glb_func_id);
			goto err_map_db;
		}
	}
...
static int roce3_get_wrid(struct roce3_qp *rqp, struct roce3_device *rdev)
{
	...
	rqp->sq.wrid = (u64 *)kzalloc((unsigned)rqp->sq.wqebb_cnt * sizeof(u64), GFP_KERNEL);
	if (rqp->sq.wrid == NULL) {
		rqp->sq.wrid = (u64 *)(void *)vzalloc((size_t)(rqp->sq.wqebb_cnt * sizeof(u64)));
	}

	rqp->rq.wrid = (u64 *)kzalloc((unsigned)rqp->rq.wqebb_cnt * sizeof(u64), GFP_KERNEL);
	if (rqp->rq.wrid == NULL) {
		rqp->rq.wrid = (u64 *)(void *)vzalloc((size_t)(rqp->rq.wqebb_cnt * sizeof(u64)));
	}

	if ((rqp->sq.wrid == NULL) || (rqp->rq.wrid == NULL)) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc wrid of sq or rq, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		ret = -ENOMEM;
	}
...
static void roce3_create_qp_kernel_set_attr(struct roce3_qp *rqp, cqm_queue_s *qp_buf_info)
{
...
static int roce3_create_qp_kernel_normal(struct roce3_device *rdev, struct roce3_qp *rqp)
{
	...
	qp_buf_info = cqm_object_rdma_queue_create(rdev->hwdev, SERVICE_T_ROCE, CQM_OBJECT_RDMA_QP, (u32)rqp->buf_size, rqp,
		true, ROCE_QP_INVLID_QP_NUM);
	if (qp_buf_info == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create rdma queue by cqm object, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return -1;
	}
	...
	rqp->mtt.mtt_type = MTT_CMTT_TYPE;
	ret = hmm_rdma_mtt_alloc(rdev->hwdev, qp_buf_info->q_room_buf_1.buf_number, (u32)rqp->buf.page_shift, &rqp->mtt,
		SERVICE_T_ROCE);
	if (ret != 0) {
...
**************************************************************************** */
static int roce3_create_qp_kernel(struct roce3_device *rdev, struct ib_qp_init_attr *init_attr, struct roce3_qp *rqp,
	u32 qpn)
{
	int ret = 0;
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to set sq buffer size in kernel, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
...
static void roce3_qp_add_cq_list(struct ib_qp_init_attr *init_attr, struct roce3_qp *rqp, struct roce3_device *rdev)
{
	...
	struct ib_qp_init_attr *init_attr, struct ib_udata *udata)
{
	...
	if (ibpd->uobject) {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to set rq size, func_id(%d)\n", __func__,
				rdev->glb_func_id);
			return ret;
		}
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] : Failed to create qp user, func_id(%d)\n", rdev->glb_func_id);
			return ret;
		}
	} else {
		...
		if (ret != 0) {
			dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create qp kernel, func_id(%d)\n", __func__,
				rdev->glb_func_id);
			return ret;
		}
...
static int roce3_create_qp_gsi(struct roce3_device *rdev, struct ib_pd *ibpd,
	...
	struct ib_qp_init_attr *init_attr)
{
	...
	sqp = (struct roce3_sqp *)kzalloc(sizeof(struct roce3_sqp), GFP_KERNEL);
	if (sqp == NULL) {
		pr_err("[ROCE, ERR] %s: Failed to alloc sqp\n", __func__);
...
static int roce3_create_check_init_attr(const struct ib_pd *ibpd, const struct ib_qp_init_attr *init_attr,
...
static int roce3_set_qp_attr(struct ib_qp_init_attr *init_attr, struct roce3_qp *rqp, struct roce3_device *rdev,
	struct ib_pd **ibpd, int qp_ext)
{
	...
	if (init_attr->qp_type == IB_QPT_XRC_TGT) {
		*ibpd = to_roce3_xrcd(init_attr->xrcd)->pd;
		...
		rqp->has_rq = false;
		rqp->xrcdn = xrcdn;
	} else if ((init_attr->qp_type == IB_QPT_XRC_INI) && roce3_is_roceaa(rdev->cfg_info.scence_id)) {
		init_attr->recv_cq = init_attr->send_cq;
		rqp->has_rq = false;
		...
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Unknow QP Type(%u), func_id(%d)\n", __func__, init_attr->qp_type,
			rdev->glb_func_id);
		return (-EINVAL);
	}
	return 0;
...
#if defined(OFED_MLNX_5_8)
static int roce3_create_gsi_qp_result(struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr,
	struct roce3_qp *rqp, struct roce3_device *rdev)
{
	int ret;

	ret = roce3_create_qp_gsi(rdev, ibpd, rqp, init_attr);
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create qp gsi, func_id(%hu)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
...
int roce3_create_qp_common(struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr, struct ib_udata *udata,
	struct roce3_qp *rqp, int qp_ext)
{
	int ret = 0;
	...
	if (init_attr->qp_type == IB_QPT_GSI) {
		return roce3_create_gsi_qp_result(ibpd, init_attr, rqp, rdev);
	}
	...
	if (ret != 0) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create qp in common process, func_id(%d)\n", __func__,
			rdev->glb_func_id);
		return ret;
	}
...
int roce3_create_qp(struct ib_qp *qp, struct ib_qp_init_attr *qp_init_attr, struct ib_udata *udata)
{
	struct roce3_qp *rqp = to_roce3_qp(qp);
	return roce3_create_qp_common(qp->pd, qp_init_attr, udata, rqp, ROCE_QP);
}
#else
static struct ib_qp *roce3_create_gsi_qp_result(struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr,
	struct roce3_device *rdev)
{
	...
	if (IS_ERR(rqp)) {
		ret = (int)PTR_ERR(rqp);
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to create qp gsi, func_id(%d)\n", __func__, rdev->glb_func_id);
...
struct ib_qp *roce3_create_qp_common(struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr, struct ib_udata *udata,
	int qp_ext)
{
	int ret = 0;
	struct roce3_qp *rqp = NULL;
	...
	rqp = (struct roce3_qp *)kzalloc(sizeof(*rqp), GFP_KERNEL);
	if (rqp == NULL) {
		dev_err(rdev->hwdev_hdl, "[ROCE, ERR] %s: Failed to alloc rqp, func_id(%d)\n", __func__, rdev->glb_func_id);
...
struct ib_qp *roce3_create_qp(struct ib_pd *ibpd, struct ib_qp_init_attr *init_attr, struct ib_udata *udata)
{
