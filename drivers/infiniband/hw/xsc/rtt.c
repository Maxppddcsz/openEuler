...
struct xsc_rtt_attributes {
	...
	ssize_t (*show)(struct xsc_rtt_interface *interface, struct xsc_rtt_attributes *attr,
			char *buf);
	ssize_t (*store)(struct xsc_rtt_interface *interface, struct xsc_rtt_attributes *attr,
			  ...
			  const char *buf, size_t count)
{
	...
	if (rtt_enable > 1) {
		xsc_core_err(g->xdev, "Failed to set rtt en, rtt_enable(%hu) out of range[0,1]\n",
			rtt_enable);
		return -EINVAL;
	}
...
static struct kobj_type rtt_ktype = {
	.sysfs_ops     = &rtt_sysfs_ops,
	.default_attrs = rtt_attrs
};
...
int xsc_rtt_sysfs_init(struct ib_device *ib_dev, struct xsc_core_device *xdev)
{
	...
	tmp = kzalloc(sizeof(struct xsc_rtt_interface), GFP_KERNEL);
	if (!tmp)
		return -ENOMEM;
