			...
			struct attribute *attr, char *buf)
{
	...
	if (xsc_counters_name_attr->dev == NULL ||
		xsc_counters_name_attr->desc == NULL ||
			...
			struct attribute *attr, char *buf)
{
	...
	if (xsc_counters_attr->dev == NULL ||
		xsc_counters_attr->desc == NULL ||
	...
	for (i = 0 ; i < xsc_counters_attr->desc_size; ++i) {
		...
		counter = *(uint64_t *)((char *)&out.hw_stats + desc->offset);
			...
			char *buf, loff_t loff, size_t size)
{
	...
	if (xsc_counters_bin_attr->dev == NULL ||
		xsc_counters_bin_attr->desc == NULL ||
	...
	for (i = 0; i < xsc_counters_bin_attr->desc_size; ++i) {
		...
		tmp_value[i] = *(uint64_t *)((char *)&out.hw_stats + desc->offset);
...
int xsc_counters_init(struct ib_device *ib_dev, struct xsc_core_device *dev)
{
	...
	if (is_pf) {
		...
		xsc_counters_bin->size = xsc_counters_bin->desc_size * sizeof(uint64_t);
	} else {
		xsc_counters_name->desc = xsc_counters->desc =
