...
static void hclgevf_rss_init_cfg(struct hclgevf_dev *hdev)
{
	...
	if (hdev->ae_dev->dev_version >= HNAE3_DEVICE_VERSION_V2) {
		...
		tuple_sets->ipv4_sctp_en = HCLGEVF_RSS_INPUT_TUPLE_SCTP4;
		tuple_sets->ipv4_fragment_en = HCLGEVF_RSS_INPUT_TUPLE_OTHER;
		tuple_sets->ipv6_tcp_en = HCLGEVF_RSS_INPUT_TUPLE_OTHER;
		tuple_sets->ipv6_udp_en = HCLGEVF_RSS_INPUT_TUPLE_OTHER;
		tuple_sets->ipv6_sctp_en = HCLGEVF_RSS_INPUT_TUPLE_SCTP6;
...
#define	MAX_LENGTH_OF_PORT_BASE_VLAN_INFO	8U
	...
	if (test_bit(HCLGEVF_STATE_RST_HANDLING, &hdev->state) ||
	    test_bit(HCLGEVF_STATE_RST_FAIL, &hdev->state)) {
		dev_err(&hdev->pdev->dev, "dev resetting\n");
		rtnl_unlock();
		return;
	}
