			 ...
			 const  u8 *key, const  u8 hfunc)
{
	...
	if (key) {
		...
		memcpy(vport->rss_hash_key, key, sizeof(vport->rss_hash_key));
	} else {
...
static int hclge_init_rss_tc_mode(struct hclge_dev *hdev)
{
	...
	for (i = 0; i < HCLGE_MAX_TC_NUM; i++) {
		 ...
		 * table.
		 */
		if (rss_size > HCLGE_RSS_IND_TBL_SIZE ||
		    rss_size == 0) {
			dev_err(&hdev->pdev->dev,
				    ...
				    u32 *unused_tuple)
{
	...
	if (fs->flow_type & FLOW_MAC_EXT) {
		if (hdev->fd_cfg.fd_mode !=
			...
			dev_err(&hdev->pdev->dev,
				"FLOW_MAC_EXT does not support in current FD mode!\n");
			return -EOPNOTSUPP;
		}
			       ...
			       u32 *unused_tuple)
{
	...
	case ETHER_FLOW:
		if (hdev->fd_cfg.fd_mode !=
			HCLGE_FD_MODE_DEPTH_2K_WIDTH_400B_STAGE_1) {
			dev_err(&hdev->pdev->dev,
				"ETHER_FLOW does not support in current FD mode!\n");
			return -EOPNOTSUPP;
		}
				     ...
				     struct ethtool_tcpip4_spec *spec_mask)
{
	...
	spec->ip4dst = cpu_to_be32(rule->tuples.dst_ip[3]);
...
{
	struct hclge_vport_mac_addr_cfg *old_node, *new_node;
...
int hclge_reset_tqp_cmd(struct hnae3_handle *handle)
{
	struct hclge_vport *vport = hclge_get_vport(handle);
	struct hclge_dev *hdev = vport->back;
...
int hclge_reset_rcb(struct hnae3_handle *handle)
{
