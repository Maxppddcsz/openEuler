...
/* confirm qp_num from rss_tmpl_id,irq_num or dcb_en */
				   ...
				   const void *priv_data)
{
	...
	nicif_info(nic_dev, drv, netdev, "Success to modify channel setting\n");
...
static u32 sss_nic_calc_toep_rss(const u32 *rss_tunple, u32 size, const u32 *rss_key)
{
	...
	for (i = 1; i <= size; i++) {
		for (j = 0; j < SSSNIC_BIT_PER_TUPLE; j++) {
			tunple = rss_tunple[i - 1] &
				 ((u32)1 << (u32)((SSSNIC_BIT_PER_TUPLE - 1) - j));
			if (tunple != 0)
				rss ^= (rss_key[i - 1] << j) |
				       ((u32)((u64)rss_key[i] >> (SSSNIC_BIT_PER_TUPLE - j)));
		}
	}
...
static u8 sss_nic_parse_ipv6_info(struct sk_buff *skb, u32 *rss_tunple,
				  u8 hash_engine, u32 *size)
{
				      ...
				      unsigned int max_sq_num)
{
	...
	if (iphdr->version == IPV4_VERSION) {
		...
		l4_proto = sss_nic_parse_ipv6_info(skb, (u32 *)rss_tunple, hash_engine, &cnt);
...
static u8 sss_nic_get_cos_by_dscp(struct sss_nic_dev *nic_dev, struct sk_buff *skb)
{
	int dscp_cp;

	if (skb->protocol == htons(ETH_P_IP))
		dscp_cp = ipv4_get_dsfield(ip_hdr(skb)) >> SSSNIC_GET_DSCP_PRI_OFFSET;
	else if (skb->protocol == htons(ETH_P_IPV6))
		dscp_cp = ipv6_get_dsfield(ipv6_hdr(skb)) >> SSSNIC_GET_DSCP_PRI_OFFSET;
	else
		return nic_dev->hw_dcb_cfg.default_cos;
	return nic_dev->hw_dcb_cfg.dscp2cos[dscp_cp];
}

static u8 sss_nic_get_cos_by_pcp(struct sss_nic_dev *nic_dev,
				 struct sk_buff *skb)
{
	if (skb->vlan_tci != 0)
		return nic_dev->hw_dcb_cfg.pcp2cos[skb->vlan_tci >> VLAN_PRIO_SHIFT];
	else
		return nic_dev->hw_dcb_cfg.default_cos;
}
			   ...
			   u16 cur_vlanprio, int vf_id, u16 vlan_id, u8 qos)
{
	...
	if (vlan_id == 0 && qos == 0) {
		ret = sss_nic_kill_vf_vlan(nic_dev->nic_io, SSSNIC_OS_VF_ID_TO_HW(vf_id));
	} else {
		if (cur_vlanprio != 0) {
			ret = sss_nic_kill_vf_vlan(nic_dev->nic_io, SSSNIC_OS_VF_ID_TO_HW(vf_id));
			if (ret != 0)
		...
		ret = sss_nic_add_vf_vlan(nic_dev->nic_io, SSSNIC_OS_VF_ID_TO_HW(vf_id),
					  vlan_id, qos);
	}
