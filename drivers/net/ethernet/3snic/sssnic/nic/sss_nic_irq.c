...
static int sss_nic_poll(struct napi_struct *napi, int budget)
{
	int tx_pkt;
	int rx_pkt;
...
static void sss_nic_add_napi(struct sss_nic_irq_cfg *nic_irq, int poll_weight)
{
	netif_napi_add(nic_irq->netdev, &nic_irq->napi, sss_nic_poll, poll_weight);
...
static int sss_nic_request_irq(struct sss_nic_dev *nic_dev, u16 qid)
{
	...
	sss_nic_add_napi(nic_irq, nic_dev->poll_weight);
