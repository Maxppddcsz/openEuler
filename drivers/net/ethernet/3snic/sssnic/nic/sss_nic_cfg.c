...
int sss_nic_set_mac(struct sss_nic_dev *nic_dev, const u8 *mac_addr, u16 vlan_id, u16 channel)
{
...
int sss_nic_del_mac(struct sss_nic_dev *nic_dev, const u8 *mac_addr, u16 vlan_id, u16 channel)
{
	struct sss_nic_mbx_mac_addr cmd_mac = {0};
	u16 out_len = sizeof(cmd_mac);
	u32 func_type;
...
int sss_nic_update_mac(struct sss_nic_dev *nic_dev, u8 *new_mac, u16 vlan_id)
{
	int ret;
	u32 func_type;
	...
	if ((vlan_id & SSSNIC_VLAN_ID_MASK) >= VLAN_N_VID) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Invalid VLAN: %d\n",
			(vlan_id & SSSNIC_VLAN_ID_MASK));
		return -EINVAL;
	}
	...
	cmd_mac_update.old_mac.vlan_id = vlan_id;
	...
	if (ret != 0 || out_len == 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev),
...
static int sss_nic_feature_nego(struct sss_nic_io *nic_io, u8 opcode,
				u64 *feature, u16 feature_cnt)
{
	struct sss_nic_mbx_feature_nego cmd_feature_nego = {0};
	u16 out_len = sizeof(cmd_feature_nego);
	int ret;

	if (!nic_io || !feature || feature_cnt > SSSNIC_MAX_FEATURE_QWORD)
		return -EINVAL;

	cmd_feature_nego.opcode = opcode;
	cmd_feature_nego.func_id = sss_get_global_func_id(nic_io->hwdev);
	if (opcode == SSSNIC_MBX_OPCODE_SET)
		memcpy(cmd_feature_nego.feature, feature, feature_cnt * sizeof(u64));
	...
	if (opcode == SSSNIC_MBX_OPCODE_GET)
		memcpy(feature, cmd_feature_nego.feature, feature_cnt * sizeof(u64));
...
{
	return sss_nic_feature_nego(nic_io, SSSNIC_MBX_OPCODE_GET, &nic_io->feature_cap, 1);
}

int sss_nic_set_feature_to_hw(struct sss_nic_io *nic_io)
{
	return sss_nic_feature_nego(nic_io, SSSNIC_MBX_OPCODE_SET, &nic_io->feature_cap, 1);
}
...
static void sss_nic_io_params_init(struct sss_nic_io *nic_io)
{
	nic_io->mag_cfg.rt_cmd.mpu_send_sfp_info = false;
	nic_io->mag_cfg.rt_cmd.mpu_send_sfp_abs = false;
...
int sss_nic_io_init(struct sss_nic_dev *nic_dev)
{
	...
	ret = sss_chip_set_func_used_state(nic_dev->hwdev, SSS_SVC_TYPE_NIC,
					   SSS_FUNC_USED_STATE, SSS_CHANNEL_NIC);
	if (ret != 0) {
...
init_func_table_err:
	sss_chip_set_func_used_state(nic_dev->hwdev, SSS_SVC_TYPE_NIC,
				     SSS_FUNC_UNUSED_STATE, SSS_CHANNEL_NIC);
...
int sss_nic_set_rx_lro_state(struct sss_nic_dev *nic_dev, bool en, u32 timer, u32 max_pkt_len)
{
	...
	nic_info(SSSNIC_DEV_TO_DEV(nic_dev), "Set LRO max coalesce packet size to %uK\n",
		 max_pkt_len);
	...
	nic_info(SSSNIC_DEV_TO_DEV(nic_dev), "Set LRO timer to %u\n", timer);
