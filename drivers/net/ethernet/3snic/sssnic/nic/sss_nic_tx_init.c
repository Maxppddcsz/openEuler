	...
	for (qid = 0; qid < qp_res->qp_num; qid++) {
		...
		if (!sq_res->dma_group) {
			nicif_err(nic_dev, drv, nic_dev->netdev,
				  "Fail to allocate transmit queue %d buffer descriptors\n", qid);
			goto error;
		}
...
int sss_nic_init_sq_info(struct sss_nic_dev *nic_dev)
{
	struct sss_nic_sq_desc *sq_desc = NULL;
	...
	for (qid = 0; qid < sq_num; qid++) {
		...
		sq_desc->qid = qid;
		sq_desc->dev = SSSNIC_DEV_TO_DEV(nic_dev);
...
void sss_nic_deinit_sq_info(struct sss_nic_dev *nic_dev)
{
...
int sss_nic_flush_all_sq(struct sss_nic_dev *nic_dev)
{
	u16 qid = 0;
	int ret = 0;
