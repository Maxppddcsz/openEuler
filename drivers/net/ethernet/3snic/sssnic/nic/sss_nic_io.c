...
enum sss_nic_qp_ctx_type {
	SSSNIC_QP_CTXT_TYPE_SQ,
	SSSNIC_QP_CTXT_TYPE_RQ,
};
					     ...
					     u16 queue_num, u16 qid)
{
	...
	qp_ctx_hdr->num_queues = queue_num;
	qp_ctx_hdr->queue_type = ctx_type;
					   ...
					   struct sss_nic_sq_ctx *sq_ctx)
{
	...
	sq_ctx->pref_wq_pfn_lo = wq_page_pfn_lo;

	sq_ctx->pref_wq_pfn_hi_ci =
		SSSNIC_SET_SQ_CTX_PREF(ci_start, CI_LOW) |
		SSSNIC_SET_SQ_CTX_PREF(wq_page_pfn_hi, WQ_PFN_HI);
	...
	sq_ctx->wq_pfn_hi_owner =
		SSSNIC_SET_SQ_CTX_WQ_PAGE(wq_page_pfn_hi, HI_PFN) |
		SSSNIC_SET_SQ_CTX_WQ_PAGE(1, OWNER);

	sq_ctx->wq_pfn_lo = wq_page_pfn_lo;

	sq_ctx->wq_block_pfn_lo = wq_block_pfn_lo;

	sq_ctx->wq_block_pfn_hi =
		SSSNIC_SET_SQ_CTX_WQ_BLOCK(wq_block_pfn_hi, PFN_HI);
					   ...
					   struct sss_nic_rq_ctx *rq_ctx)
{
	...
	u32 wq_page_pfn_lo = lower_32_bits(wq_page_pfn);
	u32 wq_page_pfn_hi = upper_32_bits(wq_page_pfn);
	u32 wq_block_pfn_lo = lower_32_bits(wq_block_pfn);
	u32 wq_block_pfn_hi = upper_32_bits(wq_block_pfn);

	rq_ctx->wq_pfn_hi_type_owner =
		SSSNIC_SET_RQ_CTX_WQ_PAGE(wq_page_pfn_hi, HI_PFN) |
	...
	rq_ctx->wq_pfn_lo = wq_page_pfn_lo;

	rq_ctx->pref_wq_pfn_lo = wq_page_pfn_lo;
	rq_ctx->pref_wq_pfn_hi_ci =
		SSSNIC_SET_RQ_CTX_PREF(wq_page_pfn_hi, WQ_PFN_HI) |
	...
	rq_ctx->wq_block_pfn_lo = wq_block_pfn_lo;
	rq_ctx->wq_block_pfn_hi =
		SSSNIC_SET_RQ_CTX_WQ_BLOCK(wq_block_pfn_hi, PFN_HI);
...
static int sss_nic_send_sq_ctx_to_hw(struct sss_nic_io *nic_io)
{
	...
	u16 max_qp;
	u64 out_param = 0;
	struct sss_nic_sq_ctx_block *sq_ctx_block = NULL;
...
static int sss_nic_send_rq_ctx_to_hw(struct sss_nic_io *nic_io)
{
	...
	u64 out_param = 0;
	struct sss_ctrl_msg_buf *msg_buf = NULL;
	struct sss_nic_rq_ctx_block *rq_ctx_block = NULL;
	...
	while (qid < nic_io->active_qp_num) {
		...
		sss_nic_fill_qp_ctx_ctrlq_header(&rq_ctx_block->ctrlq_hdr,
						 SSSNIC_QP_CTXT_TYPE_RQ, max_qp, qid);
					...
					enum sss_nic_qp_ctx_type ctx_type)
{
	...
	ctx_block->ctrlq_hdr.queue_type = ctx_type;
	ctx_block->ctrlq_hdr.num_queues = nic_io->max_qp_num;
...
static int sss_nic_reset_hw_qp_offload_ctx(struct sss_nic_io *nic_io)
{
	...
	ret = sss_nic_reset_hw_offload_ctx(nic_io, SSSNIC_QP_CTXT_TYPE_SQ);
	if (ret != 0)
		return ret;

	ret = sss_nic_reset_hw_offload_ctx(nic_io, SSSNIC_QP_CTXT_TYPE_RQ);
