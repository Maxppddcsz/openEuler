...
MODULE_PARM_DESC(set_vf_link_state,
...
enum {
	SSSNIC_IFLA_VF_LINK_STATE_AUTO = 0x0, /* link state of the uplink */
	SSSNIC_IFLA_VF_LINK_STATE_DISABLE = 0x2, /* link always down */
...
static int sss_nic_set_vlan_ctx(struct sss_nic_io *nic_io, u16 func_id,
...
int sss_nic_cfg_vf_vlan(struct sss_nic_io *nic_io, u8 opcode, u16 vlan_id, u8 qos, int vf_id)
{
	int ret;
	u32 vlan_mode;
	...
	ret = sss_nic_set_vlan_ctx(nic_io, func_id, vlan_tag,
...
int sss_nic_add_vf_vlan(struct sss_nic_io *nic_io, int vf_id, u16 vlan, u8 qos)
{
	...
	ret = sss_nic_cfg_vf_vlan(nic_io, SSSNIC_MBX_OPCODE_ADD, vlan, qos, vf_id);
	if (ret != 0)
		return ret;
...
int sss_nic_kill_vf_vlan(struct sss_nic_io *nic_io, int vf_id)
{
	...
	ret = sss_nic_cfg_vf_vlan(nic_io, SSSNIC_MBX_OPCODE_DEL,
...
int sss_nic_set_vf_spoofchk(struct sss_nic_io *nic_io, u16 vf_id, bool spoofchk)
{
	...
	ret = sss_nic_l2nic_msg_to_mgmt_sync(nic_io->hwdev, SSSNIC_MBX_OPCODE_SET_SPOOPCHK_STATE,
					     ...
					     &out_len);
	if (ret || !out_len || cmd_spoofchk_cfg.head.state) {
...
bool sss_nic_vf_info_spoofchk(struct sss_nic_io *nic_io, int vf_id)
{
	...
	return nic_io->vf_info_group[id].spoofchk;
}
...
static int sss_nic_init_vf_info(struct sss_nic_io *nic_io, u16 vf_id)
{
	...
	if (set_vf_link_state >= SSSNIC_IFLA_VF_LINK_STATE_MAX) {
		set_vf_link_state = SSSNIC_IFLA_VF_LINK_STATE_AUTO;
		nic_warn(nic_io->dev_hdl, "Invalid set_vf_link_state: %u out of range[%u - %u], adjust to %d\n",
			 set_vf_link_state, SSSNIC_IFLA_VF_LINK_STATE_AUTO,
			 SSSNIC_IFLA_VF_LINK_STATE_DISABLE, SSSNIC_IFLA_VF_LINK_STATE_AUTO);
	}

	link_state = set_vf_link_state;
	if (link_state < SSSNIC_IFLA_VF_LINK_STATE_MAX) {
		handler[link_state](&vf_info_group[vf_id]);
	} else {
		nic_err(nic_io->dev_hdl, "Fail to input set_vf_link_state: %u\n",
			link_state);
		return -EINVAL;
	}
...
static int sss_nic_register_vf_to_hw(struct sss_nic_io *nic_io)
{
	...
	cmd_register_info.extra_feature = 0;
...
static void sss_nic_clear_vf_info(struct sss_nic_io *nic_io, u16 vf_id)
{
	...
	if (vf_info->specified_mac)
		sss_nic_del_mac(nic_io->nic_dev, vf_info->drv_mac,
				vf_info->pf_vlan, SSS_CHANNEL_NIC);

	if (sss_nic_vf_info_vlan_prio(nic_io, vf_id))
		sss_nic_kill_vf_vlan(nic_io, vf_id);

	if (vf_info->max_rate > 0 && SSSNIC_SUPPORT_RATE_LIMIT(nic_io))
