...
#define SSSNIC_DEFAULT_FILTER_STATE 1
#define SSSNIC_DEFAULT_RX_MODE	(SSSNIC_RX_MODE_UC | SSSNIC_RX_MODE_MC | SSSNIC_RX_MODE_BC)

static unsigned char g_set_filter_state = SSSNIC_DEFAULT_FILTER_STATE;
module_param(g_set_filter_state, byte, 0444);
MODULE_PARM_DESC(g_set_filter_state, "Set mac filter config state: 0 - disable, 1 - enable (default=1)");

static int sss_nic_sync_uc(struct net_device *netdev, u8 *address)
{
	struct sss_nic_dev *nic_dev = netdev_priv(netdev);

	return sss_nic_set_mac(nic_dev, address, 0, SSS_CHANNEL_NIC);
}
...
static int sss_nic_unsync_uc(struct net_device *netdev, u8 *address)
{
	...
	return sss_nic_del_mac(nic_dev, address, 0, SSS_CHANNEL_NIC);
}
