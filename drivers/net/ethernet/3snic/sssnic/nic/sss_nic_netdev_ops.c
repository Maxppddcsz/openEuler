...
#endif /* end of HAVE_NDO_SELECT_QUEUE_ACCEL_FALLBACK */
{
	...
	if (nic_dev->hw_dcb_cfg.cos_qp_num[cos] == 0)
		return nic_dev->hw_dcb_cfg.cos_qp_offset[cos];

	qp_num = sq_num % nic_dev->hw_dcb_cfg.cos_qp_num[cos];
...
static void sss_nic_ndo_tx_timeout(struct net_device *netdev,
				   unsigned int __maybe_unused txqueue)
#else
static void sss_nic_ndo_tx_timeout(struct net_device *netdev)
#endif
{
	...
	SSSNIC_NIC_STATS_INC(nic_dev, tx_timeout);
...
static int sss_nic_ndo_set_mac_address(struct net_device *netdev, void *mac_addr)
{
	...
	ret = sss_nic_update_mac(nic_dev, set_addr->sa_data, 0);
	if (ret)
		return ret;
				...
				__always_unused __be16 proto, u16 vlan_id)
{
	...
	nicif_info(nic_dev, drv, netdev, "The vlan %u was added\n", vlan_id);
				 ...
				 __always_unused __be16 proto, u16 vlan_id)
{
	...
	nicif_info(nic_dev, drv, netdev, "The vlan %u was removed\n", vlan_id);
...
static int sss_nic_ndo_set_vf_mac(struct net_device *netdev, int vf_id, u8 *mac)
{
	...
	sss_nic_set_vf_mac(nic_dev, SSSNIC_OS_VF_ID_TO_HW(vf_id), mac);

	if (is_zero_ether_addr(mac))
		nic_info(SSSNIC_DEV_TO_DEV(nic_dev), "Delete mac on vf %d\n", vf_id);
	else
		nic_info(SSSNIC_DEV_TO_DEV(nic_dev), "Set mac %pM on vf %d\n", mac, vf_id);

	nic_info(SSSNIC_DEV_TO_DEV(nic_dev),
		 "Reload vf driver to make this change effective\n");
				       ...
				       bool set_spoofchk)
{
	...
	cur_spoofchk = sss_nic_vf_info_spoofchk(nic_dev->nic_io, id);
...
static int sss_nic_ndo_set_vf_trust(struct net_device *netdev, int vf, bool new_trust)
{
	bool old_trust;
	struct sss_nic_dev *nic_dev = netdev_priv(netdev);

	if ((vf >= pci_num_vf(nic_dev->pdev)) && (vf > nic_dev->nic_io->max_vf_num)) {
		nicif_err(nic_dev, drv, netdev, "Invalid vf id, VF: %d pci_num_vf: %d max_vfs: %d\n",
			  vf, pci_num_vf(nic_dev->pdev), nic_dev->nic_io->max_vf_num);
		return -EINVAL;
	}

	old_trust = nic_dev->nic_io->vf_info_group[vf].trust;
	/* Same old and new, no need to set, return success directly */
	if ((new_trust && old_trust) || (!new_trust && !old_trust))
		return 0;

	nic_dev->nic_io->vf_info_group[vf].trust = new_trust;

	nicif_info(nic_dev, drv, netdev, "Success to set VF %d trust %d to %d\n",
		   vf, old_trust, new_trust);
...
static int sss_nic_ndo_get_vf_config(struct net_device *netdev,
				     int vf, struct ifla_vf_info *ifla_vf)
{
	struct sss_nic_dev *nic_dev = netdev_priv(netdev);

	if (vf >= pci_num_vf(nic_dev->pdev))
		return -EINVAL;

	sss_nic_get_vf_attribute(nic_dev->nic_io, (u16)SSSNIC_OS_VF_ID_TO_HW(vf), ifla_vf);

	return 0;
}

/**
 * sss_nic_ndo_set_vf_link_state
				     ...
				     int vf_id, int min_rate, int max_rate)
{
	if (!SSSNIC_SUPPORT_RATE_LIMIT(nic_dev->nic_io)) {
		nicif_err(nic_dev, drv, nic_dev->netdev, "Invalid set vf rate limit support.\n");
		return -EOPNOTSUPP;
	}
...
#endif /* HAVE_NDO_SET_VF_MIN_MAX_TX_RATE */
{
	...
	if (max_tx_rate > speeds[port_info.speed] || max_tx_rate < 0) {
		nicif_err(nic_dev, drv, netdev, "Set vf max tx rate must be in [0 - %u]\n",
...
#else
	nicif_info(nic_dev, drv, netdev,
		   "Success to set VF %d max tx rate %d min tx rate %d\n",
		   vf_id, max_tx_rate, min_tx_rate);
#endif
