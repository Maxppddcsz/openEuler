...
static unsigned char qp_pending_limit = SSSNIC_DEAULT_TXRX_MSIX_PENDING_LIMIT;
...
static unsigned char g_link_follow_status = SSSNIC_LINK_FOLLOW_STATUS_MAX;
module_param(g_link_follow_status, byte, 0444);
MODULE_PARM_DESC(g_link_follow_status, "Set link follow status port status (0=default,1=follow,2=separate,3=unset");

#define SSSNIC_NIC_DEV_WQ_NAME		"sss_nic_dev_wq"
...
#define RX_BUFF_TO_BYTES(rx_buff) ((u16)((rx_buff) * CONVERT_UNIT))
...
#define POLL_WEIGHT_IS_VALID(weight) \
	(weight > 0 && weight <= SSSNIC_MAX_RX_QUEUE_DEPTH)
...
static void sss_nic_register_notifier(struct sss_nic_dev *nic_dev)
{
	...
	if (g_netdev_notifier_ref_cnt == 1) {
		...
		if (ret != 0) {
			nic_info(SSSNIC_DEV_TO_DEV(nic_dev),
...
static netdev_features_t sss_nic_default_ufo_feature(struct sss_nic_dev *nic_dev)
{
	netdev_features_t feature = 0;
					   ...
					   netdev_features_t lro_feature)
{
	...
	hw_features |= netdev->features | ufo_feature | lro_feature;
...
static void sss_nic_init_netdev_feature(struct sss_nic_dev *nic_dev)
{
	...
	netdev_features_t ufo_feature = 0;
	...
	ufo_feature = sss_nic_default_ufo_feature(nic_dev);
	...
	netdev->vlan_features |= SSSNIC_NETDEV_DEFAULT_FEATURE |
				 cso_feature | gso_feature | ufo_feature;

	sss_nic_init_netdev_hw_feature(nic_dev, ufo_feature, lro_feature);
...
static void sss_nic_init_intr_coal_param(struct sss_nic_intr_coal_info *intr_coal, u16 max_qp)
{
	...
	for (i = 0; i < max_qp; i++) {
		...
		intr_coal[i].pending_limt = qp_pending_limit;
		intr_coal[i].coalesce_timer = qp_coalesc_timer_cfg;
...
static int sss_nic_init_intr_coalesce(struct sss_nic_dev *nic_dev)
{
	...
	if (!nic_dev->coal_info) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to alloc intr_coalesce\n");
		return -ENOMEM;
	}
...
static void sss_nic_dev_deinit(struct sss_nic_dev *nic_dev)
{
	...
	sss_nic_deinit_sq_info(nic_dev);

	sss_nic_clean_mac_list_filter(nic_dev);

	sss_nic_del_mac(nic_dev, nic_dev->netdev->dev_addr, 0, SSS_CHANNEL_NIC);
...
static int sss_nic_init_mac_addr(struct sss_nic_dev *nic_dev)
{
	...
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to get MAC address\n");
		return ret;
	}

	if (!is_valid_ether_addr(netdev->dev_addr)) {
		nic_info(SSSNIC_DEV_TO_DEV(nic_dev),
			 "Invalid default MAC address %02x:%02x:%02x:%02x:%02x:%02x\n",
			 netdev->dev_addr[0], netdev->dev_addr[1], netdev->dev_addr[2],
			 netdev->dev_addr[3], netdev->dev_addr[4], netdev->dev_addr[5]);
		if (!SSSNIC_FUNC_IS_VF(nic_dev->hwdev)) {
			nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Invalid default MAC address\n");
			return -EIO;
		}
		eth_hw_addr_random(netdev);
	}

	ret = sss_nic_set_mac(nic_dev, netdev->dev_addr, 0, SSS_CHANNEL_NIC);
	if (ret != 0 && ret != SSSNIC_PF_SET_VF_ALREADY) {
		 ...
		 */
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to set default MAC\n");
		return ret;
	}
...
static int sss_nic_dev_init(struct sss_nic_dev *nic_dev)
{
	...
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to init dcb\n");
		return -EFAULT;
	}
	...
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to init mac address\n");
		goto init_mac_addr_err;
	}

	sss_nic_set_mtu_range(netdev);

	ret = sss_nic_init_sq_info(nic_dev);
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to init sq\n");
		goto init_sq_err;
	}

	ret = sss_nic_alloc_rq_desc_group(nic_dev);
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to init rq\n");
		goto init_rq_err;
	}

	ret = sss_nic_init_intr_coalesce(nic_dev);
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to init interrupt and coalesce\n");
		goto init_intr_coalesce_err;
	}

	ret = sss_nic_alloc_lb_test_buf(nic_dev);
	if (ret) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to alloc loopback test buf\n");
...
init_sq_err:
	sss_nic_del_mac(nic_dev, netdev->dev_addr, 0, SSS_CHANNEL_NIC);
				   ...
				   struct sss_hal_dev *uld_dev)
{
	...
	nic_dev->rx_buff_len = RX_BUFF_TO_BYTES(rx_buff);
	...
	nic_dev->workq = create_singlethread_workqueue(SSSNIC_NIC_DEV_WQ_NAME);
	if (!nic_dev->workq) {
...
static void sss_nic_set_default_link_follow(struct sss_nic_dev *nic_dev)
{
	...
	if (g_link_follow_status >= SSSNIC_LINK_FOLLOW_STATUS_MAX)
		return;

	ret = sss_nic_set_link_follow_state(nic_dev, g_link_follow_status);
	if (ret == SSS_MGMT_CMD_UNSUPPORTED)
		nic_warn(SSSNIC_DEV_TO_DEV(nic_dev),
...
static int sss_nic_set_default_feature_to_hw(struct sss_nic_dev *nic_dev)
{
	...
	if (ret != 0) {
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to set nic feature\n");
		return ret;
	}
	...
	if (ret != 0) {
		...
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to set netdev feature\n");
		return ret;
	}

	if (SSSNIC_SUPPORT_RXQ_RECOVERY(nic_dev->nic_io))
		set_bit(SSSNIC_RXQ_RECOVERY, &nic_dev->flags);
...
#endif
		nic_err(SSSNIC_DEV_TO_DEV(nic_dev), "Fail to register netdev\n");
		return -ENOMEM;
	}
...
#define SSSNIC_NIC_DRV_DESC "Intelligent Network Interface Card Driver"

static __init int sss_nic_nic_lld_init(void)
{
	int ret;

	pr_info("%s - version %s\n", SSSNIC_NIC_DRV_DESC,
		SSSNIC_NIC_DRV_VERSION);
...
static __exit void sss_nic_nic_lld_exit(void)
{
	sss_unregister_uld(SSS_SERVICE_TYPE_NIC);
}

#ifndef _LLT_TEST_
module_init(sss_nic_nic_lld_init);
module_exit(sss_nic_nic_lld_exit);
#endif

MODULE_AUTHOR("steven.song@3snic.com");
MODULE_DESCRIPTION("3SNIC Network Interface Card Driver");
MODULE_VERSION(SSSNIC_NIC_DRV_VERSION);
MODULE_LICENSE("GPL");
