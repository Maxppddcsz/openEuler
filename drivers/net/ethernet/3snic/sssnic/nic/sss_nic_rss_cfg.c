...
static int sss_nic_set_rss_type_by_ctrlq(struct sss_nic_dev *nic_dev,
					 struct sss_nic_rss_type rss_type, u32 ctx)
{
	int ret;
	u64 output_param = 0;
...
static int sss_nic_set_rss_type_by_mbx(struct sss_nic_dev *nic_dev,
				       struct sss_nic_rss_type type, u32 ctx)
{
...
int sss_nic_set_rss_type(struct sss_nic_dev *nic_dev, struct sss_nic_rss_type rss_type)
{
	...
	ret = sss_nic_set_rss_type_by_mbx(nic_dev, rss_type, ctx);
	if (ret == SSS_MGMT_CMD_UNSUPPORTED)
		ret = sss_nic_set_rss_type_by_ctrlq(nic_dev, rss_type, ctx);
			     ...
			     u8 cos_num, u8 *cos_map, u16 qp_num, u8 rss_en)
{
	...
	/* micro code required: cos number should be power of 2 */
