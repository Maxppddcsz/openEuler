...
static u16 g_qp_num;
module_param(g_qp_num, ushort, 0444);
MODULE_PARM_DESC(g_qp_num, "Number of Queue Pairs (default=0)");
...
static void sss_nic_init_qp_num(struct sss_nic_dev *nic_dev)
{
	...
	if (default_qp_num != 0 && default_qp_num < nic_dev->max_qp_num)
		qp_num = default_qp_num;

	if (g_qp_num > nic_dev->max_qp_num)
...
void sss_nic_init_rss_key(struct sss_nic_dev *nic_dev)
{
...
static int sss_nic_set_rss_key_to_hw(struct sss_nic_dev *nic_dev)
{
	...
	ret = sss_nic_config_rss_to_hw(nic_dev, 0, cos_map,
				       nic_dev->qp_res.qp_num, 0);
	if (ret != 0)
		return ret;
					...
					struct sss_nic_rss_type *rss_type)
{
	...
	if ((cmd->data & (RXH_L4_B_0_1 | RXH_L4_B_2_3)) == 0)
		rss_l4_en = 0;
	else if ((cmd->data & (RXH_L4_B_0_1 | RXH_L4_B_2_3)) ==
...
static inline int sss_nic_check_cmd_data(struct ethtool_rxnfc *cmd)
{
	 ...
	 */
	if (cmd->data & ~(RXH_IP_DST | RXH_IP_SRC | RXH_L4_B_0_1 |
			  RXH_L4_B_2_3))
		return -EINVAL;

	/* We need at least the IP SRC and DEST fields for hashing */
	if (!(cmd->data & RXH_IP_DST) || !(cmd->data & RXH_IP_SRC))
		return -EINVAL;
...
{
	if (rss_opt != 0)
		cmd->data |= RXH_L4_B_2_3 | RXH_L4_B_0_1;
}
