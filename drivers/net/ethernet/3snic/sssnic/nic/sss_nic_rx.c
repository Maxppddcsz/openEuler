...
static void sss_nic_rx_reuse_dma_page(struct sss_nic_rq_desc *rq_desc,
				      struct sss_nic_rx_desc *old_rqe_desc)
{
	u16 pi = rq_desc->backup_pi;
	struct sss_nic_rx_desc *new_rqe_desc;

	new_rqe_desc = &rq_desc->rx_desc_group[pi++];

	rq_desc->backup_pi = (pi < rq_desc->q_depth) ? pi : 0;
...
static bool sss_nic_skb_add_rx_frag(struct sss_nic_rq_desc *rq_desc,
				    struct sss_nic_rx_desc *rx_desc, struct sk_buff *skb, u32 size)
{
	u8 *vaddr;
	struct page *page;
	...
	prefetch(vaddr + L1_CACHE_BYTES);
#endif

	dma_sync_single_range_for_cpu(rq_desc->dev, rx_desc->buf_daddr,
				      rx_desc->page_offset, rq_desc->buf_len, DMA_FROM_DEVICE);
...
static void sss_nic_combine_skb(struct sss_nic_rq_desc *rq_desc,
				struct sk_buff *head_skb, u8 sge_num, u32 pkt_size)
{
	struct sss_nic_rx_desc *rx_desc = NULL;
	...
	while (tmp_sge_num > 0) {
		rx_desc = &rq_desc->rx_desc_group[ci];
		if (unlikely(tmp_pkt_sz > rq_desc->buf_len)) {
			size = rq_desc->buf_len;
		...
		if (likely(sss_nic_skb_add_rx_frag(rq_desc, rx_desc, skb, size)))
			sss_nic_rx_reuse_dma_page(rq_desc, rx_desc);
		else
			dma_unmap_page(rq_desc->dev, rx_desc->buf_daddr,
		...
		tmp_sge_num--;
		frag_num++;
		ci = (ci + 1) & rq_desc->qid_mask;
	}
}

static struct sk_buff *sss_nic_fetch_one_skb(struct sss_nic_rq_desc *rq_desc,
					     u32 pkt_size)
{
	struct net_device *netdev = rq_desc->netdev;
	...
	head_skb = netdev_alloc_skb_ip_align(netdev, SSSNIC_RX_HDR_SIZE);
	...
	while (unlikely(skb_num > 1)) {
		next_skb = netdev_alloc_skb_ip_align(netdev, SSSNIC_RX_HDR_SIZE);
...
static unsigned int sss_nic_eth_get_headlen(unsigned char *data,
...
static void sss_nic_pull_tail(struct sk_buff *skb)
{
	...
	len = eth_get_headlen(skb->dev, data, SSSNIC_RX_HDR_SIZE);
#else
	len = eth_get_headlen(data, SSSNIC_RX_HDR_SIZE);
#endif

#else
	len = sss_nic_eth_get_headlen(data, SSSNIC_RX_HDR_SIZE);
#endif

	skb_copy_to_linear_data(skb, data, ALIGN(len, sizeof(long)));

	skb_frag_size_sub(frag, (int)len);
	skb_frag_off_add(frag, (int)len);

	skb->tail += len;
	skb->data_len -= len;
}

static void sss_nic_check_rx_csum(struct sss_nic_rq_desc *rq_desc,
				  u32 offload_type, u32 status, struct sk_buff *skb)
{
	struct net_device *netdev = rq_desc->netdev;
	...
	if (unlikely(chksum_err != 0)) {
		if ((chksum_err &
		    (SSSNIC_RX_CSUM_HW_CHECK_NONE | SSSNIC_RX_CSUM_IPSU_OTHER_ERR)) == 0)
			rq_desc->stats.csum_errors++;
		skb->ip_summed = CHECKSUM_NONE;
		return;
	}

	if (ip_type == SSSNIC_RX_INVALID_IP_TYPE ||
	...
	if (pkt_type == SSSNIC_RX_TCP_PKT || pkt_type == SSSNIC_RX_UDP_PKT ||
...
static void sss_nic_check_rx_gso(struct sss_nic_rq_desc *rq_desc,
				 u32 offload_type, struct sk_buff *skb)
{
	struct net_device *netdev = rq_desc->netdev;
...
static inline void sss_nic_update_gso_params(struct sk_buff *skb,
					     u16 gso_segs)
{
	struct ethhdr *ether = (struct ethhdr *)(skb->data);
	__be16 protocol;

	protocol = __vlan_get_protocol(skb, ether->h_proto, NULL);

	skb_shinfo(skb)->gso_segs = gso_segs;
	skb_shinfo(skb)->gso_size = (u16)DIV_ROUND_UP((skb->len - skb_headlen(skb)),
						      gso_segs);
	skb_shinfo(skb)->gso_type = (protocol == htons(ETH_P_IP)) ?
				    SKB_GSO_TCPV4 : SKB_GSO_TCPV6;
}

#ifdef HAVE_XDP_SUPPORT
static void sss_nic_xdp_update_rx_info(struct sss_nic_rq_desc *rq_desc,
				       u16 wqe_num)
{
	struct sss_nic_rx_desc *rx_desc = NULL;
	...
	while (wqe_num > 0) {
		...
		if (likely(page_to_nid(rx_desc->page) == numa_node_id()))
			sss_nic_rx_reuse_dma_page(rq_desc, rx_desc);
...
static int sss_nic_bpf_prog_run_xdp(struct sss_nic_rq_desc *rq_desc,
				    u32 pkt_size)
{
	struct bpf_prog *xdp_prog = NULL;
	struct sss_nic_rx_desc *rx_desc = NULL;
	struct xdp_buff xdp;
	int result = SSSNIC_XDP_PKT_PASS;
	u16 wqe_num = 1;
	u8 *data = NULL;
	u32 ret;
	...
	if (unlikely(pkt_size > rq_desc->buf_len)) {
		SSSNIC_RQ_STATS_INC(rq_desc, large_xdp_pkts);
		wqe_num = (u16)(pkt_size >> rq_desc->buff_size_shift) +
	...
	rx_desc = &rq_desc->rx_desc_group[rq_desc->ci & rq_desc->qid_mask];
	data = (u8 *)page_address(rx_desc->page) + rx_desc->page_offset;
	prefetch(data);
	dma_sync_single_range_for_cpu(rq_desc->dev, rx_desc->buf_daddr,
				      rx_desc->page_offset, rq_desc->buf_len, DMA_FROM_DEVICE);
	xdp.data = data;
	...
	xdp.frame_sz = rq_desc->buf_len;
#endif
#ifdef HAVE_XDP_DATA_META
	xdp_set_data_meta_invalid(&xdp);
#endif
	prefetchw(xdp.data_hard_start);
	ret = bpf_prog_run_xdp(xdp_prog, &xdp);
	if (ret != XDP_PASS) {
		result = SSSNIC_XDP_PKT_DROP;
		if (ret != XDP_DROP)
#ifdef HAVE_BFP_WARN_NETDEV_PARAM
			bpf_warn_invalid_xdp_action(rq_desc->netdev, xdp_prog, ret);
#else
...
out:
	if (result == SSSNIC_XDP_PKT_DROP) {
		SSSNIC_RQ_STATS_INC(rq_desc, xdp_dropped);
		sss_nic_xdp_update_rx_info(rq_desc, wqe_num);
	}
				   ...
				   u32 vlan_len, u32 status)
{
	...
	xdp_result = sss_nic_bpf_prog_run_xdp(rq_desc, pkt_len);
	if (xdp_result == SSSNIC_XDP_PKT_DROP)
		return 0;
#endif

	skb = sss_nic_fetch_one_skb(rq_desc, pkt_len);
	if (unlikely(!skb)) {
	...
	if (unlikely(test_bit(SSSNIC_LP_TEST, &nic_dev->flags) != false))
	...
	if (skb_has_frag_list(skb) != false) {
...
int sss_nic_rx_poll(struct sss_nic_rq_desc *rq_desc, int budget)
{
	...
	while (likely(pkts < budget)) {
		...
		if (wqe_num >= nic_dev->lro_replenish_thld)
			break;
	}
