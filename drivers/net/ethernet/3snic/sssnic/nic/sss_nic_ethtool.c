				...
				struct ethtool_drvinfo *drvinfo)
{
	...
	strlcpy(drvinfo->driver, SSSNIC_NIC_DRV_NAME, sizeof(drvinfo->driver));
	strlcpy(drvinfo->version, SSSNIC_NIC_DRV_VERSION, sizeof(drvinfo->version));
...
static void sss_nic_set_msglevel(struct net_device *netdev, u32 msg_enable)
{
	...
	nicif_info(nic_dev, drv, netdev, "Change msg_enable from 0x%x to 0x%x\n",
				  ...
				  struct ethtool_pauseparam *pauseparam)
{
	...
	nicif_info(nic_dev, drv, netdev, "Set pauseparam option, rx: %s, tx: %s\n",
