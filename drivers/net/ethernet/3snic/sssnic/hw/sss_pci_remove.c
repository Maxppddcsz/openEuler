...
static void sss_wait_sriov_cfg_complete(struct sss_pci_adapter *adapter)
{
	...
	clear_bit(SSS_SRIOV_DEFAULT_STATE, &info->state);
...
void sss_deinit_function(struct pci_dev *pdev)
{
	...
	/* When deinit function, disable mgmt initiative report events firstly,
	 * flush mgmt work-queue secondly.
	 */
