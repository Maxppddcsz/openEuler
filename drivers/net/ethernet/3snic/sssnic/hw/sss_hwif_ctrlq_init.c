...
static unsigned int g_wq_page_order = SSS_MAX_WQ_PAGE_SIZE_ORDER;
module_param(g_wq_page_order, uint, 0444);
MODULE_PARM_DESC(g_wq_page_order,
		 "Set wq page size order, wq page size is 4K * (2 ^ g_wq_page_order) - default is 8");
...
static int sss_create_ctrlq_wq(struct sss_ctrlq_info *ctrlq_info)
{
	...
	for (q_type = SSS_CTRLQ_SYNC; q_type < ctrlq_info->num; q_type++) {
	...
	if (!SSS_WQ_IS_0_LEVEL_CLA(&ctrlq_info->ctrlq[SSS_CTRLQ_SYNC].wq)) {
		...
		for (i = SSS_CTRLQ_SYNC; i < ctrlq_info->num; i++)
...
destroy_wq:
	for (i = SSS_CTRLQ_SYNC; i < q_type; i++)
...
static void sss_destroy_ctrlq_wq(struct sss_ctrlq_info *ctrlq_info)
{
	...
	for (type = SSS_CTRLQ_SYNC; type < ctrlq_info->num; type++)
...
int sss_reinit_ctrlq_ctx(struct sss_hwdev *hwdev)
{
	...
	for (ctrlq_type = SSS_CTRLQ_SYNC; ctrlq_type < ctrlq_info->num; ctrlq_type++) {
...
static enum sss_process_ret sss_check_ctrlq_stop_handler(void *priv_data)
{
	...
	ctrlq_type = SSS_CTRLQ_SYNC;
	for (; ctrlq_type < ctrlq_info->num; ctrlq_type++) {
...
static int sss_init_ctrlq_page_size(struct sss_hwdev *hwdev)
{
	...
	if (g_wq_page_order > SSS_MAX_WQ_PAGE_SIZE_ORDER) {
		sdk_info(hwdev->dev_hdl,
			 "Invalid g_wq_page_order %u out of range, adjust to %d\n",
			 g_wq_page_order, SSS_MAX_WQ_PAGE_SIZE_ORDER);
		g_wq_page_order = SSS_MAX_WQ_PAGE_SIZE_ORDER;
	}

	hwdev->wq_page_size = SSS_GET_WQ_PAGE_SIZE(g_wq_page_order);
	ret = sss_chip_set_wq_page_size(hwdev, sss_get_global_func_id(hwdev),
					SSS_GET_WQ_PAGE_SIZE(g_wq_page_order));
	if (ret != 0) {
...
void sss_free_ctrlq_channel(struct sss_hwdev *hwdev)
{
...
void sss_ctrlq_flush_sync_cmd(struct sss_hwdev *hwdev)
{
	...
	u16 i;
	u16 id;
	struct sss_wq *nic_wq = NULL;
	struct sss_ctrlq *nic_ctrlq = NULL;
	struct sss_ctrlq_cmd_info *info = NULL;

	nic_ctrlq = &hwdev->ctrlq_info->ctrlq[SSS_CTRLQ_SYNC];

	spin_lock_bh(&nic_ctrlq->ctrlq_lock);
	nic_wq = &nic_ctrlq->wq;
	id = nic_wq->pi + nic_wq->q_depth - nic_wq->ci;
	cnt = (u16)SSS_WQ_MASK_ID(nic_wq, id);
	ci = nic_wq->ci;

	for (i = 0; i < cnt; i++) {
		info = &nic_ctrlq->cmd_info[SSS_WQ_MASK_ID(nic_wq, ci + i)];
		sss_flush_ctrlq_sync_cmd(info);
	}

	spin_unlock_bh(&nic_ctrlq->ctrlq_lock);
}
