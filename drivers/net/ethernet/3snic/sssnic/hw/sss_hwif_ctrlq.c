				       ...
				       struct sss_ctrlq_wqe *wqe, u16 ci)
{
	...
	/*write wqe_ctrl and sss_ctrlq msg type*/
	wmb();
...
static void sss_ctrlq_sync_cmd_handler(struct sss_ctrlq *ctrlq,
				       struct sss_ctrlq_wqe *wqe, u16 ci)
{
...
void sss_ctrlq_ceq_handler(void *dev, u32 data)
{
	...
	if (ctrlq_wqe) {
		...
		if (info->msg_type < SSS_MSG_TYPE_NONE ||
			...
			sss_ctrlq_sync_cmd_handler(ctrlq, ctrlq_wqe, ci);
			return;
		}
