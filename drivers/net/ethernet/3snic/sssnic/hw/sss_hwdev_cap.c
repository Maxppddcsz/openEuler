			      ...
			      struct sss_service_cap *svc_cap, struct sss_cmd_dev_cap_cfg *cmd_cap)
{
	...
	sf_svc_attr->ft_en = !!(cmd_cap->sf_svc_attr & SSS_SF_SVC_FT_BIT);
...
int sss_init_capability(struct sss_hwdev *hwdev)
{
	...
	mgmt_info->svc_cap.sf_svc_attr.ft_pf_en = false;
	mgmt_info->svc_cap.sf_svc_attr.rdma_pf_en = false;

	if (type != SSS_FUNC_TYPE_PF && type != SSS_FUNC_TYPE_VF && type != SSS_FUNC_TYPE_PPF) {
