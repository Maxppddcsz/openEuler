...
alloc_host_image_err:
	link_dev->switch_cfg = SSS_FW_CFG_DEFAULT_INDEX;
	link_dev->activate_fw = SSS_FW_CFG_DEFAULT_INDEX;
					      ...
					      struct devlink_param_gset_ctx *param_ctx)
{
	...
	param_ctx->val.vu8 = link_dev->activate_fw;
					      ...
					      struct devlink_param_gset_ctx *param_ctx)
{
	...
	link_dev->activate_fw = param_ctx->val.vu8;
	sdk_info(hwdev->dev_hdl, "Begin activate fw\n");

	ret = sss_chip_activate_firmware(hwdev, link_dev->activate_fw);
	if (ret != 0) {
		sdk_err(hwdev->dev_hdl, "Fail to activate firmware, ret: %d\n", ret);
		return ret;
	}

	sdk_info(hwdev->dev_hdl, "End activate fw\n");
					 ...
					 struct devlink_param_gset_ctx *param_ctx)
{
	...
	param_ctx->val.vu8 = link_dev->switch_cfg;
					 ...
					 struct devlink_param_gset_ctx *param_ctx)
{
	...
	link_dev->switch_cfg = param_ctx->val.vu8;
	sdk_info(hwdev->dev_hdl, "Begin switch cfg");

	ret = sss_chip_switch_config(hwdev, link_dev->switch_cfg);
	if (ret != 0) {
						...
						struct netlink_ext_ack *ext_ack)
{
	...
	if (param_val.vu8 > SSS_FW_CFG_MAX_INDEX) {
		sdk_err(hwdev->dev_hdl, "Fw cfg index out of range [0,7]\n");
		NL_SET_ERR_MSG_MOD(ext_ack, "Fw cfg index out of range [0,7]\n");
		return -ERANGE;
	}
...
int sss_init_devlink(struct sss_hwdev *hwdev)
{
	...
	hwdev->devlink_dev->switch_cfg = SSS_FW_CFG_DEFAULT_INDEX;
	hwdev->devlink_dev->activate_fw = SSS_FW_CFG_DEFAULT_INDEX;
