...
#define SSS_CHIP_NODE_HOLD_TIMEOUT		(10 * 60 * 1000) /* 10minutes */
#define SSS_WAIT_CHIP_NODE_CHANGED			(10 * 60 * 1000) /* 10minutes */
...
void sss_wait_adapter_free(struct sss_pci_adapter *adapter)
{
	unsigned long end;

	end = jiffies + msecs_to_jiffies(SSS_ADAPTER_CNT_TIMEOUT);
	do {
		if (!atomic_read(&adapter->ref_cnt))
			return;

		/* if sleep 10ms, use usleep_range to be more precise */
		usleep_range(SSS_WAIT_ADAPTER_USLEEP_MIN,
			     SSS_WAIT_ADAPTER_USLEEP_MAX);
