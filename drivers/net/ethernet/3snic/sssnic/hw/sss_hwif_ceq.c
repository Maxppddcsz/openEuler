...
#define SSS_DEF_CEQ_DEPTH					0x10000
...
static uint g_ceq_depth = SSS_DEF_CEQ_DEPTH;
module_param(g_ceq_depth, uint, 0444);
MODULE_PARM_DESC(g_ceq_depth,
		 "ceq depth, valid range is " __stringify(SSS_MIN_CEQ_DEPTH)
		 " - " __stringify(SSS_MAX_CEQ_DEPTH));

static uint g_tasklet_ceqe_num = SSS_TASK_PROCESS_EQE_LIMIT;
module_param(g_tasklet_ceqe_num, uint, 0444);
MODULE_PARM_DESC(g_tasklet_ceqe_num,
...
void sss_init_ceqe_desc(void *data)
{
	...
	for (i = 0; i < ceq->len; i++) {
		sss_ceqe = SSS_GET_CEQ_ELEM(ceq, i);
		*(sss_ceqe) = cpu_to_be32(SSS_EQ_WRAPPED(ceq));
	}
...
static void sss_ceqe_handler(struct sss_eq *ceq, u32 ceqe)
{
	...
	if (ceq_info->event_handler[ceq_event] &&
		...
		ceq_info->event_handler[ceq_event](
			ceq_info->event_handler_data[ceq_event], ceqe_data);
...
static bool sss_ceq_irq_handle(struct sss_eq *ceq)
{
	...
	for (i = 0; i < g_tasklet_ceqe_num; i++) {
			...
			struct sss_irq_desc *irq_array, u16 irq_num)
{
	...
	if (g_tasklet_ceqe_num == 0) {
		sdk_warn(hwdev->dev_hdl,
			"Invalid g_tasklet_ceqe_num can not be zero, adjust to %d\n",
			SSS_TASK_PROCESS_EQE_LIMIT);
		g_tasklet_ceqe_num = SSS_TASK_PROCESS_EQE_LIMIT;
	}

	if (g_ceq_depth < SSS_MIN_CEQ_DEPTH || g_ceq_depth > SSS_MAX_CEQ_DEPTH) {
		sdk_warn(hwdev->dev_hdl,
			 "Invalid g_ceq_depth %u out of range, adjust to %d\n",
			 g_ceq_depth, SSS_DEF_CEQ_DEPTH);
		g_ceq_depth = SSS_DEF_CEQ_DEPTH;
	}
...
int sss_hwif_init_ceq(struct sss_hwdev *hwdev)
{
	...
	act_num = sss_alloc_irq(hwdev, SSS_SERVICE_TYPE_INTF, irq, ceq_num);
	if (act_num == 0) {
	...
	ret = sss_init_ceq(hwdev, irq, ceq_num);
	if (ret != 0) {
