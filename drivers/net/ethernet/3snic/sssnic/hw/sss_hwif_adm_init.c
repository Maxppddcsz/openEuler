...
/* ADM_MSG_REQ CSR: 0x0020+api_idx*0x080 */
...
/* ADM_MSG_CTRL CSR: 0x0014+api_idx*0x080 */
...
#define SSS_MGMT_WQ_NAME					"sss_nic_mgmt"
					...
					u32 elem_id)
{
	...
	rsp_paddr = SSS_GET_ADM_MSG_REPLY_PADDR(adm_msg, elem_id);
	...
	elem =
		(struct sss_adm_msg_elem *)SSS_GET_ADM_MSG_ELEM_VADDR(adm_msg, elem_id);
	elem->read.hw_wb_reply_paddr = cpu_to_be64(rsp_paddr);
	ctx->reply_fmt = rsp_vaddr;
	elem->read.hw_msg_paddr = cpu_to_be64(buf_paddr);
	ctx->adm_msg_vaddr = buf_vaddr;
}

static void sss_init_elem_ctx(struct sss_adm_msg *adm_msg,
			      u32 elem_id)
{
...
static void sss_init_adm_msg_elem(struct sss_adm_msg *adm_msg)
{
	u32 i;
	u64 paddr;
	void *vaddr;
...
static void sss_init_adm_msg_param(struct sss_adm_msg *adm_msg,
				   struct sss_hwdev *hwdev, u8 type)
{
	adm_msg->hwdev = hwdev;
			    ...
			    struct sss_adm_msg *adm_msg)
{
	...
	sss_init_adm_msg_param(adm_msg, hwdev, SSS_ADM_MSG_WRITE_TO_MGMT_MODULE);

	ret = sss_alloc_adm_msg_buf(adm_msg);
	if (ret != 0) {
		sdk_err(hwdev->dev_hdl, "Fail to init adm cmd buf\n");
		return ret;
	}

	sss_init_adm_msg_elem(adm_msg);

	ret = sss_chip_init_adm_msg(adm_msg);
	if (ret != 0) {
		sdk_err(hwdev->dev_hdl, "Fail to init adm cmd\n");
