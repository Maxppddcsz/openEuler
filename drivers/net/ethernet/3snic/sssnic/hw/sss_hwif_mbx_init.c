...
static void sss_mbx_init_mq_param(struct sss_mbx_dma_queue *mq)
{
	mq->depth = SSS_MBX_DMA_MSG_QUEUE_DEPTH;
}
...
static int sss_init_mbx_info(struct sss_mbx *mbx)
{
	...
	sss_mbx_init_mq_param(&mbx->sync_msg_queue);
	sss_mbx_init_mq_param(&mbx->async_msg_queue);
...
void sss_hwif_deinit_mbx(struct sss_hwdev *hwdev)
{
	...
	/* destroy workqueue before free related mbx resources in case of
				  ...
				  struct sss_recv_mbx *recv_mbx, int ret, u16 size, u16 src_func_id)
{
	...
	data_size = (size == 0) ? SSS_MBX_MSG_NO_DATA_SIZE : size;
				 ...
				 struct sss_recv_mbx *recv_mbx)
{
	...
	if (SSS_SRC_IS_PF_OR_PPF(hwdev, src_func_id)) {
		...
		ret = sss_recv_pf_from_vf_mbx_handler(
			mbx, recv_mbx, src_func_id, resp_buf, &size);
	}
				 ...
				 const struct sss_msg_desc *msg_desc)
{
	...
	if (msg_desc->msg_info.msg_id == mbx->send_msg_id &&
		...
		sdk_err(SSS_TO_HWDEV(mbx)->dev_hdl,
			"Mail resp timeout, current send msg_id(0x%x), recv msg_id(0x%x), status(0x%x)\n",
