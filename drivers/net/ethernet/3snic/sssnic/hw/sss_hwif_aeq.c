...
static uint g_aeq_depth = SSS_DEF_AEQ_DEPTH;
module_param(g_aeq_depth, uint, 0444);
MODULE_PARM_DESC(g_aeq_depth,
...
static void sss_chip_set_aeq_intr(struct sss_eq *aeq)
{
	...
	intr = SSS_SET_AEQ_CTRL_0(SSS_EQ_IRQ_ID(aeq), INTR_ID) |
...
{
	u32 i;
	struct sss_aeq_elem *sss_aeqe = NULL;
	struct sss_eq *aeq = (struct sss_eq *)data;

	for (i = 0; i < aeq->len; i++) {
		sss_aeqe = SSS_GET_AEQ_ELEM(aeq, i);
		sss_aeqe->desc = cpu_to_be32(SSS_EQ_WRAPPED(aeq));
	}

	/*write all sss_aeq_elem desc*/
	wmb();
}
			...
			u16 aeq_num, struct sss_irq_desc *irq)
{
	...
	if (g_aeq_depth < SSS_MIN_AEQ_DEPTH || g_aeq_depth > SSS_MAX_AEQ_DEPTH) {
		sdk_warn(hwdev->dev_hdl, "Invalid g_aeq_depth value %u, adjust to %d\n",
			 g_aeq_depth, SSS_DEF_AEQ_DEPTH);
		g_aeq_depth = SSS_DEF_AEQ_DEPTH;
	}
