...
enum {
	...
	XSC_CMD_OP_IOCTL_GET_CC_STAT = 0x104a,
...
enum {
	XSC_CMD_RESP_LINKSPEED_MODE_25G = 0x0,
	XSC_CMD_RESP_LINKSPEED_MODE_100G = 0x1
};
...
struct xsc_cmd_init_hca_mbox_in {
	struct xsc_inbox_hdr	hdr;
	u8			rsvd0[2];
	__be16			profile;
	u8			rsvd1[4];
...
struct xsc_cmd_init_hca_mbox_out {
	struct xsc_outbox_hdr	hdr;
	u8			rsvd[8];
};

struct xsc_cmd_teardown_hca_mbox_in {
	struct xsc_inbox_hdr	hdr;
	u8			rsvd0[2];
	__be16			profile;
	u8			rsvd1[4];
...
struct xsc_cmd_teardown_hca_mbox_out {
	struct xsc_outbox_hdr	hdr;
	u8			rsvd[8];
};
...
struct xsc_nic_vport_context {
	u32		min_wqe_inline_mode:3;
	u32		disable_mc_local_lb:1;
	u32		disable_uc_local_lb:1;
	u32		roce_en:1;

	u32		arm_change_event:1;
	u32		event_on_mtu:1;
	u32		event_on_promisc_change:1;
	u32		event_on_vlan_change:1;
	u32		event_on_mc_address_change:1;
	u32		event_on_uc_address_change:1;
	u32		affiliation_criteria:4;
	u32		affiliated_vhca_id;
	...
	u32		qkey_violation_counter;

	u16		promisc_uc:1;
	u16		promisc_mc:1;
	u16		promisc_all:1;
	u16		allowed_list_type:3;
	u16		allowed_list_size:10;

	u8		permanent_address[6];
	u64		current_uc_mac_address[0];
};
...
struct xsc_modify_nic_vport_field_select {
	...
	u32		change_event:1;
	u32		promisc:1;
	u32		permanent_address:1;
	u32		addresses_list:1;
	u32		roce_en:1;
	u32		rsvd:20;
};
...
struct xsc_hw_stats {
	/*by mac port*/
	uint64_t rdma_tx_pkts;
	uint64_t rdma_tx_bytes;
	uint64_t rdma_rx_pkts;
	...
	uint64_t out_of_buffer;
	uint64_t rnr_nak_retry_err;
	uint64_t local_ack_timeout_err;
	uint64_t rx_read_requests;
	uint64_t rx_write_requests;
	uint64_t duplicate_requests;
	uint64_t rdma_tx_pkts_func;
	uint64_t rdma_tx_payload_bytes;
	uint64_t rdma_rx_pkts_func;
	uint64_t rdma_rx_payload_bytes;
	/*by global*/
	uint64_t rdma_loopback_pkts;
	uint64_t rdma_loopback_bytes;
};
...
struct xsc_pcp_pmt_get {
	uint8_t prio_map[QOS_PCP_MAX + 1];
	uint8_t max_prio;
	uint8_t rsvd[7];
};
...
struct xsc_cc_cmd_stat {
	uint32_t cnp_handled;
	uint32_t alpha_recovery;
	uint32_t reset_timeout;
	uint32_t reset_bytecount;
};
...
enum {
	XSC_TBM_CAP_IPAT_BYPASS = 0,
	XSC_TBM_CAP_PCT_BYPASS,
	XSC_TBM_CAP_HASH_PPH,
	XSC_TBM_CAP_RSS,
	XSC_TBM_CAP_BC_BYPASS,
};
...
struct xsc_enable_nic_hca_resp {
	__be16	bomt_idx;
};

struct xsc_cmd_enable_nic_hca_mbox_out {
	struct xsc_outbox_hdr		hdr;
	struct xsc_enable_nic_hca_resp	res;
	u8	status;
	u8	rsvd0;
};
...
struct xsc_cmd_disable_nic_hca_mbox_in {
	...
	struct xsc_broadcast_dis_attr	bc;
};
...
struct rtt_stats {
	uint64_t rtt_succ_snd_req_cnt;
	uint64_t rtt_succ_snd_rsp_cnt;
	uint64_t rtt_fail_snd_req_cnt;
	uint64_t rtt_fail_snd_rsp_cnt;
	uint64_t rtt_rcv_req_cnt;
	uint64_t rtt_rcv_rsp_cnt;
	uint64_t rtt_rcv_unk_cnt;
	uint64_t rtt_grp_invalid_cnt;
};
