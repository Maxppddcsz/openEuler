...
MODULE_PARM_DESC(roce_qpc_rsv_mode, "for roce reserve 4k qpc(qpn) (default=0, 0-rsv:2, 1-rsv:4k, 2-rsv:200k+2)");
...
static s32 cqm_set_fake_vf_timer(struct tag_cqm_handle *cqm_handle)
{
	...
	for (i = 0; i < (u32)child_func_number; i++) {
		ret = cqm_set_fake_vf_child_timer(cqm_handle, cqm_handle->fake_cqm_handle[i], true);
		if (ret != CQM_SUCCESS)
			goto err;
	}
...
static void cqm_capability_init_cap_print(struct hinic3_hwdev *handle)
{
	...
	cqm_info(handle->dev_hdl, "Cap init: srqc_number 0x%x, srqc_reserved 0x%x, srqc_basic_size 0x%x\n",
...
void cqm_scq_callback(void *ex_handle, u32 ceqe_data)
{
	...
	CQM_PTR_CHECK_NO_RET(ex_handle, CQM_PTR_NULL(scq_callback_ex_handle));

	atomic_inc(&handle->hw_stats.cqm_stats.cqm_scq_callback_cnt);

	cqm_handle = (struct tag_cqm_handle *)(handle->cqm_hdl);
	CQM_PTR_CHECK_NO_RET(cqm_handle, CQM_PTR_NULL(scq_callback_cqm_handle));

	cqm_dbg("Event: %s, ceqe_data=0x%x\n", __func__, ceqe_data);
	obj = cqm_object_get(ex_handle, CQM_OBJECT_NONRDMA_SCQ,
			     CQM_CQN_FROM_CEQE(ceqe_data), true);
	CQM_PTR_CHECK_NO_RET(obj, CQM_PTR_NULL(scq_callback_obj));
...
void cqm_ecq_callback(void *ex_handle, u32 ceqe_data)
{
	...
	CQM_PTR_CHECK_NO_RET(ex_handle, CQM_PTR_NULL(ecq_callback_ex_handle));

	atomic_inc(&handle->hw_stats.cqm_stats.cqm_ecq_callback_cnt);

	cqm_handle = (struct tag_cqm_handle *)(handle->cqm_hdl);
	CQM_PTR_CHECK_NO_RET(cqm_handle, CQM_PTR_NULL(ecq_callback_cqm_handle));

	obj = cqm_object_get(ex_handle, CQM_OBJECT_SERVICE_CTX,
			     CQM_XID_FROM_CEQE(ceqe_data), true);
	CQM_PTR_CHECK_NO_RET(obj, CQM_PTR_NULL(ecq_callback_obj));
...
void cqm_nocq_callback(void *ex_handle, u32 ceqe_data)
{
	...
	CQM_PTR_CHECK_NO_RET(ex_handle, CQM_PTR_NULL(nocq_callback_ex_handle));

	atomic_inc(&handle->hw_stats.cqm_stats.cqm_nocq_callback_cnt);

	cqm_handle = (struct tag_cqm_handle *)(handle->cqm_hdl);
	CQM_PTR_CHECK_NO_RET(cqm_handle, CQM_PTR_NULL(nocq_callback_cqm_handle));

	obj = cqm_object_get(ex_handle, CQM_OBJECT_SERVICE_CTX,
			     CQM_XID_FROM_CEQE(ceqe_data), true);
	CQM_PTR_CHECK_NO_RET(obj, CQM_PTR_NULL(nocq_callback_obj));
