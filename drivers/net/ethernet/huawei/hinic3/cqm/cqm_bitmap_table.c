	...
	for (i = 0; i < buf->buf_number; i++) {
		buf->buf_list[i].va =
		    cqm_get_secure_mem_pages(handle,
					     ...
					     &buf->buf_list[i].pa);
		if (!buf->buf_list[i].va) {
			cqm_err(handle->dev_hdl, CQM_ALLOC_FAIL(cqm_get_secure_mem_pages));
			break;
		}
	}
...
{
	struct hinic3_hwdev *handle = cqm_handle->ex_handle;
	cqm_cla_cache_invalid_cmd_s *cla_cache_invalid_cmd = NULL;
	s32 ret;

	cla_cache_invalid_cmd = (cqm_cla_cache_invalid_cmd_s *)(buf_in->buf);
	...
	cqm_swab32((u8 *)cla_cache_invalid_cmd,
		   ...
		   (sizeof(cqm_cla_cache_invalid_cmd_s) >> 2));
...
s32 cqm_cla_cache_invalid(struct tag_cqm_handle *cqm_handle, dma_addr_t pa, u32 cache_size)
{
	...
	buf_in->size = sizeof(cqm_cla_cache_invalid_cmd_s);
...
static s32 cqm_single_bitmap_init(struct tag_cqm_bitmap *bitmap)
{
	...
	CQM_PTR_CHECK_RET(bitmap->table, CQM_FAIL,
