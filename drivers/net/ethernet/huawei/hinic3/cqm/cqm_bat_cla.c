...
{
	struct hinic3_hwdev *handle = cqm_handle->ex_handle;
	cqm_bat_update_cmd_s *bat_update_cmd = NULL;
	s32 ret = CQM_FAIL;

	bat_update_cmd = (cqm_bat_update_cmd_s *)(buf_in->buf);
	...
	cqm_swab32((u8 *)bat_update_cmd,
		   sizeof(cqm_bat_update_cmd_s) >> CQM_DW_SHIFT);
...
static s32 cqm_bat_update(struct tag_cqm_handle *cqm_handle)
{
	...
	buf_in->size = sizeof(cqm_bat_update_cmd_s);
...
static s32 cqm_cla_xyz_check(struct tag_cqm_handle *cqm_handle, struct tag_cqm_cla_table *cla_table,
			     u32 *size)
{
	struct hinic3_hwdev *handle = cqm_handle->ex_handle;
	u32 trunk_size = 0;
...
{
	struct hinic3_hwdev *handle = cqm_handle->ex_handle;
	cqm_cla_update_cmd_s *cla_update_cmd = NULL;
	s32 ret = CQM_FAIL;

	cla_update_cmd = (cqm_cla_update_cmd_s *)(buf_in->buf);
	...
	cqm_swab32((u8 *)cla_update_cmd,
		   (sizeof(cqm_cla_update_cmd_s) >> CQM_DW_SHIFT));
			  ...
			  u32 child_index, u8 cla_update_mode)
{
	...
	CQM_PTR_CHECK_RET(buf_in, CQM_FAIL, CQM_ALLOC_FAIL(buf_in));
	buf_in->size = sizeof(cqm_cla_update_cmd_s);
