...
static int test_query_context(struct hinic3_hwdev *handle, char *data, size_t len)
{
	...
	case CQM_OBJECT_MPT:
		...
		in_secmem = IS_ADDR_IN_MEMSEC(qpc_mpt->vaddr, cqm_obj->object_size,
					      (u8 *)info->va_base, (u8 *)info->va_end);
	...
	case CQM_OBJECT_RDMA_SCQ:
		...
		in_secmem = IS_ADDR_IN_MEMSEC(cqm_queue->q_ctx_vaddr, cqm_obj->object_size,
					      (u8 *)info->va_base, (u8 *)info->va_end);
		cqm_info(handle->dev_hdl,
			 "[memsec_dfx]Query %s:0x%x, va=%p %sin secure mem\n",
...
static int cqm_get_secure_mem_cfg(void *dev, struct tag_cqm_secure_mem *info)
{
	...
	vmsec_cfg_ctx_gpa_entry_cmd_s mem_info;
	u16 out_size = sizeof(vmsec_cfg_ctx_gpa_entry_cmd_s);
	int err;
