...
struct comm_cmd_func_reset {
	...
	u16 rsvd1[3];
	u64 reset_flag; /**< reset fuction type flag @see enum func_reset_flag_e */
};
...
typedef enum COMM_FEATURE_NEGO_OPCODE {
	COMM_FEATURE_NEGO_OPCODE_GET = 0,
	COMM_FEATURE_NEGO_OPCODE_SET = 1
} COMM_FEATURE_NEGO_OPCODE_E;
...
struct comm_cmd_func_flr_set {
	...
	u8 isall;	/**< flr type  0: specify PF and associated VF flr, 1: all fuctions flr */
	u32 rsvd;
};
...
struct comm_cmd_set_mqm_limit {
	...
	u32 func_min_rate; /**< current fuction minimum rate,unit:Mbps */
	u32 func_max_rate; /**< current fuction maximum rate,unit:Mbps */
	u8 rsvd[64]; /* Reserved */
};
...
typedef struct {
	...
	mpu_pcie_pf_info_s pf_info[32]; /**< pcie info  @see struct mpu_pcie_pf_info_s */
	u32 bus_id; /**< bus id */
	u32 pf_num; /**< pf index */
} mpu_pcie_device_info_s;
...
struct cmd_update_fw {
	struct comm_info_head head; // 8B
	u16 fw_flag;	 /**< subfirmware flag, bit 0: last slice flag, bit 1 first slice flag */			  // 首片、尾片标识，以及分区标识
...
/* flash检测结果 1: ok   2: error  3:数据不一致 */
...
struct comm_cmd_enable_auto_rst_chip {
	...
	u8 enable;  /**< auto reset status 0: disable auto reset chip 1: enable auto reset chip */
	u8 rsvd[2];
};
...
typedef struct {
	...
	u16 rsvd1[3];
	u32 rsvd2[3];
} comm_cmd_switch_workmode;

#define MIG_NOR_VM_ONE_MAX_SGE_MEM (64 * 8)
#define MIG_NOR_VM_ONE_MAX_MEM (MIG_NOR_VM_ONE_MAX_SGE_MEM + 16)
#define MIG_VM_MAX_SML_ENTRY_NUM 24 /* equal to MIG_BIG_VM2_SML_ENTRY_ID_ST + MIG_BIG_VM_SML_NUM in mpu_mig.c */
...
struct cmd_get_bdf_info_s {
	struct mgmt_msg_head head;
	pf_bdf_info_s  pf_bdf_info[CMD_MAX_MAX_PF_NUM]; /**< pf bdf info
														   *@see struct pf_bdf_info_s */
	vf_bdf_info_s  vf_bdf_info[CMD_MAX_MAX_PF_NUM]; /**< vf bdf info
														   *@see struct vf_bdf_info_s */
	u32 vf_num; /**< vf num */
};
...
typedef struct {
	struct comm_info_head head;
	u8 type;		   /**< operation type 0: set operation, 1: get operation */
	u8 set_bitmap;	 /**< setting type : bit 0: enable task setting, bit 1: task interval setting */
	u8 enable;		 /**< enable task setting :0: disable task, 1: enable task*/
	u8 rsvd;
	u32 time_interval; /**< task time interval value */
} comm_cmd_cpi_tcam_dbg_s;
...
struct comm_info_reg_read_write {
	...
	u32 val_lenth; /**< register value length */
...
typedef struct tag_exc_info {
	 ...
	 */
	EXC_REGS_S reg_info; /**< register info @see struct EXC_REGS_S */
} EXC_INFO_S;
...
typedef struct tag_comm_info_up_lastword {
	...
	EXC_INFO_S stack_info; /**< stack info @see struct EXC_INFO_S */
...
struct comm_cmd_mbox_csr_rd_req {
	struct mgmt_msg_head head;
	struct csr_msg csr_info[MAX_DATA_NUM]; /**< register info
											 * @see struct csr_msg */
	u32 data_num;
};
...
struct comm_cmd_mbox_csr_wt_req {
	struct mgmt_msg_head head;
	struct csr_msg csr_info; /**< register info
							   * @see struct csr_msg */
	u64 value;
};
...
struct comm_cmd_set_virtio_dev {
	struct comm_info_head head;
	comm_virtio_dev_cmd_s virtio_dev_cmd;  /**<  @see struct comm_virtio_dev_cmd_s */
	comm_virtio_dev_ctl_s virtio_dev_ctl;  /**<  @see struct comm_virtio_dev_ctl_s */
};
...
typedef struct tag_ncsi_chan_info {
	...
	u8 rsv[3];
	ncsi_chan_capa_s capabilities; /**< ncsi channel capabilities*/
	g_ncsi_parameters parameters;  /**< ncsi state */
} ncsi_chan_info_s;
...
typedef struct {
	...
	comm_cmd_ncsi_settings setting_info; /**< ncsi setting info
										   * @see struct comm_cmd_ncsi_settings */
} comm_cmd_ncsi_cfg_s;
