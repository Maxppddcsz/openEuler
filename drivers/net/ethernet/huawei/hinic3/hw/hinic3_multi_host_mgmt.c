					 ...
					 void *buf_out, u16 *out_size)
{
	...
	case HINIC3_SW_CMD_SLAVE_HOST_PPF_REGISTER:
		...
		*out_size = sizeof(*slave_host);
		...
		sdk_info(hwdev->dev_hdl, "Slave host registers PPF, host_id: %hhu, ppf_idx: %hhu\n",
	...
	case HINIC3_SW_CMD_SLAVE_HOST_PPF_UNREGISTER:
		...
		sdk_info(hwdev->dev_hdl, "Slave host unregisters PPF, host_id: %hhu, ppf_idx: %hhu\n",
			 slave_host->host_id, slave_host->ppf_idx);
				       ...
				       void *buf_out, u16 *out_size)
{
	...
	*out_size = sizeof(*nic_state);
	...
	sdk_info(hwdev->dev_hdl, "Slave func %hu %s nic\n",
		 nic_state->func_idx,
	...
	if (nic_state->enable) {
		...
		if ((test_bit(nic_state->func_idx, mhost_mgmt->func_nic_en)) &&
			...
			sdk_warn(hwdev->dev_hdl, "VF%hu in vm, delete tap port failed\n",
				 nic_state->func_idx);
				    ...
				    void *buf_out, u16 *out_size)
{
	...
	*out_size = sizeof(*nic_state);
	...
	sdk_info(hwdev->dev_hdl, "Slave func %hu %s vroce\n", nic_state->func_idx,
		 nic_state->enable ? "register" : "unregister");
			     ...
			     u16 *out_size)
{
	...
	if (err)
		sdk_err(hwdev->dev_hdl, "PPF process sw funcs cmd %hu failed, err: %d\n",
			cmd, err);
			   ...
			   u16 in_size, void *buf_out, u16 *out_size)
{
	...
	if (IS_SLAVE_HOST(hwdev)) {
		...
		if (err && err != HINIC3_MBOX_PF_BUSY_ACTIVE_FW)
			sdk_err(hwdev->dev_hdl, "PF mbox mod %d cmd %hhu callback handler err: %d\n",
				mod, cmd, err);
	}
...
int hinic3_register_slave_ppf(struct hinic3_hwdev *hwdev, bool registered)
{
	...
	if (!host_info) {
		sdk_err(hwdev->dev_hdl, "Failed to alloc host info memory\n");
		return -ENOMEM;
	}
			     ...
			     struct hinic3_func_nic_state *state)
{
	...
	if (err == MBOX_ERRCODE_UNKNOWN_DES_FUNC) {
		sdk_warn(hwdev->dev_hdl, "Can not notify func %hu %s state because slave host isn't initialized\n",
			 state->func_idx, state->vroce_flag ? "vroce" : "nic");
	} else if (err || !out_size || nic_state.status) {
		sdk_err(hwdev->dev_hdl, "Failed to set slave host functions %s state, err: %d, out_size: 0x%x, status: 0x%x\n",
				...
				int *opened)
{
	...
	err = hinic3_mbox_to_host_sync(hwdev, HINIC3_MOD_SW_FUNC,
				       ...
				       &out_size, 0, HINIC3_CHANNEL_COMM);
	if (err == MBOX_ERRCODE_UNKNOWN_DES_FUNC) {
		sdk_warn(hwdev->dev_hdl, "Can not get func %hu netdev state because slave host isn't initialized\n",
			 func_idx);
	} else if (err || !out_size || nic_state.status) {
		sdk_err(hwdev->dev_hdl, "Failed to get function netdev state, err: %d, out_size: 0x%x, status: 0x%x\n",
			err, out_size, nic_state.status);
		return -EFAULT;
	}
...
int hinic3_set_func_nic_state(void *hwdev, struct hinic3_func_nic_state *state)
{
	...
	if (IS_MASTER_HOST(ppf_hwdev) &&
		...
		sdk_warn(ppf_hwdev->dev_hdl, "Should disable vroce before disable nic for function %hu\n",
			 state->func_idx);
		return -EFAULT;
	}
	...
	if (err) {
		sdk_err(ppf_hwdev->dev_hdl, "Failed to get function %hu current state, err: %d\n",
			state->func_idx, err);
		return err;
	}
	...
	if (err) {
		sdk_err(ppf_hwdev->dev_hdl, "Failed to get function %hu host id, err: %d\n", state->func_idx, err);
		if (state->vroce_flag)
			return -EFAULT;
	...
	if (err != 0) {
		sdk_err(ppf_hwdev->dev_hdl, "Get slave host %hhu enable failed, ret %d\n", host_id, err);
		return err;
	}
	sdk_info(ppf_hwdev->dev_hdl, "Set slave host %hhu(status: %hhu) func %hu %s %s\n",
				   ...
				   bool *en)
{
	...
	if (hinic3_func_type(hwdev) == TYPE_VF) {
		...
		if (err || !out_size || nic_state.status) {
			sdk_err(hwdev->dev_hdl, "Failed to get func %hu nic state, err: %d, out_size: 0x%x, status: 0x%x\n",
				  ...
				  u8 *en)
{
	...
	if (hinic3_func_type(hwdev) == TYPE_VF) {
		...
		if (err || !out_size || vroce_state.status) {
			sdk_err(hwdev->dev_hdl, "Failed to get func %hu vroce state, err: %d, out_size: 0x%x, status: 0x%x\n",
