...
static int phytium_dwmac_probe(struct platform_device *pdev)
{
	...
	plat->stmmac_clk = clk_register_fixed_rate(&pdev->dev, clk_name,
						   NULL, 0, clk_freq);
	...
	plat->dma_cfg->pblx8 = !fwnode_property_read_bool(fwnode,
							  "snps,no-pbl-x8");
	plat->dma_cfg->aal = fwnode_property_read_bool(fwnode, "snps,aal");
	plat->dma_cfg->fixed_burst = fwnode_property_read_bool(fwnode,
							       "snps,fixed-burst");
	plat->dma_cfg->mixed_burst = fwnode_property_read_bool(fwnode,
							       "snps,mixed-burst");
	...
	if (IS_ERR(stmmac_res.addr)) {
		dev_err(&pdev->dev, "resource map failed.\n");
