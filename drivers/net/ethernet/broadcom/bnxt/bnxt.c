...
static const struct {
	...
	[BCM57602] = { "Broadcom BCM57602 NetXtreme-E 10Gb/25Gb/50Gb Ethernet" },
	[BCM57601] = { "Broadcom BCM57601 NetXtreme-E 10Gb/25Gb/50Gb Ethernet" },
#endif
	...
	[NETXTREME_E_P7_VF] = { "Broadcom BCM5760X NetXtreme-E Virtual Function" },
};
...
static bool __bnxt_tx_int(struct bnxt *bp, struct bnxt_tx_ring_info *txr)
{
	...
	while (RING_TX(cons) != hw_cons) {
		struct bnxt_sw_tx_bd *tx_buf;
		struct sk_buff *skb;
		bool is_ts_pkt;
		int j, last;

		tx_buf = &txr->tx_buf_ring[RING_TX(cons)];
...
static void bnxt_tx_int(struct bnxt *bp, struct bnxt_napi *bnapi)
{
	struct bnxt_tx_ring_info *txr;
	bool more = false;
	int i;
	...
	bnxt_for_each_napi_tx(i, bnapi, txr) {
		if (txr->tx_hw_cons != RING_TX(txr->tx_cons))
		       ...
		       u16 prod, gfp_t gfp)
{
	...
	struct bnxt_sw_rx_bd *rx_buf = &rxr->rx_buf_ring[RING_RX(prod)];
	dma_addr_t mapping;
...
void bnxt_reuse_rx_data(struct bnxt_rx_ring_info *rxr, u16 cons, void *data)
{
	...
	prod_rx_buf = &rxr->rx_buf_ring[RING_RX(prod)];
...
{
	struct rx_bd *rxbd =
		&rxr->rx_agg_desc_ring[RX_AGG_RING(prod)][RX_IDX(prod)];
	...
	rxr->rx_sw_agg_prod = RING_RX_AGG(NEXT_RX_AGG(sw_prod));
...
struct rx_agg_cmp *bnxt_get_tpa_agg_p5(struct bnxt *bp,
				       struct bnxt_rx_ring_info *rxr,
				       u16 agg_id, u16 curr)
{
				   ...
				   u16 start, u32 agg_bufs, bool tpa)
{
	...
	for (i = 0; i < agg_bufs; i++) {
		...
		sw_prod = RING_RX_AGG(NEXT_RX_AGG(sw_prod));
	}
			   ...
			   struct rx_tpa_start_cmp_ext *tpa_start1)
{
	...
	cons = RING_RX(NEXT_RX(cons));
	rxr->rx_next_cons = RING_RX(NEXT_RX(cons));
	cons_rx_buf = &rxr->rx_buf_ring[cons];
				    ...
				    struct hwrm_async_event_cmpl *cmpl)
{
	...
	case ASYNC_EVENT_CMPL_EVENT_ID_UDCC_SESSION_CHANGE: {
		...
		netif_notice(bp, hw, bp->dev,
			     "UDCC event session_id: 0x%x, session opcode: 0x%x\n",
			     data1, data2);
		...
		udcc_work->bp = bp;
		udcc_work->session_id = BNXT_EVENT_UDCC_SESSION_ID(data1);
		udcc_work->session_opcode = BNXT_EVENT_UDCC_SESSION_OPCODE(data2);
	...
	while (1) {
		...
		if (cmp_type == CMP_TYPE_TX_L2_CMP ||
			...
			if (cmp_type == CMP_TYPE_TX_L2_COAL_CMP)
				...
				txr->tx_hw_cons = RING_TX(TX_OPAQUE_PROD(opaque));
...
{
	if (bnapi->events & BNXT_TX_CMP_EVENT)
		bnapi->tx_int(bp, bnapi);
	...
	for (i = 0; i < bp->nr_vnics; i++) {
		...
		if (bp->vnic_info[i].rss_hash_key) {
			if (i == 0) {
				...
				bool update_toeplitz_key = false;
				int k;

				if (bp->rss_hash_key_valid || bp->rss_hash_key_updated) {
					...
					get_random_bytes(vnic->rss_hash_key,
						      HW_HASH_KEY_SIZE);
					memcpy(bp->rss_hash_key, vnic->rss_hash_key,
					       HW_HASH_KEY_SIZE);
					update_toeplitz_key = true;
				}
				if (bp->rss_hash_key_updated) {
					update_toeplitz_key = true;
					bp->rss_hash_key_updated = false;
				}
				if (!update_toeplitz_key)
					continue;
				for (k = 0; k < 8; k++) {
...
static u32 bnxt_get_rss_flow_tuple_len(struct bnxt *bp, struct flow_keys *fkeys)
{
	if (fkeys->basic.n_proto == htons(ETH_P_IP)) {
		if ((fkeys->basic.ip_proto == IPPROTO_TCP &&
			...
			return (sizeof(fkeys->addrs.v4addrs) + sizeof(fkeys->ports));
	...
	if (fkeys->basic.n_proto == htons(ETH_P_IPV6)) {
		if ((fkeys->basic.ip_proto == IPPROTO_TCP &&
			...
			return (sizeof(fkeys->addrs.v6addrs) + sizeof(fkeys->ports));
	...
	if (fltr->base.flags & BNXT_ACT_RSS_CTX) {
		list_for_each_entry_safe(rss_ctx, tmp, &bp->rss_ctx_list, list) {
			if (rss_ctx->index == fltr->base.fw_vnic_id) {
				req->dst_id = rss_ctx->vnic->fw_vnic_id;
				break;
			}
		}
		return;
...
int bnxt_alloc_rss_indir_tbl(struct bnxt *bp, struct bnxt_rss_ctx *rss_ctx)
{
	...
	if (rss_ctx) {
		rss_ctx->rss_indir_tbl_entries = entries;
...
void bnxt_set_dflt_rss_indir_tbl(struct bnxt *bp, struct bnxt_rss_ctx *rss_ctx)
{
	...
	if (rss_ctx) {
		...
		pad = rss_ctx->rss_indir_tbl_entries - max_entries;
	} else {
...
#ifdef CONFIG_BNXT_CUSTOM_FLOWER_OFFLOAD
		j = (vnic->q_index != INVALID_HW_RING_ID) ? vnic->q_index : bp->rss_indir_tbl[i];
#else
				    ...
				    u32 ring_type, u32 map_index)
{
	...
	case HWRM_RING_ALLOC_RX:
		...
		if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
			...
			req->stat_ctx_id = cpu_to_le32(grp_info->fw_stats_ctx);
			req->enables |= cpu_to_le32(RING_ALLOC_REQ_ENABLES_RX_BUF_SIZE_VALID);
			if (NET_IP_ALIGN == 2)
	...
	case HWRM_RING_ALLOC_AGG:
		if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
			...
			req->stat_ctx_id = cpu_to_le32(grp_info->fw_stats_ctx);
			req->enables |= cpu_to_le32(RING_ALLOC_REQ_ENABLES_RX_RING_ID_VALID |
...
__bnxt_hwrm_reserve_pf_rings(struct bnxt *bp, struct bnxt_hw_rings *hwr)
{
	...
	if (BNXT_NEW_RM(bp)) {
		...
		enables |= hwr->grp ? FUNC_CFG_REQ_ENABLES_NUM_RSSCOS_CTXS : 0;
		...
		if (bp->flags & BNXT_FLAG_CHIP_P5_PLUS) {
			 ...
			 */
			num_rss_ctxs = bnxt_get_nr_rss_ctxs(bp, hwr->grp);
			req->num_rsscos_ctxs = cpu_to_le16(num_rss_ctxs * hwr->grp + num_rss_ctxs);
#else
			if (BNXT_SUPPORTS_NTUPLE_VNIC(bp))
				rss_ctx *= hwr->vnic;
...
#ifdef BNXT_THOR2
	else if (BNXT_CHIP_P7(bp) && BNXT_PF(bp))
		bnxt_hwrm_func_backing_store_qcaps_v2(bp);
#endif
	if (BNXT_VF(bp))
		return 0;
...
static int bnxt_backing_store_cfg_v2(struct bnxt *bp)
{
	...
	if (BNXT_PF(bp) && ktls) {
		ctxm = &ctx->ctx_arr[BNXT_CTX_TKC];
		rc = bnxt_setup_ctxm_pg_tbls(bp, ctxm, ktls->tkc.max_ctx, 1);
		if (rc)
			return rc;
		ctxm = &ctx->ctx_arr[BNXT_CTX_RKC];
		rc = bnxt_setup_ctxm_pg_tbls(bp, ctxm, ktls->rkc.max_ctx, 1);
		if (rc)
			return rc;
	}
	...
	if (bp->hdbr_info.hdbr_enabled) {
		for (type = BNXT_CTX_SQDBS; type <= BNXT_CTX_CQDBS; type++) {
			...
			if (rc)
				return rc;
			rc = bnxt_hdbr_l2_init(bp, type - BNXT_CTX_SQDBS);
			if (rc)
				return rc;
		}
	}
...
#ifdef BNXT_THOR2
	else if ((bp->flags & BNXT_FLAG_CHIP_P7) && BNXT_PF(bp)) {
		rc = bnxt_backing_store_cfg_v2(bp);
		if (rc)
			return rc;
...
static int __bnxt_hwrm_func_qcaps(struct bnxt *bp)
{
	...
	if (flags_ext2 & FUNC_QCAPS_RESP_FLAGS_EXT2_HW_LAG_SUPPORTED)
...
{
	struct bnxt_vnic_info tmp_vnic = { 0 };
	int rc, i, nr_ctxs;
	...
	if (bp->ktls_info) {
		tmp_vnic.metadata_format = VNIC_UPDATE_REQ_METADATA_FORMAT_TYPE_4;
		bnxt_hwrm_vnic_update(bp, &tmp_vnic,
...
static void bnxt_init_napi(struct bnxt *bp)
{
	...
	if (bp->flags & BNXT_FLAG_USING_MSIX) {
		...
		if (BNXT_CHIP_TYPE_NITRO_A0(bp)) {
			...
			___netif_napi_add(bp->dev, &bnapi->napi,
					  bnxt_poll_nitroa0);
			napi_hash_add(&bnapi->napi);
		}
	} else {
		bnapi = bp->bnapi[0];
		___netif_napi_add(bp->dev, &bnapi->napi, bnxt_poll);
		napi_hash_add(&bnapi->napi);
	}
}
...
static int bnxt_open(struct net_device *dev)
{
	...
	if (BNXT_CHIP_P7(bp) && BNXT_PF(bp)) {
		/* Initialize TFC here until ULP port is complete
		 */
		bp->tfp = vzalloc(sizeof(*tfcp));
		if (!bp->tfp)
			return -ENOMEM;
		tfcp = bp->tfp;
		tfcp->bp = bp;
		...
		rc = tfc_open(tfcp);
		if (rc) {
			netdev_err(bp->dev, "tfc_open() fails rc=%d\n", rc);
			return rc;
		}
	}
...
void __bnxt_close_nic(struct bnxt *bp, bool irq_re_init, bool link_re_init)
{
...
static int bnxt_close(struct net_device *dev)
{
	...
	if (BNXT_CHIP_P7(bp) && BNXT_PF(bp)) {
		/* Free TFC here until ULP port is complete
		 */
		tfc_close(bp->tfp);
		vfree(bp->tfp);
	}
...
{
	int max_vnics, max_rss_ctxs, rss_ctx = 1;
	struct bnxt_hw_rings hwr = {0};
	...
	if (BNXT_SUPPORTS_NTUPLE_VNIC(bp)) {
		...
		hwr.vnic = 2;
		if (new_rss_ctx)
			hwr.vnic += bp->num_rss_ctx + 1;

		hwr.grp = bp->rx_nr_rings;
		rss_ctx = bnxt_get_nr_rss_ctxs(bp, hwr.grp) * hwr.vnic;
...
check_reserve_vnic:
	...
	if (!(bp->flags & BNXT_FLAG_CHIP_P5_PLUS) && !(bp->rss_cap & BNXT_RSS_CAP_NEW_RSS_CAP))
		rss_ctx = hwr.vnic;

	if (hwr.vnic > max_vnics || rss_ctx > max_rss_ctxs) {
		if (bp->rx_nr_rings > 1)
			netdev_warn(bp->dev,
	...
	if (hwr.vnic <= bp->hw_resc.resv_vnics && rss_ctx <= bp->hw_resc.resv_rsscos_ctxs)
		return true;
			    ...
			    struct bnxt_ntuple_filter *f2)
{
	...
	return keys1->ports.src == keys2->ports.src &&
	       ...
	       f1->l2_fltr->l2_key.filter_key == f2->l2_fltr->l2_key.filter_key;
}
...
static void bnxt_log_live_data(void *d)
{
	struct bnxt *bp = d;
