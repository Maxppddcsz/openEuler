...
static int bnxt_hwrm_port_ts_query(struct bnxt *bp, u32 flags, u64 *ts)
{
	...
	if (flags == PORT_TS_QUERY_REQ_FLAGS_PATH_TX) {
		...
		req->ts_req_timeout = cpu_to_le16(BNXT_PTP_QTS_TIMEOUT(bp));
...
reissue:
	...
	if (!rc) {
		if (!(bp->flags & BNXT_FLAG_CHIP_P5_PLUS)) {
			if (ts < ptp->skb_pre_xmit_ts && retry_cnt++ < 2) {
				netdev_warn(bp->dev, "ts: %lld < current_time: %lld retry_cnt: %d\n",
					    ts, ptp->current_time, retry_cnt);
				goto reissue;
			}

			if (retry_cnt >= 2) {
				netdev_err(bp->dev, "Could not get Tx TS even after retry!\n");
				goto exit;
			}
		}
...
#endif
int bnxt_get_tx_ts(struct bnxt *bp, struct sk_buff *skb)
{
	struct bnxt_ptp_cfg *ptp = bp->ptp_cfg;
