...
#define BNXT_PARTITION_CAP_BITS						\
	  (FUNC_QCAPS_RESP_XID_PARTITION_CAP_KTLS_TKC |			\
	   FUNC_QCAPS_RESP_XID_PARTITION_CAP_KTLS_RKC)
...
static int bnxt_set_partition_mode(struct bnxt *bp)
{
	...
	req->xid_partition_cfg =
		cpu_to_le16(FUNC_CFG_REQ_XID_PARTITION_CFG_KTLS_TKC |
			    FUNC_CFG_REQ_XID_PARTITION_CFG_KTLS_RKC);
			       ...
			       u32 kid, struct crypto_prefix_cmd *pre_cmd)
{
	...
	psbd = (void *)&txr->tx_desc_ring[TX_RING(prod)][TX_IDX(prod)];
	...
	prod = NEXT_TX(prod);
	pcmd = (void *)&txr->tx_desc_ring[TX_RING(prod)][TX_IDX(prod)];
