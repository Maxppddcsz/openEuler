...
static int bnxt_del_one_rss_ctx(struct bnxt *bp, struct bnxt_rss_ctx *rss_ctx, bool all)
{
	...
	for (i = 0; i < BNXT_MAX_CTX_PER_VNIC; i++) {
		if (rss_ctx->vnic->fw_rss_cos_lb_ctx[i] != INVALID_HW_RING_ID)
			bnxt_hwrm_vnic_ctx_free_one(bp, rss_ctx->vnic, i);
	}
	if (all) {
		list_for_each_entry_safe(usr_fltr, tmp, &bp->usr_fltr_list, list) {
			if ((usr_fltr->flags & BNXT_ACT_RSS_CTX) &&
				...
				ntp_fltr = container_of(usr_fltr, struct bnxt_ntuple_filter, base);
		...
		if (rss_ctx->vnic->rss_table)
			dma_free_coherent(&bp->pdev->dev, rss_ctx->vnic->rss_table_size,
					  rss_ctx->vnic->rss_table,
					  rss_ctx->vnic->rss_table_dma_addr);
...
void bnxt_hwrm_realloc_rss_ctx_vnic(struct bnxt *bp)
{
	struct bnxt_rss_ctx *rss_ctx, *tmp;

	list_for_each_entry_safe(rss_ctx, tmp, &bp->rss_ctx_list, list) {
		if (bnxt_hwrm_vnic_alloc(bp, rss_ctx->vnic, 0, bp->rx_nr_rings))
			goto err;
		if (bnxt_hwrm_vnic_set_tpa(bp, rss_ctx->vnic, bp->flags & BNXT_FLAG_TPA))
			goto err;
		if (__bnxt_setup_vnic_p5(bp, rss_ctx->vnic))
			goto err;
		continue;
err:
...
static struct bnxt_rss_ctx *bnxt_alloc_rss_ctx_and_vnic(struct bnxt *bp)
{
	struct bnxt_rss_ctx *rss_ctx = NULL;

	rss_ctx = kzalloc(sizeof(*rss_ctx), GFP_KERNEL);
	if (rss_ctx) {
		rss_ctx->vnic = kzalloc(sizeof(*rss_ctx->vnic), GFP_KERNEL);
		if (!rss_ctx->vnic) {
			kfree(rss_ctx);
			return NULL;
		}
...
void bnxt_init_multi_rss_ctx(struct bnxt *bp)
{
	bp->rss_ctx_bmap = bitmap_zalloc(BNXT_RSS_CTX_BMAP_LEN, GFP_KERNEL);
	if (bp->rss_ctx_bmap) {
		/* burn index 0 since we cannot have context 0 */
		bitmap_find_free_region(bp->rss_ctx_bmap, BNXT_RSS_CTX_BMAP_LEN, 0);
		INIT_LIST_HEAD(&bp->rss_ctx_list);
		bp->flags |= BNXT_FLAG_MULTI_RSS_CTX;
	}
}

static struct bnxt_rss_ctx *bnxt_get_rss_ctx_from_index(struct bnxt *bp, u32 index)
{
	struct bnxt_rss_ctx *rss_ctx, *tmp;
...
static int bnxt_alloc_rss_ctx_rss_table(struct bnxt *bp, struct bnxt_rss_ctx *rss_ctx)
{
	int size = L1_CACHE_ALIGN(BNXT_MAX_RSS_TABLE_SIZE_P5);

	rss_ctx->vnic->rss_table_size = size + HW_HASH_KEY_SIZE;
	rss_ctx->vnic->rss_table = dma_alloc_coherent(&bp->pdev->dev,
						      rss_ctx->vnic->rss_table_size,
						      &rss_ctx->vnic->rss_table_dma_addr,
						      GFP_KERNEL);
	if (!rss_ctx->vnic->rss_table)
		return -ENOMEM;

	rss_ctx->vnic->rss_hash_key = ((void *)rss_ctx->vnic->rss_table) + size;
	rss_ctx->vnic->rss_hash_key_dma_addr = rss_ctx->vnic->rss_table_dma_addr + size;

	return 0;
}

static int bnxt_set_rxfh_context(struct net_device *dev,
				 ...
				 bool delete)
{
	...
	if (*rss_context != ETH_RXFH_CONTEXT_ALLOC) {
		...
		if (!rss_ctx)
			return -EINVAL;
		if (delete)
			return bnxt_del_one_rss_ctx(bp, rss_ctx, true);
		modify = true;
		goto modify_context;
	}
	...
	rss_ctx = bnxt_alloc_rss_ctx_and_vnic(bp);
	if (!rss_ctx)
		return -ENOMEM;

	rss_ctx->vnic->flags |= BNXT_VNIC_RSSCTX_FLAG;
	rss_ctx->vnic->vnic_id = BNXT_VNIC_ID_INVALID;
	...
	memcpy(rss_ctx->vnic->rss_hash_key, bp->rss_hash_key, HW_HASH_KEY_SIZE);

	rc = bnxt_hwrm_vnic_alloc(bp, rss_ctx->vnic, 0, bp->rx_nr_rings);
	if (rc)
		goto out;

	rc = bnxt_hwrm_vnic_set_tpa(bp, rss_ctx->vnic, bp->flags & BNXT_FLAG_TPA);
	if (rc)
		goto out;
modify_context:
	if (indir) {
		...
		pad = rss_ctx->rss_indir_tbl_entries - tbl_size;
		if (pad)
			memset(&rss_ctx->rss_indir_tbl[i], 0, pad * sizeof(u16));
	}

	if (key)
		memcpy(rss_ctx->vnic->rss_hash_key, key, HW_HASH_KEY_SIZE);

	if (modify)
		return bnxt_hwrm_vnic_rss_cfg(bp, rss_ctx->vnic);

	rc = __bnxt_setup_vnic_p5(bp, rss_ctx->vnic);
	if (rc)
		goto out;

	bit_id = bitmap_find_free_region(bp->rss_ctx_bmap, BNXT_RSS_CTX_BMAP_LEN, 0);
	if (bit_id < 0) {
		rc = -ENOMEM;
		goto out;
	}
...
static int bnxt_get_rxfh_context(struct net_device *dev, u32 *indir,
				 u8 *key, u8 *hfunc, u32 rss_context)
{
	...
	if (key)
		memcpy(key, rss_ctx->vnic->rss_hash_key, HW_HASH_KEY_SIZE);
	return 0;
}
#else
void bnxt_clear_rss_ctxs(struct bnxt *bp, bool all)
...
void bnxt_reconfig_rss_ctx_vnic(struct bnxt *bp)
{
}
...
static int bnxt_get_num_tx_sw_stats(struct bnxt *bp)
{
	if (bp->tx_push_mode == BNXT_PUSH_MODE_NONE)
		return 0;
	return NUM_RING_TX_SW_STATS;
}
...
static int bnxt_get_num_ring_stats(struct bnxt *bp)
{
	...
	tx = NUM_RING_TX_HW_STATS + bnxt_get_num_tx_sw_stats(bp);
...
skip_tpa_ring_stats:
		...
		if (is_tx_ring(bp, i)) {
			for (k = 0; k < bnxt_get_num_tx_sw_stats(bp); j++, k++)
				buf[j] = sw[k];
		}
...
skip_tpa_stats:
			...
			if (is_tx_ring(bp, i)) {
				num_str = bnxt_get_num_tx_sw_stats(bp);
				for (j = 0; j < num_str; j++) {
					sprintf(buf, "[%d]: %s", i,
...
static int bnxt_set_dump(struct net_device *dev, struct ethtool_dump *dump)
{
	...
	if (dump->flag > BNXT_DUMP_DRIVER) {
		netdev_info(dev, "Supports only Live(0), Crash(1) and Driver(2) dumps.\n");
		return -EINVAL;
	}
...
int bnxt_set_priv_flags(struct net_device *dev, u32 flags)
{
	struct bnxt *bp = netdev_priv(dev);
	bool reload = false;
	int rc = 0;
...
u32 bnxt_get_priv_flags(struct net_device *dev)
{
	struct bnxt *bp = netdev_priv(dev);
	u32 flags = 0;
