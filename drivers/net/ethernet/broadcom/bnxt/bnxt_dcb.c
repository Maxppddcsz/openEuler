 ...
 */
static struct hwrm_queue_cos2bw_qcfg_output *
...
static __le32 bnxt_get_max_bw_from_queue(struct bnxt *bp,
					 struct hwrm_queue_cos2bw_qcfg_output *resp, u8 queue_id)
{
	...
	for (i = 0; i < bp->max_tc; i++, data += sizeof(cos2bw.cfg)) {
		...
		if (cos2bw.queue_id == queue_id)
			return cos2bw.max_bw;
	}
...
static int bnxt_hwrm_queue_cos2bw_cfg(struct bnxt *bp, struct ieee_ets *ets,
				      u8 max_tc)
{
	struct hwrm_queue_cos2bw_qcfg_output *cos2bw_qcfg_resp;
