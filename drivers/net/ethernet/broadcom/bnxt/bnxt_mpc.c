			...
			void *data, uint len, unsigned long handle)
{
	...
	bds = DIV_ROUND_UP(len + sizeof(*txbd) - 1, sizeof(*txbd));
	total_bds = bds + 1;
	...
	tx_buf = &txr->tx_mpc_buf_ring[RING_TX(prod)];
...
int bnxt_mpc_cmp(struct bnxt *bp, struct bnxt_cp_ring_info *cpr, u32 *raw_cons)
{
	...
	if (!bnxt_mpc_unsolicit(mpcmp)) {
		...
		mpc_buf = &txr->tx_mpc_buf_ring[RING_TX(tx_cons)];
		handle = mpc_buf->handle;
		tx_cons += mpc_buf->inline_bds;
		txr->tx_cons = tx_cons;
		txr->tx_hw_cons = RING_TX(tx_cons);
	}
