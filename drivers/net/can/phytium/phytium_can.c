...
enum phytium_can_reg {
	...
	CAN_ARB_RATE_CTRL	= 0x08,		/* Arbitration rate control
						   register */
	CAN_DAT_RATE_CTRL	= 0x0c,		/* Data rate control register */
	CAN_ACC_ID0		= 0x10,		/* Acceptance identifier0
						   register */
	CAN_ACC_ID1		= 0x14,		/* Acceptance identifier1
						   register */
	CAN_ACC_ID2		= 0x18,		/* Acceptance identifier2
						   register */
	CAN_ACC_ID3		= 0x1c,		/* Acceptance identifier3
						   register */
	CAN_ACC_ID0_MASK	= 0x20,		/* Acceptance identifier0
						   mask register */
	CAN_ACC_ID1_MASK	= 0x24,		/* Acceptance identifier1
						   mask register */
	CAN_ACC_ID2_MASK	= 0x28,		/* Acceptance identifier2
						   mask register */
	CAN_ACC_ID3_MASK	= 0x2c,		/* Acceptance identifier3
						   mask register */
	CAN_XFER_STS		= 0x30,		/* Transfer status register */
	CAN_ERR_CNT		= 0x34,		/* Error counter register */
	CAN_FIFO_CNT		= 0x38,		/* FIFO counter register */
	CAN_DMA_CTRL		= 0x3c,		/* DMA request control
						   register */
	CAN_XFER_EN		= 0x40,		/* Transfer enable register */
	CAN_INTR1		= 0x44,		/* Interrupt register 1 */
	CAN_FRM_INFO		= 0x48,		/* Frame valid number
						   register */
	...
	CAN_RX_INFO_FIFO	= 0x300,	/* RX information FIFO shadow
						   register */
	CAN_PIDR4		= 0xfd0,	/* Peripheral Identification
						   Register 4 */
	CAN_PIDR0		= 0xfe0,	/* Peripheral Identification
						   Register 0 */
	CAN_PIDR1		= 0xfe4,	/* Peripheral Identification
						   Register 1 */
	CAN_PIDR2		= 0xfe8,	/* Peripheral Identification
						   Register 2 */
	CAN_PIDR3		= 0xfec,	/* Peripheral Identification
						   Register 3 */
	CAN_CIDR0		= 0xff0,	/* Component Identification
						   Register 0 */
	CAN_CIDR1		= 0xff4,	/* Component Identification
						   Register 1 */
	CAN_CIDR2		= 0xff8,	/* Component Identification
						   Register 2 */
	CAN_CIDR3		= 0xffc,	/* Component Identification
						   Register 3 */
};
...
#define CTRL_RST		BIT(7)	/* Write 1 to soft reset and self
					   clear */
#define CTRL_RFEIDF		BIT(8)	/* Allow RX frame end interrupt during
					   ID filtered frame */
#define CTRL_RFEDT		BIT(9)	/* Allow RX frame end interrupt during
					   TX frame */
...
#define INTR_EN_MASK	 (INTR_RFIE | INTR_REIE | INTR_TEIE)
...
#define ARB_RATE_CTRL_ARJW	GENMASK(1, 0)	/* Arbitration field resync
						   jump width */
#define ARB_RATE_CTRL_APRS	GENMASK(4, 2)	/* Arbitration field
						   propagation segment */
#define ARB_RATE_CTRL_APH1S	GENMASK(7, 5)	/* Arbitration field phase1
						   segment */
#define ARB_RATE_CTRL_APH2S	GENMASK(10, 8)	/* Arbitration field phase2
						   segment */
#define ARB_RATE_CTRL_APD	GENMASK(28, 16)	/* Arbitration field prescaler
						   divider */

/* Data rate control register (DAT_RATE_CTRL) */
#define DAT_RATE_CTRL_DRJW	GENMASK(1, 0)	/* Data field resync jump
						   width */
#define DAT_RATE_CTRL_DPRS	GENMASK(4, 2)	/* Data field propagation
						   segment */
...
#define DAT_RATE_CTRL_DPD	GENMASK(28, 16)	/* Data field prescaler
						   divider */
...
#define ACC_IDX_MASK_AID_MASK	GENMASK(28, 0)	/* Acceptance identifier
						   mask */
...
#define FIFO_CNT_RFN		GENMASK(6, 0)	/* Receive FIFO valid data
						   number */
#define FIFO_CNT_TFN		GENMASK(22, 16)	/* Transmit FIFO valid data
						   number */

/* DMA request control register (DMA_CTRL) */
#define DMA_CTRL_RFTH		GENMASK(5, 0)	/* Receive FIFO DMA request
						   threshold */
#define DMA_CTRL_RFRE		BIT(6)		/* Receive FIFO DMA request
						   enable */
#define DMA_CTRL_TFTH		GENMASK(21, 16)	/* Transmit FIFO DMA request
						   threshold */
#define DMA_CTRL_TFRE		BIT(22)		/* Transmit FIFO DMA request
						   enable */
...
#define RX_INFO_FIFO_WNORF	GENMASK(4, 0)	/* Word (4-byte) number of
						   current receive frame */
#define RX_INFO_FIFO_RORF	BIT(5)		/* RTR value of current
						   receive frame */
#define RX_INFO_FIFO_FORF	BIT(6)		/* FDF value of current
						   receive frame */
#define RX_INFO_FIFO_IORF	BIT(7)		/* IDE value of current
						   receive frame */
...
static inline u32 phytium_can_read(const struct phytium_can_dev *cdev, enum
		phytium_can_reg reg)
{
	return readl(cdev->base + reg);
}

static inline void phytium_can_write(const struct phytium_can_dev *cdev, enum
		phytium_can_reg reg, u32 val)
{
	writel(val, cdev->base + reg);
}

static inline void phytium_can_enable_all_interrupts(struct phytium_can_dev
		*cdev)
{
	phytium_can_write(cdev, CAN_INTR, INTR_EN_MASK);
}

static inline void phytium_can_disable_all_interrupt(struct phytium_can_dev
		*cdev)
{
...
static int phytium_can_read_fifo(struct net_device *dev)
{
	...
	if (id & CAN_IDE_MASK) {
		...
		if (dlc & CAN_ID2_FDF_MASK) {
			...
			if (dlc & CANFD_ID2_ESI_MASK)
				cf->flags |= CANFD_ESI;
			cf->len = can_dlc2len((dlc & CANFD_ID2_DLC_MASK) >>
					CANFD_ID2_DLC_OFF);
		} else {
			/* CAN extended frame */
			cf->len = get_can_dlc((dlc & CAN_ID2_DLC_MASK) >>
					CAN_ID2_DLC_OFF);
		}
		...
		if (id & CAN_ID1_FDF_MASK) {
			...
			if (id & CANFD_ID1_ESI_MASK)
				cf->flags |= CANFD_ESI;
			cf->len = can_dlc2len((id & CANFD_ID1_DLC_MASK) >>
					CANFD_ID1_DLC_OFF);
		} else {
			/* CAN extended frame */
			cf->len = get_can_dlc((id & CAN_ID1_DLC_MASK) >>
					CAN_ID1_DLC_OFF);
		}
	...
	if (!(cf->can_id & CAN_RTR_FLAG))
		...
		for (i = 0; i < cf->len; i += 4)
			*(__be32 *)(cf->data + i) = phytium_can_read(cdev,
					CAN_RX_FIFO);
...
static int phytium_can_rx_handler(struct net_device *dev, int quota)
{
	...
	if (isr & INTR_REIS) {
		...
		rx_work_or_err = phytium_can_do_rx_poll(dev, (quota -
					work_done));
		if (rx_work_or_err < 0)
...
static netdev_tx_t phytium_can_tx_handler(struct phytium_can_dev *cdev)
{
	...
	tx_fifo_used = (phytium_can_read(cdev, CAN_FIFO_CNT) &
			FIFO_CNT_TFN) >> 16;
...
static int phytium_can_set_bittiming(struct net_device *dev)
{
	...
	if (is_config_mode) {
		netdev_alert(dev, "BUG! Cannot set bittiming - CAN is not in
				config mode\n");
		return -EPERM;
	}
