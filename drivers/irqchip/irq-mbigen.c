 ...
 * bit[31:24] mbi_type:
 * - 0x10b support vTimer irqbypass
 */
#define MBIGEN_NODE_CFG_OFFSET			0x0004
#define MBIGEN_TYPE_MASK			0x03000000
#define MBIGEN_TYPE_SHIFT			24
#define TYPE_VITMER_ENABLED			0x02

#define REG_VTIMER_MBIGEN_WIDTH			0x0004
#define PPIS_PER_MBIGEN_NODE			32
#define REG_VTIMER_MBIGEN_TYPE_OFFSET		0x1000
#define REG_VTIMER_MBIGEN_SET_AUTO_CLR_OFFSET	0x1100
#define REG_VTIMER_MBIGEN_CLR_AUTO_CLR_OFFSET	0x1110
#define REG_VTIMER_GIC_SET_AUTO_CLR_OFFSET	0x1150
#define REG_VTIMER_GIC_CLR_AUTO_CLR_OFFSET	0x1160
#define REG_VTIMER_MBIGEN_ATV_STAT_OFFSET	0x1120
#define REG_VTIMER_MBIGEN_VEC_OFFSET		0x1200
#define REG_VTIMER_MBIGEN_ATV_CLR_OFFSET	0xa008
...
#define get_abs_offset(cpu, cpu_base) \
	((get_gicr_paddr(cpu) - get_gicr_paddr(cpu_base)) / GICR_LENGTH)
...
struct mbigen_device {
	...
	void __iomem			*base;
	bool				vtimer_bypass_enabled;
	struct vtimer_mbigen_device	*vtimer_mbigen_chip;
};

static struct arch_timer_kvm_info *vtimer_kvm_info;
...
void vtimer_mbigen_set_vector(int cpu_id, u16 vpeid)
{
	...
	addr = chip->base + REG_VTIMER_MBIGEN_VEC_OFFSET +
	       cpu_abs_offset * REG_VTIMER_MBIGEN_WIDTH;
...
bool vtimer_mbigen_get_active(int cpu_id)
{
	...
	cpu_abs_offset = get_abs_offset(cpu_id, chip->cpu_base);
	addr = chip->base + REG_VTIMER_MBIGEN_ATV_STAT_OFFSET +
		(cpu_abs_offset / PPIS_PER_MBIGEN_NODE) * REG_VTIMER_MBIGEN_WIDTH;
...
void vtimer_mbigen_set_active(int cpu_id, bool set)
{
	...
	offset = set ? REG_VTIMER_MBIGEN_ATV_STAT_OFFSET :
		 REG_VTIMER_MBIGEN_ATV_CLR_OFFSET;
	addr = chip->base + offset +
		(cpu_abs_offset / PPIS_PER_MBIGEN_NODE) * REG_VTIMER_MBIGEN_WIDTH;
	val = 1 << (cpu_abs_offset % PPIS_PER_MBIGEN_NODE);
...
{
	struct vtimer_mbigen_device *chip = get_vtimer_mbigen(cpu_id);
	void __iomem *addr;
	int cpu_abs_offset;
	u32 val, mask;
	...
	cpu_abs_offset = get_abs_offset(cpu_id, chip->cpu_base);
	addr = chip->base + REG_VTIMER_MBIGEN_TYPE_OFFSET +
	       (cpu_abs_offset / PPIS_PER_MBIGEN_NODE) * REG_VTIMER_MBIGEN_WIDTH;
...
{
	vtimer_kvm_info = arch_timer_get_kvm_info();

	vtimer_kvm_info->irqbypass_flag |= VT_EXPANDDEV_PROBED;
}

static int vtimer_mbigen_chip_get_aff3(struct vtimer_mbigen_device *chip)
{
...
static int vtimer_mbigen_device_probe(struct platform_device *pdev)
{
	...
	vtimer_mgn_chip->mpidr_aff3 = vtimer_mbigen_chip_get_aff3(vtimer_mgn_chip);
...
static int mbigen_device_probe(struct platform_device *pdev)
{
	...
	return vtimer_mbigen_device_probe(pdev);
}
