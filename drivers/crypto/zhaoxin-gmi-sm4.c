				...
				SM4_CIPHER *sm4_data, u64 count)
{
	...
	__asm__ __volatile__(
		...
		"pop %%rbx"
		:
		: "r"((u64)input), "r"((u64)output), "r"(rax), "r"((u64)key), "r"((u64)count), "r"((u64)iv)
...
static inline u8 *rep_xcrypt_ebc_ONE(const u8 *input, u8 *output, void *key,
				u8 *iv, SM4_CIPHER *sm4_data, u64 count)
{
	SM4_CIPHER cw;
...
static int sm4_cipher_ctr(struct skcipher_request *req, SM4_CIPHER *cw)
{
	...
	while ((nbytes = walk.nbytes) > 0) {
		...
		while (nbytes >= SM4_BLOCK_SIZE) {
			...
			iv = rep_xcrypt_ctr(walk.src.virt.addr, walk.dst.virt.addr, ctx->rkey_enc, walk.iv, cw,
					blocks);
		...
		while (blocks--) {
			...
			for (n = 0; n < SM4_BLOCK_SIZE; n += sizeof(size_t))
				*(size_t *)(walk.dst.virt.addr + n) =
					*(size_t *)(walk.iv + n) ^ *(size_t *)(walk.src.virt.addr + n);
		...
		while (blocks--) {
			...
			if (cw->cword.b.encdec)
				...
				for (n = 0; n < SM4_BLOCK_SIZE; n += sizeof(size_t)) {
					...
					*(size_t *)(walk.dst.virt.addr + n) = *(size_t *)(walk.iv + n) ^ t;
					*(size_t *)(walk.iv + n) = t;
				}
	...
	if (((c->x86 == 6) && (c->x86_model >= 0x0f)) ||
		...
		if (!boot_cpu_has(X86_FEATURE_CCS) || !boot_cpu_has(X86_FEATURE_CCS_EN)) {
			...
			if ((edx & 0x0030) == 0x0030) {
				pr_info("GMI SM3/SM4 is detected by CPUID\n");
				return 0;
			}
			return -ENODEV;
		}
