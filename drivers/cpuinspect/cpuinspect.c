...
{
	unsigned int cpu = (unsigned int)(unsigned long)data;
	unsigned int inspect_times = 0, group, sleep_us, ret;
	ktime_t start_time, duration;

	while (!kthread_should_stop()) {
		if (inspect_times >= ci_core.inspect_times)
			break;
