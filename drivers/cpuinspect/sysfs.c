			  ...
			  const char *buf, size_t size)
{
	 ...
	 * inspection process.
	 */
	if (start_patrol == ci_core.inspect_on) {
...
{
	sysfs_notify(&cpu_subsys.dev_root->kobj, "cpuinspect", "result");
}
...
static DEVICE_ULONG_ATTR(patrol_times, 0644, ci_core.inspect_times);
...
static struct attribute *cpuinspect_attrs[] = {
	...
	&dev_attr_patrol_times.attr.attr,
 ...
 */
int cpuinspect_add_interface(struct device *dev)
{
	return sysfs_create_group(&dev->kobj, &cpuinspect_attr_group);
}
 ...
 * @dev: the target device
 */
void cpuinspect_remove_interface(struct device *dev)
{
	sysfs_remove_group(&dev->kobj, &cpuinspect_attr_group);
}
