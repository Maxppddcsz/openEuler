					  ...
					  struct ubcore_user_ctl_out *out)
{
	...
	memcpy(&cfg_in, (void *)in->addr, min(in->len, (uint32_t)sizeof(cfg_in)));
				       ...
				       struct ubcore_user_ctl_out *out)
{
	...
	memcpy(&attr_in, (void *)in->addr, min(in->len, (uint32_t)sizeof(attr_in)));
				       ...
				       struct ubcore_user_ctl_out *out)
{
	...
	memcpy((void *)out->addr, &info_out, min(out->len, (uint32_t)sizeof(info_out)));
