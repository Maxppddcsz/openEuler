...
The kernel could try to enable mappings for different sizes, eg, 64K for
arm64, BIT0 for file mapping, BIT1 for anonymous mapping, and THP size
page, BIT3 for anonymous mapping, where 64K for arm64 is dependent on
BIT3 being turned on, the above feature are disabled by default, and
could enable the above feature by writing the corresponding bit to 1::

	echo 0x4 >/sys/kernel/mm/transparent_hugepage/thp_mapping_align
	echo 0x5 >/sys/kernel/mm/transparent_hugepage/thp_mapping_align
	echo 0x6 >/sys/kernel/mm/transparent_hugepage/thp_mapping_align
	echo 0x7 >/sys/kernel/mm/transparent_hugepage/thp_mapping_align
