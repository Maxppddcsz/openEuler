...
int attach_manually(void)
{
	...
	err = bpf_prog_attach(bpf_program__fd(skel->progs.netacc_sockops), cg_fd, BPF_CGROUP_SOCK_OPS, 0);
	if (err) {
...
void detach_manually(void)
{
	...
	err = bpf_prog_detach2(bpf_program__fd(skel->progs.netacc_sockops), cg_fd, BPF_CGROUP_SOCK_OPS);
	if (err)
