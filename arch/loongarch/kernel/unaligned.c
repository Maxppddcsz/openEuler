...
static void emulate_load_store_insn(struct pt_regs *regs, void __user *addr, unsigned int *pc)
{
	...
	if (insn.reg2i12_format.opcode == ldd_op ||
		...
		res = copy_from_user_nofault(&value, addr, 8);
		if (res)
			goto fault;
		...
		res = copy_from_user_nofault(&value, addr, 4);
		value = (unsigned long)(signed long)(int)value;
		if (res)
			goto fault;
		regs->regs[insn.reg2i12_format.rd] = value;
		...
		res = copy_to_user_nofault(addr, &value, 8);
		if (res)
			goto fault;
		...
		value = regs->regs[insn.reg2i12_format.rd];
		res = copy_to_user_nofault(addr, &value, 4);
		if (res)
			goto fault;
