...
{
	unsigned long ret;
	struct cca_device_cert cmd = {0};
	unsigned char *cert_buf;

	cert_buf = kmalloc(GRANULE_SIZE, GFP_KERNEL);
	if (!cert_buf)
		return -ENOMEM;
	ret = tsi_get_device_cert(&cmd, cert_buf);
	if (ret != TSI_SUCCESS) {
		pr_err("tmm_tsi: tsi call tsi_get_device_cert failed (%lu)!\n", ret);
		kfree(cert_buf);
		return -EIO;
	}
	memcpy(cmd.value, cert_buf, cmd.size);
	kfree(cert_buf);

	ret = copy_to_user(arg, &cmd, sizeof(struct cca_device_cert));
	if (ret) {
		pr_err("tmm_tsi: copy data to user failed (%lu)!\n", ret);
		return -EFAULT;
	}
