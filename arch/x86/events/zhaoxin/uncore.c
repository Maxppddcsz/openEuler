...
#define YONGFENG_MAX_SUBNODE_NUMBER    8
static int yongfeng_pcibus_limit[YONGFENG_MAX_SUBNODE_NUMBER];

/* get CPU topology register */
#define BJ_GLOBAL_STATUS_MSR	0x1610
#define BJ_HDW_CONFIG_MSR	0X1628

/* Wudaokou/Lujiazui event control */
...
static void get_topology_number(void)
{
	...
	if (boot_cpu_data.x86_model != ZHAOXIN_FAM7_YONGFENG)
		return;
...
static int get_pcibus_limit(void)
{
	...
	yongfeng_pcibus_limit[i++] = (val & 0x1f) << 3 | 0x7;
	yongfeng_pcibus_limit[i++] = (val >> 8 & 0x1f) << 3 | 0x7;
	if (dies_per_socket == 2) {
		yongfeng_pcibus_limit[i++] = (val >> 16 & 0x1f) << 3 | 0x7;
		yongfeng_pcibus_limit[i++] = (val >> 24 & 0x1f) << 3 | 0x7;
	}
...
static int uncore_pcibus_to_subnodeid(struct pci_bus *bus)
{
	...
	for (i = 0; i < YONGFENG_MAX_SUBNODE_NUMBER; i++) {
		if (bus->number < yongfeng_pcibus_limit[i])
			break;
	}
...
static void get_topology_info(void)
{
	...
	for_each_present_cpu(cpu) {
		...
		/* only yongfeng needs cluster and subnode info */
		if (boot_cpu_data.x86_model != ZHAOXIN_FAM7_YONGFENG)
			continue;
...
/*Wudaokou/Lujiazui uncore ops start*/
...
/*Wudaokou/Lujiazui uncore ops end*/
		...
		struct perf_event *event)
{
	...
	pci_write_config_dword(pdev, hwc->config_base,
			hwc->config | YONGFENG_PMON_CTL_EN);
}
...
static int uncore_event_cpu_offline(unsigned int cpu)
{
	...
	if (x86_model == ZHAOXIN_FAM7_YONGFENG)
...
static int yongfeng_event_cpu_online(unsigned int cpu)
{
	...
	cluster_target = cpumask_any_and(&uncore_cpu_cluster_mask, topology_cluster_core_cpumask(cpu));
	subnode_target = cpumask_any_and(&uncore_cpu_subnode_mask, topology_subnode_core_cpumask(cpu));
...
{
	int ret = yongfeng_pci2node_map_init();      //do nothing
...
static const struct x86_cpu_id zhaoxin_uncore_match[] __initconst = {
	CENTAUR_UNCORE_MODEL_MATCH(ZHAOXIN_FAM7_WUDAOKOU,	wudaokou_uncore_init),
	CENTAUR_UNCORE_MODEL_MATCH(ZHAOXIN_FAM7_LUJIAZUI,	wudaokou_uncore_init),
	CENTAUR_UNCORE_MODEL_MATCH(ZHAOXIN_FAM7_YONGFENG,	yongfeng_uncore_init),
	ZHAOXIN_UNCORE_MODEL_MATCH(ZHAOXIN_FAM7_WUDAOKOU,	wudaokou_uncore_init),
	ZHAOXIN_UNCORE_MODEL_MATCH(ZHAOXIN_FAM7_LUJIAZUI,	wudaokou_uncore_init),
	ZHAOXIN_UNCORE_MODEL_MATCH(ZHAOXIN_FAM7_YONGFENG,	yongfeng_uncore_init),
	{},
};
