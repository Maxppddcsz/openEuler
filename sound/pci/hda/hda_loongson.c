...
MODULE_PARM_DESC(index, "Index value for Loongson HD audio interface.");
module_param_array(id, charp, NULL, 0444);
MODULE_PARM_DESC(id, "ID string for Loongson HD audio interface.");
module_param_array(enable, bool, NULL, 0444);
MODULE_PARM_DESC(enable, "Enable Loongson HD audio interface.");
...
static int azx_plat_probe(struct platform_device *pdev)
{
	...
	err = azx_create(card, NULL, pdev, dev, AZX_DRIVER_ICH | AZX_DCAPS_LOONGSON_HDA_WORKAROUND, &chip);
	if (err < 0)
		goto out_free;
...
static struct of_device_id audio_id_table[] = {
	{ .compatible = "loongson,loongson3-hda", },
};
#endif
