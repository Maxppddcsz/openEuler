...
static int es8323_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	iface = snd_soc_component_read(codec, ES8323_IFACE);
	adciface = snd_soc_component_read(codec, ES8323_ADC_IFACE);
	daciface = snd_soc_component_read(codec, ES8323_DAC_IFACE);
		...
		struct snd_soc_dai *dai)
{
	struct snd_soc_component *codec = dai->component;
	struct es8323_priv *es8323 = snd_soc_component_get_drvdata(codec);
	u16 srate = snd_soc_component_read(codec, ES8323_IFACE) & 0x80;
	u16 adciface = snd_soc_component_read(codec, ES8323_ADC_IFACE) & 0xE3;
	u16 daciface = snd_soc_component_read(codec, ES8323_DAC_IFACE) & 0xC7;
	int coeff;
...
static int es8323_probe(struct snd_soc_component *codec)
{
	...
	ret = snd_soc_component_read(codec, 0x09);
	ret = snd_soc_component_read(codec, 0x12);
	ret = snd_soc_component_read(codec, 0x13);
