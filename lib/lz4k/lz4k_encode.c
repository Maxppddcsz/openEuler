	...
	bool check_out)
{
	...
	if (nr_bytes_max < nr_mask) {
		...
		bytes_left = nr_bytes_max - nr_mask;
...
static uint8_t *out_r_bytes_left(
	...
	/* =false when out_max>=encoded_bytes_max(in_max), =true otherwise */
{
...
inline unsigned int encode_state_bytes_min(void)
{
	unsigned int bytes_total = (1U << HT_BYTES_LOG2);
	return bytes_total;
}

#if !defined(LZ4K_DELTA) && !defined(LZ4K_MAX_CR)

unsigned int lz4k_encode_state_bytes_min(void)
{
...
{
	const unsigned int gain_max = 64 > (in_max >> 6) ? 64 : (in_max >> 6);
	const unsigned int out_limit_min = in_max < out_max ? in_max : out_max;
	const uint8_t *volatile in_end = (const uint8_t *)in + in_max;
	const uint8_t *volatile out_end = (uint8_t *)out + out_max;
	const void *volatile state_end = (uint8_t *)state + encode_state_bytes_min();
